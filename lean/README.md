# BluefinDecryptxP2pe Lean SDK



The Lean SDK for the BluefinDecryptxP2pe API — an entity-oriented client
...
[Releases](https://github.com/voxgig-sdk/bluefin-decryptx-p2pe-sdk/releases)) or from a source checkout. The runtime is
...
def main : IO Unit := do
  let ctx ← mkCtx
  (do
    let opts ← newMap #[("base", .str "https://api.example.com"), ("apikey", .str (← IO.getEnv "BLUEFIN_DECRYPTX_P2PE").getD "")]
    let sdk ← Sdk.newSdk opts
    let result ← Attestation.list sdk (← emptyMap) (← emptyMap)
