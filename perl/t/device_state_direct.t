...
DIRECT_LIST: {
  my $setup = device_state_direct_setup([
    { 'id' => 'direct01' },
    { 'id' => 'direct02' },
  ]);
  my ($_should_skip, $_reason) = BluefinDecryptxP2peTestRunner::is_control_skipped(
    'direct', 'direct-list-device_state', $setup->{live} ? 'live' : 'unit');
  if ($_should_skip) {
    note($_reason || 'skipped via sdk-test-control.json');
    pass('direct-list-device_state: skipped via sdk-test-control.json');
    last DIRECT_LIST;
  }
  ...
  if ($setup->{live}) {
    ...
    if (defined $result->{err}) {
      ...
      pass('direct-list-device_state: skipped (live)');
      last DIRECT_LIST;
    }
    unless ($result->{ok}) {
    ...
    pass('direct-list-device_state: live ok');
  }
  else {
    ok(!defined $result->{err}, 'direct-list-device_state: no error');
    ok($result->{ok}, 'direct-list-device_state: ok');
    is(BluefinDecryptxP2peHelpers::to_int($result->{status}), 200, 'direct-list-device_state: status');
    ok(Voxgig::Struct::islist($result->{data}), 'direct-list-device_state: data is array');
    is(scalar @{ $result->{data} }, 2, 'direct-list-device_state: data length');
    is(scalar @{ $setup->{calls} }, 1, 'direct-list-device_state: 1 call');
  }
}


sub device_state_direct_setup {
  my ($mockres) = @_;
