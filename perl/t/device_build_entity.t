...

{
  ...
  my $ent = $testsdk->DeviceBuild(undef);
  ok(defined $ent, 'device_build: create instance');
}

BASIC_FLOW: {
  my $setup = device_build_basic_setup(undef);
  my $_live = $setup->{live} ? 1 : 0;
  # Per-op sdk-test-control.json skip.
  for my $_op ('list', 'load') {
    my ($_should_skip, $_reason) = BluefinDecryptxP2peTestRunner::is_control_skipped(
      'entityOp', "device_build." . $_op, $_live ? 'live' : 'unit');
    if ($_should_skip) {
      note($_reason || 'skipped via sdk-test-control.json');
      pass('device_build: basic flow skipped via sdk-test-control.json');
      last BASIC_FLOW;
    }
  }
  ...
  $V{device_build_ref01_data_raw} = Voxgig::Struct::items(BluefinDecryptxP2peHelpers::to_map(
    BluefinDecryptxP2peHelpers::gpath($setup->{data}, 'existing.device_build')));
  $V{device_build_ref01_data} = undef;
  if (@{ $V{device_build_ref01_data_raw} || [] }) {
    $V{device_build_ref01_data} = BluefinDecryptxP2peHelpers::to_map($V{device_build_ref01_data_raw}[0][1]);
  }

  # LIST
  $V{device_build_ref01_ent} = $client->DeviceBuild(undef);
  $V{device_build_ref01_match} = {};

  $V{device_build_ref01_list_result} = $V{device_build_ref01_ent}->list($V{device_build_ref01_match}, undef);
  ok(Voxgig::Struct::islist($V{device_build_ref01_list_result}), 'device_build list: is array');

  # LOAD
  $V{device_build_ref01_match_dt0} = {
...
sub device_build_basic_setup {
  my ($extra) = @_;
  BluefinDecryptxP2peTestRunner::load_env_local();

  my $entity_data_file = Cwd::abs_path(
    "$FindBin::Bin/../../.sdk/test/entity/device_build/DeviceBuildTestData.json");
  my $entity_data = do {
