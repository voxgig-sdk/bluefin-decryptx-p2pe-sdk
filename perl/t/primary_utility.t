...

{
  ...
  my $spec_client = BluefinDecryptxP2peSDK->test(undef, $setup_opts);
...
runset('operator.basic', get_spec($PRIMARY, 'operator', 'basic'), sub {
  my ($entry) = @_;
  my $in_val = $entry->{in} || {};
  my $op = BluefinDecryptxP2peOperation->new($in_val);
  return {
    'entity' => $op->{entity},
    'name' => $op->{name},
    'input' => $op->{input},
...

{
  ...
  my $auth_client = BluefinDecryptxP2peSDK->test(undef, $setup_opts);
