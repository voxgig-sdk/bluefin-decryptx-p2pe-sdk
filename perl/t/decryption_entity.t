...
BASIC_FLOW: {
  ...
  if ($setup->{synthetic_only}) {
    note('live entity test uses synthetic IDs from fixture - set BLUEFINDECRYPTXP_PE_TEST_DECRYPTION_ENTID JSON to run live');
  ...
  $V{decryption_ref01_data} = BluefinDecryptxP2peHelpers::to_map($V{decryption_ref01_data_result});
...
sub decryption_basic_setup {
  ...
  # skip.
  my $entid_env_raw = $ENV{'BLUEFINDECRYPTXP_PE_TEST_DECRYPTION_ENTID'};
  ...
  my $idmap_resolved = BluefinDecryptxP2peHelpers::to_map($env->{'BLUEFINDECRYPTXP_PE_TEST_DECRYPTION_ENTID'});
