...
BASIC_FLOW: {
  ...
  if ($setup->{synthetic_only}) {
    note('live entity test uses synthetic IDs from fixture - set BLUEFIN_DECRYPTX_P2PE_TEST_SUCCESS_ENTID JSON to run live');
  ...
  $V{success_ref01_data} = BluefinDecryptxP2peHelpers::to_map(ref($V{success_ref01_data_result}) && $V{success_ref01_data_result}->can('data_get') ? $V{success_ref01_data_result}->data_get : $V{success_ref01_data_result});
...
sub success_basic_setup {
  ...
  # skip.
  my $entid_env_raw = $ENV{'BLUEFIN_DECRYPTX_P2PE_TEST_SUCCESS_ENTID'};
  ...
  my $idmap_resolved = BluefinDecryptxP2peHelpers::to_map($env->{'BLUEFIN_DECRYPTX_P2PE_TEST_SUCCESS_ENTID'});
