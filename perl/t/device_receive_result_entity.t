...
BASIC_FLOW: {
  ...
  if ($setup->{synthetic_only}) {
    note('live entity test uses synthetic IDs from fixture - set BLUEFINDECRYPTXP_PE_TEST_DEVICE_RECEIVE_RESULT_ENTID JSON to run live');
  ...
  $V{device_receive_result_ref01_data} = BluefinDecryptxP2peHelpers::to_map($V{device_receive_result_ref01_data_result});
...
sub device_receive_result_basic_setup {
  ...
  # skip.
  my $entid_env_raw = $ENV{'BLUEFINDECRYPTXP_PE_TEST_DEVICE_RECEIVE_RESULT_ENTID'};
  ...
  my $idmap_resolved = BluefinDecryptxP2peHelpers::to_map($env->{'BLUEFINDECRYPTXP_PE_TEST_DEVICE_RECEIVE_RESULT_ENTID'});
