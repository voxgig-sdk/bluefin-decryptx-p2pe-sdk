...
my $runner = BluefinDecryptxP2peOmni::make_runner($corpus_path, $client);
my $R = $runner->('struct');
...
my $NULLMARK = BluefinDecryptxP2peOmni::NULLMARK();
...
{
  my $td = BluefinDecryptxP2peOmni::tostruct(
...
{
  my $basic = BluefinDecryptxP2peOmni::tostruct(
...
grun('inject.string', $inject_spec->{string},
  sub { $S->{inject}->($_[0]{val}, $_[0]{store},
    { modify => \&BluefinDecryptxP2peOmni::null_modifier }) });
