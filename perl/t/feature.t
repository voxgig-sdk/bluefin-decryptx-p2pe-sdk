...
# operation pipeline. $features is a list of { name =>, options => }.
{
  ...
  sub new {
    ...
    $self->{utility} = BluefinDecryptxP2peUtility->new;
    ...
    for my $fspec (@$features) {
      ...
      my $f = BluefinDecryptxP2peFeatures::make_feature($name);
  ...
  sub op {
    ...
    my $out = eval {
      ...
      if (Scalar::Util::blessed($ctx->{out}{point})
        && $ctx->{out}{point}->isa('BluefinDecryptxP2peError')) {
        die $ctx->{out}{point};
      }
      ...
      $ctx->{response} = Voxgig::Struct::ismap($fetched)
        ? BluefinDecryptxP2peResponse->new($fetched) : undef;
    ...
    if (my $err = $@) {
      if (Scalar::Util::blessed($err) && $err->isa('BluefinDecryptxP2peError')) {
        $ctx->{ctrl}{err} = $err;
  ...
  sub populate_result {
    ...
    my $result = BluefinDecryptxP2peResult->new({});
    $ctx->{result} = $result;
...
if (has_feature('retry')) {
...

  {
    ...
    my ($server, $calls) = recording_server(sub {
      my ($n) = @_;
      return $n < 3
        ? (undef, BluefinDecryptxP2peError->new('boom', 'boom'))
