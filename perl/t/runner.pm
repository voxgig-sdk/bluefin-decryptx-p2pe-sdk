...
package BluefinDecryptxP2peTestRunner;
...
sub is_control_skipped {
  ...
  my $skip = BluefinDecryptxP2peHelpers::gpath($ctrl, "test.skip.$mode") || {};
  my $items = $skip->{$kind} || [];
...
sub live_client_options {
  my $ctrl = load_test_control();
  my $opts = BluefinDecryptxP2peHelpers::gpath($ctrl, 'test.client.options');
...
sub live_delay_ms {
  my $ctrl = load_test_control();
  my $v = BluefinDecryptxP2peHelpers::gpath($ctrl, 'test.live.delayMs');
