...
BASIC_FLOW: {
  ...
  if ($setup->{synthetic_only}) {
    note('live entity test uses synthetic IDs from fixture - set BLUEFINDECRYPTXP_PE_TEST_TRANSACTION_ENTID JSON to run live');
  ...
  $V{transaction_ref01_data} = BluefinDecryptxP2peHelpers::to_map($V{transaction_ref01_data_result});
  ...
  $V{transaction_ref01_data_dt0_load_result} = BluefinDecryptxP2peHelpers::to_map($V{transaction_ref01_data_dt0_loaded});
...
sub transaction_basic_setup {
  ...
  # skip.
  my $entid_env_raw = $ENV{'BLUEFINDECRYPTXP_PE_TEST_TRANSACTION_ENTID'};
  ...
  my $idmap_resolved = BluefinDecryptxP2peHelpers::to_map($env->{'BLUEFINDECRYPTXP_PE_TEST_TRANSACTION_ENTID'});
