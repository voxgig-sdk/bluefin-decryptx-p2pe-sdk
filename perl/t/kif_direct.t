...
DIRECT_LIST: {
  my $setup = kif_direct_setup([
    { 'id' => 'direct01' },
    { 'id' => 'direct02' },
  ]);
  my ($_should_skip, $_reason) = BluefinDecryptxP2peTestRunner::is_control_skipped(
    'direct', 'direct-list-kif', $setup->{live} ? 'live' : 'unit');
  if ($_should_skip) {
    note($_reason || 'skipped via sdk-test-control.json');
    pass('direct-list-kif: skipped via sdk-test-control.json');
    last DIRECT_LIST;
  }
  ...
  if ($setup->{live}) {
    ...
    if (defined $result->{err}) {
      ...
      pass('direct-list-kif: skipped (live)');
      last DIRECT_LIST;
    }
    unless ($result->{ok}) {
    ...
    pass('direct-list-kif: live ok');
  }
  else {
    ok(!defined $result->{err}, 'direct-list-kif: no error');
    ok($result->{ok}, 'direct-list-kif: ok');
    is(BluefinDecryptxP2peHelpers::to_int($result->{status}), 200, 'direct-list-kif: status');
    ok(Voxgig::Struct::islist($result->{data}), 'direct-list-kif: data is array');
    is(scalar @{ $result->{data} }, 2, 'direct-list-kif: data length');
    is(scalar @{ $setup->{calls} }, 1, 'direct-list-kif: 1 call');
  }
}


sub kif_direct_setup {
  my ($mockres) = @_;
