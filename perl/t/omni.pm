...
#    `contextify` answers a TIED plain hash (BluefinDecryptxP2peOmniViewTie) whose
...
package BluefinDecryptxP2peOmniEntityRef;
...
# --------------------------------------------------------------------------
package BluefinDecryptxP2peOmniViewTie;
...
sub _iserr { return Scalar::Util::blessed($_[0]) && $_[0]->isa('BluefinDecryptxP2peError') }
...
sub _wrap {
  ...
  if ('ok' eq $name && Scalar::Util::blessed($t) && $t->isa('BluefinDecryptxP2peResult')
    && defined $val && !ref $val) {
  ...
  if (ref($val) eq 'HASH' && tied(%$val)
    && tied(%$val)->isa('BluefinDecryptxP2peOmniViewTie')) {
    my $inner = tied(%$val)->{target};
    return $inner if $seen->{ Scalar::Util::refaddr($inner) };
    return BluefinDecryptxP2peOmni::objview($inner,
  ...
  if (Scalar::Util::blessed($val)) {
    ...
    if (('HASH' eq (Scalar::Util::reftype($val) || ''))) {
      ...
      return BluefinDecryptxP2peOmni::objview($val,
        { %$seen, Scalar::Util::refaddr($t) => 1 });
    }
  ...
  if (ref($val) eq 'HASH' || ref($val) eq 'ARRAY') {
    return BluefinDecryptxP2peOmni::toomni($val);
  }
...
# --------------------------------------------------------------------------
package BluefinDecryptxP2peOmniProviderTie;
...
package BluefinDecryptxP2peOmniProvider;
...
# The resolver proper.
# --------------------------------------------------------------------------
package BluefinDecryptxP2peOmni;
...
sub objview {
  my ($target, $seen) = @_;
  tie my %view, 'BluefinDecryptxP2peOmniViewTie', $target, $seen;
  return \%view;
}
...
sub isview {
  ...
  return ref($val) eq 'HASH' && tied(%$val)
    && tied(%$val)->isa('BluefinDecryptxP2peOmniViewTie') ? 1 : 0;
}
...
sub enrich {
  ...
  if (Voxgig::Struct::ismap($ctxmap->{spec}) && !Scalar::Util::blessed($ctxmap->{spec})) {
    $ctx->{spec} = BluefinDecryptxP2peSpec->new($ctxmap->{spec});
  }

  my $res_map = $ctxmap->{result};
  if (Voxgig::Struct::ismap($res_map) && !Scalar::Util::blessed($res_map)) {
    $ctx->{result} = BluefinDecryptxP2peResult->new($res_map);
    if (Voxgig::Struct::ismap($res_map->{err})
      && defined $res_map->{err}{message} && !ref $res_map->{err}{message}) {
      $ctx->{result}{err} = BluefinDecryptxP2peError->new('', $res_map->{err}{message});
    }
  }

  my $resp_map = $ctxmap->{response};
  if (Voxgig::Struct::ismap($resp_map) && !Scalar::Util::blessed($resp_map)) {
    $ctx->{response} = BluefinDecryptxP2peResponse->new($resp_map);
    if (BluefinDecryptxP2peHelpers::rb_truthy($resp_map->{body})) {
...
sub sdkhooks {
  ...
  my $struct = $utility->{struct} || BluefinDecryptxP2peHelpers::struct_facade();
  ...
  my $subject = sub {
    ...
    $found = $utility->{ BluefinDecryptxP2peOmniViewTie::_snake($name) } if !defined $found;
  ...
  my $contextify = sub {
    ...
    if (Voxgig::Struct::ismap($ent) && !Scalar::Util::blessed($ent)
      ...
      $ctxmap->{entity} = BluefinDecryptxP2peOmniEntityRef->new($ent->{name});
    }
    my $ctx = BluefinDecryptxP2peContext->new($ctxmap, undef);
    enrich($ctxmap, $ctx);
    $ctx->{utility} = $utility;
...
sub sdkprovider {
  my ($sdk) = @_;
  tie my %provider, 'BluefinDecryptxP2peOmniProviderTie', $sdk, sdkhooks($sdk);
  return bless \%provider, 'BluefinDecryptxP2peOmniProvider';
}
