...
BASIC_FLOW: {
  ...
  if ($setup->{synthetic_only}) {
    note('live entity test uses synthetic IDs from fixture - set BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_RKI_ACTIVATE_RESULT_ENTID JSON to run live');
  ...
  $V{device_rki_activate_result_ref01_data} = BluefinDecryptxP2peHelpers::to_map(ref($V{device_rki_activate_result_ref01_data_result}) && $V{device_rki_activate_result_ref01_data_result}->can('data_get') ? $V{device_rki_activate_result_ref01_data_result}->data_get : $V{device_rki_activate_result_ref01_data_result});
...
sub device_rki_activate_result_basic_setup {
  ...
  # skip.
  my $entid_env_raw = $ENV{'BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_RKI_ACTIVATE_RESULT_ENTID'};
