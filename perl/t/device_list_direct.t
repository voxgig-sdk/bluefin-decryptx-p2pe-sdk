...
DIRECT_LOAD: {
  my $setup = device_list_direct_setup({ 'id' => 'direct01' });
  my ($_should_skip, $_reason) = BluefinDecryptxP2peTestRunner::is_control_skipped(
    'direct', 'direct-load-device_list', $setup->{live} ? 'live' : 'unit');
  if ($_should_skip) {
    note($_reason || 'skipped via sdk-test-control.json');
    pass('direct-load-device_list: skipped via sdk-test-control.json');
    last DIRECT_LOAD;
  }
  if ($setup->{live}) {
    note('live direct-load needs real ID - set *_ENTID env var with real IDs to run');
    pass('direct-load-device_list: skipped (live)');
    last DIRECT_LOAD;
  }
  ...
  if ($setup->{live}) {
    ...
    pass('direct-load-device_list: live ok');
  }
  else {
    ok(!defined $result->{err}, 'direct-load-device_list: no error');
    ok($result->{ok}, 'direct-load-device_list: ok');
    is(BluefinDecryptxP2peHelpers::to_int($result->{status}), 200, 'direct-load-device_list: status');
    ok(defined $result->{data}, 'direct-load-device_list: data');
    if (Voxgig::Struct::ismap($result->{data})) {
      is($result->{data}{id}, 'direct01', 'direct-load-device_list: id');
    }
    is(scalar @{ $setup->{calls} }, 1, 'direct-load-device_list: 1 call');
  }
}


sub device_list_direct_setup {
  my ($mockres) = @_;
