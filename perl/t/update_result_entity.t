...
BASIC_FLOW: {
  ...
  if ($setup->{synthetic_only}) {
    note('live entity test uses synthetic IDs from fixture - set BLUEFINDECRYPTXP_PE_TEST_UPDATE_RESULT_ENTID JSON to run live');
  ...
  $V{update_result_ref01_data} = BluefinDecryptxP2peHelpers::to_map($V{update_result_ref01_data_result});
  ...
  $V{update_result_ref01_resdata_up0} = BluefinDecryptxP2peHelpers::to_map($V{update_result_ref01_resdata_up0_result});
...
sub update_result_basic_setup {
  ...
  # skip.
  my $entid_env_raw = $ENV{'BLUEFINDECRYPTXP_PE_TEST_UPDATE_RESULT_ENTID'};
  ...
  my $idmap_resolved = BluefinDecryptxP2peHelpers::to_map($env->{'BLUEFINDECRYPTXP_PE_TEST_UPDATE_RESULT_ENTID'});
