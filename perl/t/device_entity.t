...
BASIC_FLOW: {
  ...
  if ($setup->{synthetic_only}) {
    note('live entity test uses synthetic IDs from fixture - set BLUEFINDECRYPTXP_PE_TEST_DEVICE_ENTID JSON to run live');
  ...
  $V{device_ref01_data} = BluefinDecryptxP2peHelpers::to_map($V{device_ref01_data_result});
  ...
  $V{device_ref01_data_dt0_load_result} = BluefinDecryptxP2peHelpers::to_map($V{device_ref01_data_dt0_loaded});
...
sub device_basic_setup {
  ...
  # skip.
  my $entid_env_raw = $ENV{'BLUEFINDECRYPTXP_PE_TEST_DEVICE_ENTID'};
  ...
  my $idmap_resolved = BluefinDecryptxP2peHelpers::to_map($env->{'BLUEFINDECRYPTXP_PE_TEST_DEVICE_ENTID'});
