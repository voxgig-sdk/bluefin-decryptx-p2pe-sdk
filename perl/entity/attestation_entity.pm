...
sub make {
  ...
  return AttestationEntity->new($self->{_client}, $opts);
}
...
# Returns the current Attestation data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Attestation fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; AttestationLoadMatch
...
# Returns the loaded Attestation data (hashref); dies with BluefinDecryptxP2peError
# on failure.
sub load {
...
#
# reqmatch: match filter hashref (any subset of Attestation fields;
# AttestationListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching Attestation items as an arrayref; dies with
...
#
# reqdata: body data hashref (AttestationCreateData shape). ctrl: optional
# per-call control. Returns the created Attestation data (hashref); dies
