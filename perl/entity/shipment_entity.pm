...
sub make {
  ...
  return ShipmentEntity->new($self->{_client}, $opts);
}
...
# Returns the current Shipment data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Shipment fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; ShipmentLoadMatch
...
# Returns the loaded Shipment data (hashref); dies with BluefinDecryptxP2peError
# on failure.
sub load {
...
#
# reqmatch: match filter hashref (any subset of Shipment fields;
# ShipmentListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching Shipment items as an arrayref; dies with
...
#
# reqdata: body data hashref (ShipmentCreateData shape). ctrl: optional
# per-call control. Returns the created Shipment data (hashref); dies
