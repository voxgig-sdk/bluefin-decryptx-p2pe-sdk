# BluefinDecryptxP2pe SDK DeviceCustodyList entity
...
package DeviceCustodyListEntity;
...
sub make {
  ...
  return DeviceCustodyListEntity->new($self->{_client}, $opts);
}
...
# Returns the current DeviceCustodyList data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of DeviceCustodyList fields).
sub match_get {
  my ($self) = @_;
...
# List DeviceCustodyList items matching the given filter.
#
# reqmatch: match filter hashref (any subset of DeviceCustodyList fields;
# DeviceCustodyListListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching DeviceCustodyList items as an arrayref; dies with
