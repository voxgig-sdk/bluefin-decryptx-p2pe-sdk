# BluefinDecryptxP2pe SDK Kif entity
...
sub make {
  ...
  return KifEntity->new($self->{_client}, $opts);
}
...
# Returns the current Kif data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Kif fields).
sub match_get {
  my ($self) = @_;
...
# List Kif items matching the given filter.
#
# reqmatch: match filter hashref (any subset of Kif fields;
# KifListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching Kif items as an arrayref; dies with
