# BluefinDecryptxP2pe SDK DeviceReceiveResult entity
...
package DeviceReceiveResultEntity;
...
sub make {
  ...
  return DeviceReceiveResultEntity->new($self->{_client}, $opts);
}
...
# Returns the current DeviceReceiveResult data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of DeviceReceiveResult fields).
sub match_get {
  my ($self) = @_;
...
# Create a new DeviceReceiveResult.
#
# reqdata: body data hashref (DeviceReceiveResultCreateData shape). ctrl: optional
# per-call control. Returns the created DeviceReceiveResult data (hashref); dies
