# BluefinDecryptxP2pe SDK Device entity
...
sub make {
  ...
  return DeviceEntity->new($self->{_client}, $opts);
}
...
# Returns the current Device data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Device fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; DeviceLoadMatch
...
# Returns the loaded Device data (hashref); dies with BluefinDecryptxP2peError
# on failure.
sub load {
...
# List Device items matching the given filter.
#
# reqmatch: match filter hashref (any subset of Device fields;
# DeviceListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching Device items as an arrayref; dies with
...
# Create a new Device.
#
# reqdata: body data hashref (DeviceCreateData shape). ctrl: optional
# per-call control. Returns the created Device data (hashref); dies
