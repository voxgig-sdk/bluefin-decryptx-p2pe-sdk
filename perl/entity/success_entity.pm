# BluefinDecryptxP2pe SDK Success entity
...
sub make {
  ...
  return SuccessEntity->new($self->{_client}, $opts);
}
...
# Returns the current Success data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Success fields).
sub match_get {
  my ($self) = @_;
...
#
# reqdata: body data hashref (SuccessCreateData shape). ctrl: optional
# per-call control. Returns the created Success data (hashref); dies
...
# Remove an Success matching the given criteria.
#
# reqmatch: match criteria hashref (id/query fields; SuccessRemoveMatch
# shape). ctrl: optional per-call control. Returns the removed Success
