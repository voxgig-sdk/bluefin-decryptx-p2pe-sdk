# BluefinDecryptxP2pe SDK CreateResult entity
...
sub make {
  ...
  return CreateResultEntity->new($self->{_client}, $opts);
}
...
# Returns the current CreateResult data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of CreateResult fields).
sub match_get {
  my ($self) = @_;
...
# Create a new CreateResult.
#
# reqdata: body data hashref (CreateResultCreateData shape). ctrl: optional
# per-call control. Returns the created CreateResult data (hashref); dies
