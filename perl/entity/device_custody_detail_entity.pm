...
package DeviceCustodyDetailEntity;
...
sub make {
  ...
  return DeviceCustodyDetailEntity->new($self->{_client}, $opts);
}
...
# Returns the current DeviceCustodyDetail data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of DeviceCustodyDetail fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; DeviceCustodyDetailLoadMatch
...
# Returns the loaded DeviceCustodyDetail data (hashref); dies with BluefinDecryptxP2peError
# on failure.
sub load {
