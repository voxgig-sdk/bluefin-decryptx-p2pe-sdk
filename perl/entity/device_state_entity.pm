...
sub make {
  ...
  return DeviceStateEntity->new($self->{_client}, $opts);
}
...
# Returns the current DeviceState data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of DeviceState fields).
sub match_get {
  my ($self) = @_;
...
# List DeviceState items matching the given filter.
#
# reqmatch: match filter hashref (any subset of DeviceState fields;
# DeviceStateListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching DeviceState items as an arrayref; dies with
