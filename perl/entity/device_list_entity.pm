# BluefinDecryptxP2pe SDK DeviceList entity
...
sub make {
  ...
  return DeviceListEntity->new($self->{_client}, $opts);
}
...
# Returns the current DeviceList data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of DeviceList fields).
sub match_get {
  my ($self) = @_;
...
# Load a single DeviceList.
#
# reqmatch: match criteria hashref (id/query fields; DeviceListLoadMatch
...
# Returns the loaded DeviceList data (hashref); dies with BluefinDecryptxP2peError
# on failure.
sub load {
