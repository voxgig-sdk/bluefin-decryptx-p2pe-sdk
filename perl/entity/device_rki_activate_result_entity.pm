# BluefinDecryptxP2pe SDK DeviceRkiActivateResult entity
...
package DeviceRkiActivateResultEntity;
...
sub new {
  ...
  my $self = bless {
    _name => 'device_rki_activate_result',
    _client => $client,
...
sub make {
  ...
  return DeviceRkiActivateResultEntity->new($self->{_client}, $opts);
}
...
# Returns the current DeviceRkiActivateResult data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of DeviceRkiActivateResult fields).
sub match_get {
  my ($self) = @_;
...
# Create a new DeviceRkiActivateResult.
#
# reqdata: body data hashref (DeviceRkiActivateResultCreateData shape). ctrl: optional
# per-call control. Returns the created DeviceRkiActivateResult data (hashref); dies
