# BluefinDecryptxP2pe SDK Location entity
...
sub new {
  ...
  elsif (BluefinDecryptxP2peHelpers::is_false($entopts->{active})) {
    # keep false
  }
  else {
    $entopts->{active} = Voxgig::Struct::JTRUE();
...
sub data_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_data} = BluefinDecryptxP2peHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetData');
  }
  return;
}

# Returns the current Location data (hashref).
sub data_get {
  my ($self) = @_;
...
sub match_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_match} = BluefinDecryptxP2peHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetMatch');
  }
  return;
}

# Returns the current match filter (any subset of Location fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; LocationLoadMatch
...
# Returns the loaded Location data (hashref); dies with BluefinDecryptxP2peError
# on failure.
sub load {
  ...
  return $self->_run_op($ctx, sub {
    ...
    if ($result) {
      ...
      if ($result->{resdata}) {
        $self->{_data} = BluefinDecryptxP2peHelpers::to_map(
...
#
# reqmatch: match filter hashref (any subset of Location fields;
# LocationListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching Location items as an arrayref; dies with
# BluefinDecryptxP2peError on failure.
sub list {
  my ($self, $reqmatch, $ctrl) = @_;
  my $utility = $self->{_utility};
  my $ctx = $utility->{make_context}->({
...
#
# reqdata: body data hashref (LocationCreateData shape). ctrl: optional
# per-call control. Returns the created Location data (hashref); dies
# with BluefinDecryptxP2peError on failure.
sub create {
...
# Remove an Location matching the given criteria.
#
# reqmatch: match criteria hashref (id/query fields; LocationRemoveMatch
# shape). ctrl: optional per-call control. Returns the removed Location
# data (hashref); dies with BluefinDecryptxP2peError on failure.
sub remove {
  my ($self, $reqmatch, $ctrl) = @_;
  my $utility = $self->{_utility};
  my $ctx = $utility->{make_context}->({
...
sub stream {
  ...
  my $utility = $self->{_utility};
  $callopts = BluefinDecryptxP2peHelpers::to_map($callopts) || {};
  my $signal = BluefinDecryptxP2peHelpers::gp($callopts, 'signal');
  my $ctrl = BluefinDecryptxP2peHelpers::to_map(
    BluefinDecryptxP2peHelpers::gp($callopts, 'ctrl')) || {};
  ...
  my $ctx = $utility->{make_context}->({
    ...
    'match' => $self->{_match},
    'data' => $self->{_data},
    %{ BluefinDecryptxP2peHelpers::to_map($args) || {} },
  }, $self->{_entctx});

  # Outbound: expose an async-iterable/list payload so the request builder /
  # transport can stream it as the request body.
  my $body = BluefinDecryptxP2peHelpers::gp($callopts, 'body');
  if (defined $body) {
    my $reqdata = BluefinDecryptxP2peHelpers::to_map($ctx->{reqdata}) || {};
    $reqdata->{'body$'} = $body;
    $ctx->{reqdata} = $reqdata;
    $ctx->{stream_out} = $body;
  }
