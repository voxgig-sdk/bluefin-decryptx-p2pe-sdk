...
sub make {
  ...
  return DeviceTypeEntity->new($self->{_client}, $opts);
}
...
# Returns the current DeviceType data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of DeviceType fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; DeviceTypeLoadMatch
...
# Returns the loaded DeviceType data (hashref); dies with BluefinDecryptxP2peError
# on failure.
sub load {
...
#
# reqmatch: match filter hashref (any subset of DeviceType fields;
# DeviceTypeListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching DeviceType items as an arrayref; dies with
