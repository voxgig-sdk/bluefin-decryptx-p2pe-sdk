# BluefinDecryptxP2pe SDK Decryption entity
...
sub make {
  ...
  return DecryptionEntity->new($self->{_client}, $opts);
}
...
# Returns the current Decryption data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Decryption fields).
sub match_get {
  my ($self) = @_;
...
# Create a new Decryption.
#
# reqdata: body data hashref (DecryptionCreateData shape). ctrl: optional
# per-call control. Returns the created Decryption data (hashref); dies
