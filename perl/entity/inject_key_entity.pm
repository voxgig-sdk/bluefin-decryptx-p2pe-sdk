...
sub make {
  ...
  return InjectKeyEntity->new($self->{_client}, $opts);
}
...
# Returns the current InjectKey data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of InjectKey fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; InjectKeyLoadMatch
...
# Returns the loaded InjectKey data (hashref); dies with BluefinDecryptxP2peError
# on failure.
sub load {
...
#
# reqmatch: match filter hashref (any subset of InjectKey fields;
# InjectKeyListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching InjectKey items as an arrayref; dies with
