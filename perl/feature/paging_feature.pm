# BluefinDecryptxP2pe SDK paging feature
#
...
package BluefinDecryptxP2pePagingFeature;
...
sub init {
  ...
  $self->{active} = BluefinDecryptxP2peHelpers::is_true($self->{options}{active});
  return;
}
...
sub PreRequest {
  ...
  if ('graphql' eq (BluefinDecryptxP2peHelpers::gp($ctx->{point}, 'kind') // '')) {
...
sub _graphql_pre_request {
  ...
  my %declared;
  my $varlist = BluefinDecryptxP2peHelpers::gpath($ctx->{point}, 'graphql.vars');
  if (ref($varlist) eq 'ARRAY') {
    for my $v (@$varlist) {
      my $name = BluefinDecryptxP2peHelpers::gp($v, 'name');
...
sub PreResult {
  ...
  my $page = BluefinDecryptxP2peHelpers::gpath($ctx->{point}, 'graphql.page');
  if (ref($page) eq 'HASH' && Voxgig::Struct::ismap($body)) {
    ...
    if (defined $connpath && '' ne $connpath) {
      my $sub = BluefinDecryptxP2peHelpers::gpath($body, $connpath);
      $conn = $sub if defined $sub;
    }

    my $cursorpath = $page->{cursor};
    if (defined $cursorpath && '' ne $cursorpath) {
      my $cursor = BluefinDecryptxP2peHelpers::gpath($conn, $cursorpath);
      $paging->{cursor} = $cursor if defined $cursor;
    }

    my $morepath = $page->{more};
    if (defined $morepath && '' ne $morepath) {
      my $more = BluefinDecryptxP2peHelpers::gpath($conn, $morepath);
      if (BluefinDecryptxP2peHelpers::is_true($more) || BluefinDecryptxP2peHelpers::is_false($more)) {
        $paging->{hasMore} = BluefinDecryptxP2peHelpers::is_true($more) ? 1 : 0;
        $explicit_more = 1;
      }
    }
  }
  ...
  if (Voxgig::Struct::ismap($body)) {
    my $bnext = BluefinDecryptxP2peHelpers::gp($body, 'next');
    $paging->{next} = defined $paging->{next} ? $paging->{next} : $bnext
      if defined $bnext;
    my $bcursor = BluefinDecryptxP2peHelpers::gp($body, 'cursor');
    $paging->{cursor} = $bcursor if defined $bcursor;
    my $bnextcursor = BluefinDecryptxP2peHelpers::gp($body, 'nextCursor');
    $paging->{cursor} = $bnextcursor if defined $bnextcursor;
    my $bhasmore = $body->{hasMore};
    if (BluefinDecryptxP2peHelpers::is_true($bhasmore) || BluefinDecryptxP2peHelpers::is_false($bhasmore)) {
      $paging->{hasMore} = BluefinDecryptxP2peHelpers::is_true($bhasmore) ? 1 : 0;
      $explicit_more = 1;
    }
  }
