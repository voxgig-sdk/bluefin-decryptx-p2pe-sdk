# BluefinDecryptxP2pe SDK test feature
...
package BluefinDecryptxP2peTestFeature;
...
sub init {
  ...
  my $entity = BluefinDecryptxP2peHelpers::gp($options, 'entity');
  ...
  my $envelope = sub {
    ...
    my $tm = BluefinDecryptxP2peHelpers::gp($fctx->{point}, 'transform');
    my $spec = BluefinDecryptxP2peHelpers::gp($tm, 'res');
  ...
  my $test_fetcher = sub {
    ...
    my $op = $fctx->{op};
    my $entmap = BluefinDecryptxP2peHelpers::gp($entity, $op->{entity});
    ...
    my $resolve_match = sub {
      ...
      for my $src ($fctx->{match}, $fctx->{data}) {
        ...
        my $v = BluefinDecryptxP2peHelpers::gp($src, 'id');
    ...
    if ('load' eq $op->{name}) {
      ...
      my $ent = BluefinDecryptxP2peHelpers::ge($found, 0);
      return $respond->($fctx, 404, undef, { 'statusText' => 'Not found' })
        unless BluefinDecryptxP2peHelpers::rb_truthy($ent);
      ...
      if (Voxgig::Struct::ismap($fctx->{reqdata})) {
        ...
        if ($op->{alias}) {
          my $alias_id = BluefinDecryptxP2peHelpers::gp($op->{alias}, 'id');
      ...
      if (Voxgig::Struct::ismap($ent)) {
        my $id = BluefinDecryptxP2peHelpers::gp($ent, 'id');
  ...
  my $net = BluefinDecryptxP2peHelpers::gp($options, 'net');
...
sub make_netsim {
  ...
  my $do_sleep = sub {
    ...
    if (ref $net->{sleep} eq 'CODE') {
      ...
      BluefinDecryptxP2peHelpers::sleep_ms($ms);
    }
  };
  ...
  return sub {
    ...
    if (BluefinDecryptxP2peHelpers::is_true($net->{offline})) {
...
sub build_args {
  ...
  my $points = BluefinDecryptxP2peHelpers::gpath($ctx->{config},
    "entity.$entname.op.$opname.points");
  my $point = BluefinDecryptxP2peHelpers::ge($points, -1);

  my $params_path = BluefinDecryptxP2peHelpers::gpath($point, 'args.params');
    ...
    if ($keys) {
      for my $key (@$keys) {
        ...
        if ($is_id || $is_reqd) {
          ...
          my $ka = $op->{alias} ? BluefinDecryptxP2peHelpers::gp($op->{alias}, $key) : undef;
