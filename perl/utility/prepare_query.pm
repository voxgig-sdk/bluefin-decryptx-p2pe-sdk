# BluefinDecryptxP2pe SDK utility: prepare_query
...
$REGISTRY{prepare_query} = sub {
  ...
  my $params = [];
  if ($point) {
    my $p = BluefinDecryptxP2peHelpers::gp($point, 'params');
  ...
  if ($items) {
    for my $item (@$items) {
      my ($key, $val) = @$item;
      next unless BluefinDecryptxP2peHelpers::rb_truthy($val) && defined $key && !ref $key;
