# BluefinDecryptxP2pe SDK utility: make_spec
...
$REGISTRY{make_spec} = sub {
  ...
  my $base = BluefinDecryptxP2peHelpers::gp($options, 'base');
  $base = '' unless defined $base;
  my $prefix = BluefinDecryptxP2peHelpers::gp($options, 'prefix');
  $prefix = '' unless defined $prefix;
  my $suffix = BluefinDecryptxP2peHelpers::gp($options, 'suffix');
  $suffix = '' unless defined $suffix;

  my $parts = [];
  $parts = BluefinDecryptxP2peHelpers::gp($point, 'parts') if $point;
  ...
  my $allow_method = BluefinDecryptxP2peHelpers::gpath($options, 'allow.method');
  ...
  my $pkind = BluefinDecryptxP2peHelpers::gp($ctx->{point}, 'kind');
  if (defined $pkind && 'graphql' eq $pkind) {
    ...
    $ctx->{spec}{headers}{'content-type'} =
      $BluefinDecryptxP2peUtilities::GRAPHQL_CONTENT_TYPE;
  }
  else {
