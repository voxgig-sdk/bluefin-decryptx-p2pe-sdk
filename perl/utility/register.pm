...
$BluefinDecryptxP2peUtility::REGISTRAR = sub {
  my ($u) = @_;
  for my $k (keys %BluefinDecryptxP2peUtilities::REGISTRY) {
    $u->{$k} = $BluefinDecryptxP2peUtilities::REGISTRY{$k};
  }
  # The vendored struct utility, reachable as $utility->{struct}{<fn>}.
  $u->{struct} = BluefinDecryptxP2peHelpers::struct_facade();
};
