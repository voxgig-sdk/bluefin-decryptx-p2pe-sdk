# BluefinDecryptxP2pe SDK utility: make_options
...
$REGISTRY{make_options} = sub {
  ...
  my $custom_utils = BluefinDecryptxP2peHelpers::gp($options, 'utility');
  ...
  my $extend_raw = BluefinDecryptxP2peHelpers::gp($options, 'extend');
  ...
  my @featureorder;
  my $feature_raw = BluefinDecryptxP2peHelpers::gp($opts, 'feature');
  if (Voxgig::Struct::islist($feature_raw)) {
    my %fmap;
    for my $entry (@$feature_raw) {
      next unless Voxgig::Struct::ismap($entry);
      my $name = BluefinDecryptxP2peHelpers::gp($entry, 'name');
  ...
  BluefinDecryptxP2peHelpers::coerce_bools($opts);
  ...
  my $sys_fetch = BluefinDecryptxP2peHelpers::gpath($opts, 'system.fetch');

  # CLONE the config side: `config` is a process-wide singleton
  # (BluefinDecryptxP2peConfig::shared_config) and merge uses its nested hashes as merge
  ...
  my $clean_keys = BluefinDecryptxP2peHelpers::gpath($opts, 'clean.keys');
  ...
  if (!@featureorder) {
    my $fmap = BluefinDecryptxP2peHelpers::to_map(
      BluefinDecryptxP2peHelpers::gp($opts, 'feature')) || {};
