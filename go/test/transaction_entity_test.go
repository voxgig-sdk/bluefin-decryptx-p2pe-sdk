...
func TestTransactionEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFINDECRYPTXP_PE_TEST_TRANSACTION_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func transactionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFINDECRYPTXP_PE_TEST_TRANSACTION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFINDECRYPTXP_PE_TEST_TRANSACTION_ENTID"])
