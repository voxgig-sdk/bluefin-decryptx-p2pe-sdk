...
func TestDeviceStateEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		deviceStateRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.device_state")))
...
func device_stateBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_STATE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_STATE_ENTID"])
