...
func TestDeviceTypeEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		deviceTypeRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.device_type")))
		...
		deviceTypeRef01DataDt0LoadResult := core.ToMapAny(entityData(deviceTypeRef01DataDt0Loaded))
...
func device_typeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_TYPE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_TYPE_ENTID"])
