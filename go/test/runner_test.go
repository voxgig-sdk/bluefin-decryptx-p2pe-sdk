...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("BLUEFIN_DECRYPTX_P2PE_TEST_LIVE") == "TRUE" ||
		os.Getenv("BLUEFIN_DECRYPTX_P2PE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("BLUEFIN_DECRYPTX_P2PE_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.BluefinDecryptxP2peSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.BluefinDecryptxP2peError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.BluefinDecryptxP2peSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.BluefinDecryptxP2peError{Msg: msg, Code: code}
}
