...
func TestCreateResultEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		createResultRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "create_result"}), "create_result_ref01"))
		...
		createResultRef01Data = core.ToMapAny(entityData(createResultRef01DataResult))
...
func create_resultBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_DECRYPTX_P2PE_TEST_CREATE_RESULT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_DECRYPTX_P2PE_TEST_CREATE_RESULT_ENTID"])
