...
func TestAttestationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		attestationRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "attestation"}), "attestation_ref01"))

		attestationRef01DataResult, err := attestationRef01Ent.Create(attestationRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		attestationRef01Data = core.ToMapAny(entityData(attestationRef01DataResult))
		...
		attestationRef01DataDt0LoadResult := core.ToMapAny(entityData(attestationRef01DataDt0Loaded))
...
func attestationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_DECRYPTX_P2PE_TEST_ATTESTATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_DECRYPTX_P2PE_TEST_ATTESTATION_ENTID"])
