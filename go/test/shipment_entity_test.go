...
func TestShipmentEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		shipmentRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "shipment"}), "shipment_ref01"))

		shipmentRef01DataResult, err := shipmentRef01Ent.Create(shipmentRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		shipmentRef01Data = core.ToMapAny(entityData(shipmentRef01DataResult))
		...
		shipmentRef01DataDt0LoadResult := core.ToMapAny(entityData(shipmentRef01DataDt0Loaded))
...
func shipmentBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_DECRYPTX_P2PE_TEST_SHIPMENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_DECRYPTX_P2PE_TEST_SHIPMENT_ENTID"])
