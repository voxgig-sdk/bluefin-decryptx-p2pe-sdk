...
func TestDeviceEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFINDECRYPTXP_PE_TEST_DEVICE_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		deviceRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath([]any{"new", "device"}, setup.data), "device_ref01"))
		...
		deviceRef01Data = core.ToMapAny(deviceRef01DataResult)
		...
		// LOAD
		deviceRef01MatchDt0 := map[string]any{
			"id": deviceRef01Data["id"],
		}
		deviceRef01DataDt0Loaded, err := deviceRef01Ent.Load(deviceRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		deviceRef01DataDt0LoadResult := core.ToMapAny(deviceRef01DataDt0Loaded)
		if deviceRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if deviceRef01DataDt0LoadResult["id"] != deviceRef01Data["id"] {
...
func deviceBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"device01", "device02", "device03", "serial_number01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("BLUEFINDECRYPTXP_PE_TEST_DEVICE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFINDECRYPTXP_PE_TEST_DEVICE_ENTID"])
