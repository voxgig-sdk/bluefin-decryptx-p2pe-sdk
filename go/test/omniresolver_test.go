...
import (
	...
	omni "github.com/voxgig-sdk/bluefin-decryptx-p2pe-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BluefinDecryptxP2peError); is {
				out := map[string]any{
					"name":    "BluefinDecryptxP2peError",
...
func omniCtx(arg any, client *sdk.BluefinDecryptxP2peSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BluefinDecryptxP2peSDK); is {
			client = live
