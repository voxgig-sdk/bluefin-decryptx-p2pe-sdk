...
func TestLocationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFINDECRYPTXP_PE_TEST_LOCATION_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		locationRef01Data = core.ToMapAny(locationRef01DataResult)
...
func locationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFINDECRYPTXP_PE_TEST_LOCATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFINDECRYPTXP_PE_TEST_LOCATION_ENTID"])
