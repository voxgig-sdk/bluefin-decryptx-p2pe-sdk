...
func TestDeviceCustodyDetailEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		deviceCustodyDetailRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.device_custody_detail")))
		...
		deviceCustodyDetailRef01DataDt0LoadResult := core.ToMapAny(entityData(deviceCustodyDetailRef01DataDt0Loaded))
...
func device_custody_detailBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_CUSTODY_DETAIL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_CUSTODY_DETAIL_ENTID"])
