...
func NewContext(ctxmap map[string]any, basectx *Context) *Context {
	...
	if c := getCtxProp(ctxmap, "client"); c != nil {
		if sdk, ok := c.(*BluefinDecryptxP2peSDK); ok {
			ctx.Client = sdk
		}
	}
...
func (ctx *Context) MakeError(code string, msg string) *BluefinDecryptxP2peError {
	return NewBluefinDecryptxP2peError(code, msg, ctx)
}
