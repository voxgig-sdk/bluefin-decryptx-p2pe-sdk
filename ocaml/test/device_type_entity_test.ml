(* Generated device_type entity test. *)
...
let () =
  test "device_type.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.device_type client Noval in
      check_str "name" ent.e_name "device_type")

let () =
  test "device_type.seeded_ops" (fun () ->
      let record = jo [("id", Str "device_type01")] in
      let seed = jo [("device_type",
                      jo [("device_type01", record)])] in
      ...
      let loaded = ent.e_load (jo [("id", Str "device_type01")]) Noval in
      let loaded_data = loaded.e_data_get () in
      check "load data is a map" (ismap loaded_data);
      check_vstr "load id" (getp loaded_data "id") "device_type01";
      ())

let () =
  test "device_type.stream" (fun () ->
      let mk_seed () =
        jo [("device_type",
      ...
      let ent2 = Sdk_client.device_type client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.device_type sclient Noval in
        ...
        let cent = Sdk_client.device_type cclient Noval in
