(* Generated location entity test. *)
...
let () =
  test "location.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.location client Noval in
      check_str "name" ent.e_name "location")

let () =
  test "location.seeded_ops" (fun () ->
      let record = jo [("id", Str "location01")] in
      let seed = jo [("location",
                      jo [("location01", record)])] in
      ...
      let loaded = ent.e_load (jo [("id", Str "location01")]) Noval in
      let loaded_data = loaded.e_data_get () in
      check "load data is a map" (ismap loaded_data);
      check_vstr "load id" (getp loaded_data "id") "location01";
      ())

let () =
  test "location.stream" (fun () ->
      let mk_seed () =
        jo [("location",
      ...
      let ent2 = Sdk_client.location client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.location sclient Noval in
        ...
        let cent = Sdk_client.location cclient Noval in
