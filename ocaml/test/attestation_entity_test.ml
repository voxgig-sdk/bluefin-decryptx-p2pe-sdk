(* Generated attestation entity test. *)
...
let () =
  test "attestation.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.attestation client Noval in
      check_str "name" ent.e_name "attestation")

let () =
  test "attestation.seeded_ops" (fun () ->
      let record = jo [("id", Str "attestation01")] in
      let seed = jo [("attestation",
                      jo [("attestation01", record)])] in
      ...
      let loaded = ent.e_load (jo [("id", Str "attestation01")]) Noval in
      let loaded_data = loaded.e_data_get () in
      check "load data is a map" (ismap loaded_data);
      check_vstr "load id" (getp loaded_data "id") "attestation01";
      ())

let () =
  test "attestation.stream" (fun () ->
      let mk_seed () =
        jo [("attestation",
      ...
      let ent2 = Sdk_client.attestation client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.attestation sclient Noval in
        ...
        let cent = Sdk_client.attestation cclient Noval in
