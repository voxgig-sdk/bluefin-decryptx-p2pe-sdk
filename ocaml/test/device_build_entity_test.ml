(* Generated device_build entity test. *)
...
let () =
  test "device_build.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.device_build client Noval in
      check_str "name" ent.e_name "device_build")

let () =
  test "device_build.seeded_ops" (fun () ->
      let record = jo [("id", Str "device_build01")] in
      let seed = jo [("device_build",
                      jo [("device_build01", record)])] in
      ...
      let loaded = ent.e_load (jo [("id", Str "device_build01")]) Noval in
      let loaded_data = loaded.e_data_get () in
      check "load data is a map" (ismap loaded_data);
      check_vstr "load id" (getp loaded_data "id") "device_build01";
      ())

let () =
  test "device_build.stream" (fun () ->
      let mk_seed () =
        jo [("device_build",
      ...
      let ent2 = Sdk_client.device_build client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.device_build sclient Noval in
        ...
        let cent = Sdk_client.device_build cclient Noval in
