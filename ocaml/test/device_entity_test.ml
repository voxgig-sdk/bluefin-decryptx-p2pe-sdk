...
let () =
  test "device.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.device client Noval in
      check_str "name" ent.e_name "device")

let () =
  test "device.seeded_ops" (fun () ->
      let record = jo [("id", Str "device01")] in
      let seed = jo [("device",
                      jo [("device01", record)])] in
      ...
      let loaded = ent.e_load (jo [("id", Str "device01")]) Noval in
      let loaded_data = loaded.e_data_get () in
      check "load data is a map" (ismap loaded_data);
      check_vstr "load id" (getp loaded_data "id") "device01";
      ())

let () =
  test "device.stream" (fun () ->
      let mk_seed () =
        jo [("device",
      ...
      let ent2 = Sdk_client.device client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.device sclient Noval in
        ...
        let cent = Sdk_client.device cclient Noval in
