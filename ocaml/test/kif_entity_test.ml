...
let () =
  test "kif.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.kif client Noval in
      check_str "name" ent.e_name "kif")

let () =
  test "kif.seeded_ops" (fun () ->
      let record = jo [("id", Str "kif01")] in
      let seed = jo [("kif",
                      jo [("kif01", record)])] in
...
let () =
  test "kif.stream" (fun () ->
      ...
      let ent2 = Sdk_client.kif client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.kif sclient Noval in
        ...
        let cent = Sdk_client.kif cclient Noval in
