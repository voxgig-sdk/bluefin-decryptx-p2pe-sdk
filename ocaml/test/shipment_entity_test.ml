(* Generated shipment entity test. *)
...
let () =
  test "shipment.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.shipment client Noval in
      check_str "name" ent.e_name "shipment")

let () =
  test "shipment.seeded_ops" (fun () ->
      let record = jo [("id", Str "shipment01")] in
      let seed = jo [("shipment",
                      jo [("shipment01", record)])] in
      ...
      let loaded = ent.e_load (jo [("id", Str "shipment01")]) Noval in
      let loaded_data = loaded.e_data_get () in
      check "load data is a map" (ismap loaded_data);
      check_vstr "load id" (getp loaded_data "id") "shipment01";
      ())

let () =
  test "shipment.stream" (fun () ->
      let mk_seed () =
        jo [("shipment",
      ...
      let ent2 = Sdk_client.shipment client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.shipment sclient Noval in
        ...
        let cent = Sdk_client.shipment cclient Noval in
