(* Generated device_state entity test. *)
...
let () =
  test "device_state.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.device_state client Noval in
      check_str "name" ent.e_name "device_state")

let () =
  test "device_state.seeded_ops" (fun () ->
      let record = jo [("id", Str "device_state01")] in
      let seed = jo [("device_state",
                      jo [("device_state01", record)])] in
...
let () =
  test "device_state.stream" (fun () ->
      ...
      let ent2 = Sdk_client.device_state client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.device_state sclient Noval in
        ...
        let cent = Sdk_client.device_state cclient Noval in
