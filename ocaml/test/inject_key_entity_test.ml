(* Generated inject_key entity test. *)
...
let () =
  test "inject_key.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.inject_key client Noval in
      check_str "name" ent.e_name "inject_key")

let () =
  test "inject_key.seeded_ops" (fun () ->
      let record = jo [("id", Str "inject_key01")] in
      let seed = jo [("inject_key",
                      jo [("inject_key01", record)])] in
      ...
      let loaded = ent.e_load (jo [("id", Str "inject_key01")]) Noval in
      let loaded_data = loaded.e_data_get () in
      check "load data is a map" (ismap loaded_data);
      check_vstr "load id" (getp loaded_data "id") "inject_key01";
      ())

let () =
  test "inject_key.stream" (fun () ->
      let mk_seed () =
        jo [("inject_key",
      ...
      let ent2 = Sdk_client.inject_key client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.inject_key sclient Noval in
        ...
        let cent = Sdk_client.inject_key cclient Noval in
