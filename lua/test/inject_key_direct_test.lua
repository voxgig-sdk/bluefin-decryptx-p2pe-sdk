...
describe("InjectKeyDirect", function()
  it("should direct-list-inject_key", function()
    local setup = inject_key_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-inject_key", setup.live and "live" or "unit")
  ...
  it("should direct-load-inject_key", function()
    local setup = inject_key_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-inject_key", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function inject_key_direct_setup(mockres)
