...
describe("DeviceCustodyListDirect", function()
  it("should direct-list-device_custody_list", function()
    local setup = device_custody_list_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-device_custody_list", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"device_type01", "serial_number01"}) do
