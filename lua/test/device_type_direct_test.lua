...
describe("DeviceTypeDirect", function()
  it("should direct-list-device_type", function()
    local setup = device_type_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-device_type", setup.live and "live" or "unit")
  ...
  it("should direct-load-device_type", function()
    local setup = device_type_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-device_type", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function device_type_direct_setup(mockres)
