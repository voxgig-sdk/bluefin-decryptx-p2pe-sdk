...
describe("ShipmentDirect", function()
  it("should direct-list-shipment", function()
    local setup = shipment_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-shipment", setup.live and "live" or "unit")
  ...
  it("should direct-load-shipment", function()
    local setup = shipment_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-shipment", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function shipment_direct_setup(mockres)
