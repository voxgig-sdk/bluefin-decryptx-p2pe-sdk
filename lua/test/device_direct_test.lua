...
describe("DeviceDirect", function()
  it("should direct-list-device", function()
    local setup = device_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-device", setup.live and "live" or "unit")
  ...
  it("should direct-load-device", function()
    local setup = device_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-device", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function device_direct_setup(mockres)
