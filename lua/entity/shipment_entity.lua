...
local ShipmentEntity = {}
ShipmentEntity.__index = ShipmentEntity


function ShipmentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ShipmentEntity)
  self._name = "shipment"
  self._client = client
...
function ShipmentEntity:get_name()
  return self._name
end


function ShipmentEntity:make()
  local opts = {}
...
function ShipmentEntity:mark_deleted()
  self._deleted = true
end


function ShipmentEntity:deleted()
  return true == self._deleted
end


function ShipmentEntity:data_set(args)
  if args ~= nil then
...
function ShipmentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ShipmentEntity:match_set(args)
  if args ~= nil then
...
function ShipmentEntity:match_get()
...
function ShipmentEntity:stream(action, args, callopts)
...
---@param reqmatch ShipmentLoadMatch
---@param ctrl? table
---@return Shipment
---@return string? err
function ShipmentEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ShipmentListMatch
---@param ctrl? table
---@return Shipment[]
---@return string? err
function ShipmentEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata ShipmentCreateData
---@param ctrl? table
---@return Shipment
---@return string? err
function ShipmentEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function ShipmentEntity:_run_op(ctx, post_done)
  local utility = self._utility
