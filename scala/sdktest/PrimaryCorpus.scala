...
import voxgig.bluefindecryptxp2pesdk.core.*
...
object PrimaryCorpus {
  ...
  def toValue(o: Object): Value = o match {
    ...
    case x if x eq voxgig.bluefindecryptxp2pesdk.utility.struct.Struct.UNDEF => VNull
    case m: JMap[_, _] =>
  ...
  private def corpusCtx(client: BluefinDecryptxP2peSDK, ctxmap: Value): Context = {
  ...
  private def runset(name: String, client: BluefinDecryptxP2peSDK)(f: (Context, Seq[Value]) => Value): Unit =
  ...
  private def clientFor(name: String, shared: BluefinDecryptxP2peSDK): BluefinDecryptxP2peSDK =
    sectionSetup(name) match {
      case m @ VMap(_) => BluefinDecryptxP2peSDK.testSDK(null, jmap(m))
      case _           => shared
    }
  ...
  def run(testfile: String): Int = {
    ...
    val sdk = BluefinDecryptxP2peSDK.testSDK()
