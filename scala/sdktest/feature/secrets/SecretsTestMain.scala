...
import voxgig.bluefindecryptxp2pesdk.core._
import voxgig.bluefindecryptxp2pesdk.feature._
...
object SecretsTestMain {
  ...
  private val ENVPREFIX = "BLUEFIN_DECRYPTX_P2PE_TEST_SECRETS_"
  ...
  private def secretsFeatureOf(client: BluefinDecryptxP2peSDK): SecretsFeature = {
  ...
  private def withSecrets(build: Boolean => BluefinDecryptxP2peSDK): BluefinDecryptxP2peSDK = {
    val client = build(false)
    if (null == secretsFeatureOf(client)) build(true) else client
  }

  private def secretsClient(w: Wire, extra: (String, Object)*): BluefinDecryptxP2peSDK =
    withSecrets { extend =>
      val opts = sdkopts(w, extra*)
      if (extend) opts.put("extend", jlist(new SecretsFeature()))
      new BluefinDecryptxP2peSDK(opts)
    }

  private def testModeClient(w: Wire, extra: (String, Object)*): BluefinDecryptxP2peSDK =
    withSecrets { extend =>
      val opts = sdkopts(w, extra*)
      if (extend) opts.put("extend", jlist(new SecretsFeature()))
      BluefinDecryptxP2peSDK.testSDK(null, opts)
    }
  ...
  private def entities(client: BluefinDecryptxP2peSDK): List[SdkEntity] = {
  ...
  private def driveUntil(client: BluefinDecryptxP2peSDK, stop: () => Boolean): (Boolean, String) = {
    var lasterr = ""
    var done = false
  ...
  private def driveOneCall(client: BluefinDecryptxP2peSDK, w: Wire): Boolean = {
  ...
  private def testInactive(): Unit = {
    val client = BluefinDecryptxP2peSDK.testSDK(null, jmap("apikey" -> "OPTKEY01"))
    ...
    val bare = BluefinDecryptxP2peSDK.testSDK(null, null)
