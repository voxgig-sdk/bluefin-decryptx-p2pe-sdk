// Generated direct-call tests for the location entity (mirrors the java
...
object LocationDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-list-location") {
      ...
      rep.eq("direct-list-location.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-list-location.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-list-location.islist", result.get("data").isInstanceOf[JList[?]], "expected data to be an array, got " + result.get("data"))
      val listData = result.get("data").asInstanceOf[JList[Object]]
      rep.eqI("direct-list-location.size", 2, listData.size())
      rep.eqI("direct-list-location.calls", 1, setup.calls.size())
    }

    rep.scope("direct-load-location") {
      ...
      rep.eq("direct-load-location.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-load-location.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-load-location.data", result.get("data") != null, "expected data to be non-null")
      val dataMap = Helpers.toMapAny(result.get("data"))
      if (dataMap != null) rep.eq("direct-load-location.dataId", "direct01", dataMap.get("id"))
      rep.eqI("direct-load-location.calls", 1, setup.calls.size())
      val url = setup.calls.get(0).get("url") match { case s: String => s; case _ => "" }
      rep.check("direct-load-location.url1", url.contains("direct01"), "expected url to contain direct01, got " + url)
    }
  }
}
