// Generated direct-call tests for the kif entity (mirrors the java
...
object KifDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-list-kif") {
      ...
      rep.eq("direct-list-kif.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-list-kif.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-list-kif.islist", result.get("data").isInstanceOf[JList[?]], "expected data to be an array, got " + result.get("data"))
      val listData = result.get("data").asInstanceOf[JList[Object]]
      rep.eqI("direct-list-kif.size", 2, listData.size())
      rep.eqI("direct-list-kif.calls", 1, setup.calls.size())
    }
