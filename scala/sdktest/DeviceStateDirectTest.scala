// Generated direct-call tests for the device_state entity (mirrors the java
...
object DeviceStateDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-list-device_state") {
      ...
      rep.eq("direct-list-device_state.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-list-device_state.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-list-device_state.islist", result.get("data").isInstanceOf[JList[?]], "expected data to be an array, got " + result.get("data"))
      val listData = result.get("data").asInstanceOf[JList[Object]]
      rep.eqI("direct-list-device_state.size", 2, listData.size())
      rep.eqI("direct-list-device_state.calls", 1, setup.calls.size())
    }
