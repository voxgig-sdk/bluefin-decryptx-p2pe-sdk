// Generated direct-call tests for the inject_key entity (mirrors the java
...
object InjectKeyDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-list-inject_key") {
      ...
      rep.eq("direct-list-inject_key.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-list-inject_key.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-list-inject_key.islist", result.get("data").isInstanceOf[JList[?]], "expected data to be an array, got " + result.get("data"))
      val listData = result.get("data").asInstanceOf[JList[Object]]
      rep.eqI("direct-list-inject_key.size", 2, listData.size())
      rep.eqI("direct-list-inject_key.calls", 1, setup.calls.size())
    }

    rep.scope("direct-load-inject_key") {
      ...
      rep.eq("direct-load-inject_key.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-load-inject_key.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-load-inject_key.data", result.get("data") != null, "expected data to be non-null")
      val dataMap = Helpers.toMapAny(result.get("data"))
      if (dataMap != null) rep.eq("direct-load-inject_key.dataId", "direct01", dataMap.get("id"))
      rep.eqI("direct-load-inject_key.calls", 1, setup.calls.size())
      val url = setup.calls.get(0).get("url") match { case s: String => s; case _ => "" }
      rep.check("direct-load-inject_key.url1", url.contains("direct01"), "expected url to contain direct01, got " + url)
    }
  }
}
