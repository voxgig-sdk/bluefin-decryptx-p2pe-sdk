// Generated direct-call tests for the device_type entity (mirrors the java
...
object DeviceTypeDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-list-device_type") {
      ...
      rep.eq("direct-list-device_type.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-list-device_type.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-list-device_type.islist", result.get("data").isInstanceOf[JList[?]], "expected data to be an array, got " + result.get("data"))
      val listData = result.get("data").asInstanceOf[JList[Object]]
      rep.eqI("direct-list-device_type.size", 2, listData.size())
      rep.eqI("direct-list-device_type.calls", 1, setup.calls.size())
    }

    rep.scope("direct-load-device_type") {
      ...
      rep.eq("direct-load-device_type.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-load-device_type.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-load-device_type.data", result.get("data") != null, "expected data to be non-null")
      val dataMap = Helpers.toMapAny(result.get("data"))
      if (dataMap != null) rep.eq("direct-load-device_type.dataId", "direct01", dataMap.get("id"))
      rep.eqI("direct-load-device_type.calls", 1, setup.calls.size())
      val url = setup.calls.get(0).get("url") match { case s: String => s; case _ => "" }
      rep.check("direct-load-device_type.url1", url.contains("direct01"), "expected url to contain direct01, got " + url)
    }
  }
}
