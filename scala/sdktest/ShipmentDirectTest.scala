// Generated direct-call tests for the shipment entity (mirrors the java
...
object ShipmentDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-list-shipment") {
      ...
      rep.eq("direct-list-shipment.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-list-shipment.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-list-shipment.islist", result.get("data").isInstanceOf[JList[?]], "expected data to be an array, got " + result.get("data"))
      val listData = result.get("data").asInstanceOf[JList[Object]]
      rep.eqI("direct-list-shipment.size", 2, listData.size())
      rep.eqI("direct-list-shipment.calls", 1, setup.calls.size())
    }

    rep.scope("direct-load-shipment") {
      ...
      rep.eq("direct-load-shipment.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-load-shipment.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-load-shipment.data", result.get("data") != null, "expected data to be non-null")
      val dataMap = Helpers.toMapAny(result.get("data"))
      if (dataMap != null) rep.eq("direct-load-shipment.dataId", "direct01", dataMap.get("id"))
      rep.eqI("direct-load-shipment.calls", 1, setup.calls.size())
      val url = setup.calls.get(0).get("url") match { case s: String => s; case _ => "" }
      rep.check("direct-load-shipment.url1", url.contains("direct01"), "expected url to contain direct01, got " + url)
    }
  }
}
