...
class ShipmentEntityTest {
  ...
  fun basic() {
    ...
    Assumptions.assumeFalse(
      setup.syntheticOnly,
      "live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P2PE_TEST_SHIPMENT_ENTID JSON to run live",
    )
    val client = setup.client

    // CREATE
    val shipmentRef01Ent = client.shipment(null)
    var shipmentRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
        Struct.getpath(setup.data, "new.shipment"), "shipment_ref01")) ?: linkedMapOf())

    val shipmentRef01DataResult = shipmentRef01Ent.create(shipmentRef01Data, null)
    shipmentRef01Data = Helpers.toMapAny(if (shipmentRef01DataResult is SdkEntity) shipmentRef01DataResult.data() else shipmentRef01DataResult) ?: linkedMapOf()
    assertNotNull(shipmentRef01Data, "expected create result to be a map")
    assertNotNull(shipmentRef01Data["id"], "expected created entity to have an id")

    // LIST
    val shipmentRef01Match = linkedMapOf<String, Any?>()

    val shipmentRef01ListResult = shipmentRef01Ent.list(shipmentRef01Match, null)
    assertTrue(shipmentRef01ListResult is List<*>,
        "expected list result to be an array, got " + shipmentRef01ListResult)
    val shipmentRef01List = shipmentRef01ListResult as List<Any?>

    val foundItem = Struct.select(
        RunnerSupport.entityListToData(shipmentRef01List),
        Struct.jm("id", shipmentRef01Data["id"]))
    assertFalse(Struct.isempty(foundItem), "expected to find created entity in list")

    // LOAD
    val shipmentRef01MatchDt0 = linkedMapOf<String, Any?>()
    shipmentRef01MatchDt0["id"] = shipmentRef01Data["id"]
    val shipmentRef01DataDt0Loaded = shipmentRef01Ent.load(shipmentRef01MatchDt0, null)
    val shipmentRef01DataDt0LoadResult = Helpers.toMapAny(if (shipmentRef01DataDt0Loaded is SdkEntity) shipmentRef01DataDt0Loaded.data() else shipmentRef01DataDt0Loaded) ?: linkedMapOf()
    assertNotNull(shipmentRef01DataDt0LoadResult, "expected load result to be a map")
    assertEquals(shipmentRef01Data["id"], shipmentRef01DataDt0LoadResult["id"],
  ...
  fun stream() {
    ...
    val setup = shipmentBasicSetup(streamingActive)
    Assumptions.assumeFalse(
      setup.live,
      "stream test streams the seeded fixture data (unit mode only)",
    )

    val ent = setup.client.shipment(null)
    ...
    // items.
    val setup2 = shipmentBasicSetup(null)
    val ent2 = setup2.client.shipment(null)
  ...
  companion object {
    fun shipmentBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read shipment test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFIN_DECRYPTX_P2PE_TEST_SHIPMENT_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFIN_DECRYPTX_P2PE_TEST_SHIPMENT_ENTID"] = idmap
