...
class DeviceTypeDirectTest {

  @Test
  fun directListDeviceType() {
    val mockres = mutableListOf<Any?>()
    mockres.add(jm("id", "direct01"))
    mockres.add(jm("id", "direct02"))
    val setup = directSetup(mockres)
    val mode = if (setup.live) "live" else "unit"
    val reason = RunnerSupport.skipReason("direct", "direct-list-device_type", mode)
  ...
  @Test
  fun directLoadDeviceType() {
    val setup = directSetup(jm("id", "direct01"))
    val mode = if (setup.live) "live" else "unit"
    val reason = RunnerSupport.skipReason("direct", "direct-load-device_type", mode)
    Assumptions.assumeTrue(
      reason == null,
      if (reason == null || "" == reason) "skipped via sdk-test-control.json" else reason,
    )
  ...
  companion object {
    ...
    fun directSetup(mockres: Any?): DirectSetup {
      ...
      envm["BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_TYPE_ENTID"] = linkedMapOf<String, Any?>()
      ...
      if (live) {
        ...
        var idmap: MutableMap<String, Any?> = linkedMapOf()
        val entidRaw = env["BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_TYPE_ENTID"]
