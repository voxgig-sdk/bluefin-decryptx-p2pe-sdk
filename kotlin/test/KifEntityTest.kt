...
class KifEntityTest {
  ...
  fun basic() {
    ...
    val kifRef01DataRaw = Struct.items(Helpers.toMapAny(
        Struct.getpath(setup.data, "existing.kif")))
    val kifRef01Data: MutableMap<String, Any?> = if (kifRef01DataRaw.isEmpty())
        linkedMapOf() else (Helpers.toMapAny(kifRef01DataRaw[0][1]) ?: linkedMapOf())

    // LIST
    val kifRef01Ent = client.kif(null)
    val kifRef01Match = linkedMapOf<String, Any?>()

    val kifRef01ListResult = kifRef01Ent.list(kifRef01Match, null)
    assertTrue(kifRef01ListResult is List<*>,
  ...
  fun stream() {
    ...
    val setup = kifBasicSetup(streamingActive)
    Assumptions.assumeFalse(
      setup.live,
      "stream test streams the seeded fixture data (unit mode only)",
    )

    val ent = setup.client.kif(null)
    ...
    // items.
    val setup2 = kifBasicSetup(null)
    val ent2 = setup2.client.kif(null)
  ...
  companion object {
    fun kifBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read kif test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFIN_DECRYPTX_P2PE_TEST_KIF_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFIN_DECRYPTX_P2PE_TEST_KIF_ENTID"] = idmap
      ...
      var idmapResolved = Helpers.toMapAny(env["BLUEFIN_DECRYPTX_P2PE_TEST_KIF_ENTID"])
