...
class DeviceListEntityTest {
  ...
  @Test
  fun basic() {
    val setup = deviceListBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf("load")) {
      val reason = RunnerSupport.skipReason("entityOp", "device_list.$op", mode)
    ...
    Assumptions.assumeFalse(
      setup.syntheticOnly,
      "live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_LIST_ENTID JSON to run live",
    )
    val client = setup.client

    // Bootstrap entity data from existing test data (no create step in flow).
    val deviceListRef01DataRaw = Struct.items(Helpers.toMapAny(
        Struct.getpath(setup.data, "existing.device_list")))
    val deviceListRef01Data: MutableMap<String, Any?> = if (deviceListRef01DataRaw.isEmpty())
        linkedMapOf() else (Helpers.toMapAny(deviceListRef01DataRaw[0][1]) ?: linkedMapOf())

    // LOAD
    val deviceListRef01Ent = client.deviceList(null)
    val deviceListRef01MatchDt0 = linkedMapOf<String, Any?>()
  ...
  companion object {
    fun deviceListBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read device_list test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_LIST_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_LIST_ENTID"] = idmap
