...
class PrimaryUtilityTest {
  ...
  companion object {
    ...
    private var CLIENT: BluefinDecryptxP2peSDK? = null
    ...
    fun run(): OmniResolver.Run {
      var r = RUN
      if (null == r) {
        val client = BluefinDecryptxP2peSDK.testSDK()
        CLIENT = client
    ...
    fun client(): BluefinDecryptxP2peSDK {
      run()
      return CLIENT!!
    }
  ...
  fun featureHookBasic() {
    val hookClient = BluefinDecryptxP2peSDK.testSDK()
  ...
  fun featureInitBasic() {
    val initClient = BluefinDecryptxP2peSDK.testSDK()
