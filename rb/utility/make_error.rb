...
module BluefinDecryptxP2peUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = BluefinDecryptxP2peContext.new({}, nil)
    end
    op = ctx.op || BluefinDecryptxP2peOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || BluefinDecryptxP2peResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(BluefinDecryptxP2peError) ? err.msg : err.to_s
    ...
    sdk_err = BluefinDecryptxP2peError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(BluefinDecryptxP2peError)
