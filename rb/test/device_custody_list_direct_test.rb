...
class DeviceCustodyListDirectTest < Minitest::Test
  def test_direct_list_device_custody_list
    setup = device_custody_list_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-device_custody_list", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["device_type01", "serial_number01"].each do |_live_key|
...
def device_custody_list_direct_setup(mockres)
