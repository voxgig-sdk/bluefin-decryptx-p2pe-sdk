...
class LocationEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P2PE_TEST_LOCATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    location_ref01_data_result = location_ref01_ent.create(location_ref01_data, nil)
    location_ref01_data = Helpers.to_map(location_ref01_data_result.respond_to?(:data_get) ? location_ref01_data_result.data_get : location_ref01_data_result)
    assert !location_ref01_data.nil?
    assert !location_ref01_data["id"].nil?
    ...
    location_ref01_ent.remove(location_ref01_match_rm0, nil)

    # LIST
    location_ref01_match_rt0 = {}

    location_ref01_list_rt0_result = location_ref01_ent.list(location_ref01_match_rt0, nil)
    assert location_ref01_list_rt0_result.is_a?(Array)
...
def location_basic_setup(extra)
  ...
  entid_env_raw = ENV["BLUEFIN_DECRYPTX_P2PE_TEST_LOCATION_ENTID"]
