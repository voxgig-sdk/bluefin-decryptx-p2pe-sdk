...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = BluefinDecryptxP2peOmni.make_runner(TEST_JSON_FILE, BluefinDecryptxP2peSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    BluefinDecryptxP2peError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : BluefinDecryptxP2peOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = BluefinDecryptxP2peOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
