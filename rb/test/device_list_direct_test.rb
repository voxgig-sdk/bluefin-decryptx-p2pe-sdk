...
class DeviceListDirectTest < Minitest::Test
  def test_direct_load_device_list
    setup = device_list_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-device_list", setup[:live] ? "live" : "unit")
    if _should_skip
...
def device_list_direct_setup(mockres)
