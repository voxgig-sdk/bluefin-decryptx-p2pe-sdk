...
class DeviceStateEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.DeviceState(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.DeviceState(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = device_state_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "device_state." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    device_state_ref01_data = nil
    if device_state_ref01_data_raw.length > 0
      device_state_ref01_data = Helpers.to_map(device_state_ref01_data_raw[0][1])
    end

    # LIST
    device_state_ref01_ent = client.DeviceState(nil)
    device_state_ref01_match = {}

    device_state_ref01_list_result = device_state_ref01_ent.list(device_state_ref01_match, nil)
    assert device_state_ref01_list_result.is_a?(Array)

  end
end

def device_state_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "device_state", "DeviceStateTestData.json")
