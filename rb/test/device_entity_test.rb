...
class DeviceEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    device_ref01_data = Helpers.to_map(device_ref01_data_result.respond_to?(:data_get) ? device_ref01_data_result.data_get : device_ref01_data_result)
    ...
    device_ref01_data_dt0_load_result = Helpers.to_map(device_ref01_data_dt0_loaded.respond_to?(:data_get) ? device_ref01_data_dt0_loaded.data_get : device_ref01_data_dt0_loaded)
...
def device_basic_setup(extra)
  ...
  entid_env_raw = ENV["BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_ENTID"]
