...
class DeviceStateDirectTest < Minitest::Test
  def test_direct_list_device_state
    setup = device_state_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-device_state", setup[:live] ? "live" : "unit")
    if _should_skip
...
def device_state_direct_setup(mockres)
