...
class TransactionEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P2PE_TEST_TRANSACTION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def transaction_basic_setup(extra)
  ...
  entid_env_raw = ENV["BLUEFIN_DECRYPTX_P2PE_TEST_TRANSACTION_ENTID"]
