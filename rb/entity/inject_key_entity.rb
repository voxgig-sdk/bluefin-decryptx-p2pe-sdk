...
class InjectKeyEntity
  ...
  def make
    opts = @_entopts.dup
    InjectKeyEntity.new(@_client, opts)
  end
  ...
  # @return [InjectKey, Hash] the current InjectKey data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of InjectKey fields)
  def match_get
  ...
  #
  # @param reqmatch [InjectKeyLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.InjectKey.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [InjectKey, Hash] the loaded InjectKey; raises BluefinDecryptxP2peError on failure
  ...
  #
  # @param reqmatch [InjectKeyListMatch, Hash, nil] match filter (any subset of
  #   InjectKey fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<InjectKey>, Array] the matching InjectKey items; raises BluefinDecryptxP2peError on failure
