...
class DeviceStateEntity
  ...
  def make
    opts = @_entopts.dup
    DeviceStateEntity.new(@_client, opts)
  end
  ...
  # @return [DeviceState, Hash] the current DeviceState data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of DeviceState fields)
  def match_get
  ...
  #
  # @param reqmatch [DeviceStateListMatch, Hash, nil] match filter (any subset of
  #   DeviceState fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<DeviceState>, Array] the matching DeviceState items; raises BluefinDecryptxP2peError on failure
