...
class ShipmentEntity
  ...
  def make
    opts = @_entopts.dup
    ShipmentEntity.new(@_client, opts)
  end
  ...
  # @return [Shipment, Hash] the current Shipment data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Shipment fields)
  def match_get
  ...
  #
  # @param reqmatch [ShipmentLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Shipment.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Shipment, Hash] the loaded Shipment; raises BluefinDecryptxP2peError on failure
  ...
  #
  # @param reqmatch [ShipmentListMatch, Hash, nil] match filter (any subset of
  #   Shipment fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Shipment>, Array] the matching Shipment items; raises BluefinDecryptxP2peError on failure
  ...
  #
  # @param reqdata [ShipmentCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Shipment, Hash] the created Shipment; raises BluefinDecryptxP2peError on failure
