...
class DeviceListEntity
  ...
  def make
    opts = @_entopts.dup
    DeviceListEntity.new(@_client, opts)
  end
  ...
  # @return [DeviceList, Hash] the current DeviceList data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of DeviceList fields)
  def match_get
  ...
  #
  # @param reqmatch [DeviceListLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.DeviceList.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [DeviceList, Hash] the loaded DeviceList; raises BluefinDecryptxP2peError on failure
