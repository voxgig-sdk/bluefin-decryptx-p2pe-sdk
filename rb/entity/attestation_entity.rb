...
class AttestationEntity
  ...
  def make
    opts = @_entopts.dup
    AttestationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BluefinDecryptxP2peHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Attestation, Hash] the current Attestation data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BluefinDecryptxP2peHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Attestation fields)
  def match_get
  ...
  #
  # @param reqmatch [AttestationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Attestation.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Attestation, Hash] the loaded Attestation; raises BluefinDecryptxP2peError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = BluefinDecryptxP2peHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [AttestationListMatch, Hash, nil] match filter (any subset of
  #   Attestation fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Attestation>, Array] the matching Attestation items; raises BluefinDecryptxP2peError on failure
  ...
  #
  # @param reqdata [AttestationCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Attestation, Hash] the created Attestation; raises BluefinDecryptxP2peError on failure
