...
final class PrimaryUtilityTest: XCTestCase {
  ...
  private static func primaryRun() throws -> OmniResolver.Run {
    ...
    let run = try OmniResolver.makeRunner(
      SdkRunner.testJsonPath(), BluefinDecryptxP2peSDK.testSDK(nil, nil))("primary")
    RUN = run
    return run
  }

  // XCTestCase has no custom init; set these up per test.
  private var client: BluefinDecryptxP2peSDK!
  private var utility: Utility!

  override func setUp() {
    super.setUp()
    client = BluefinDecryptxP2peSDK.testSDK(nil, nil)
  ...
  private func makeTestCtx(_ client: BluefinDecryptxP2peSDK, _ utility: Utility,
  ...
  private func makeTestFullCtx(_ client: BluefinDecryptxP2peSDK, _ utility: Utility) -> Context {
  ...
  func testFeatureHookBasic() {
    let hookClient = BluefinDecryptxP2peSDK.testSDK(nil, nil)
  ...
  func testFeatureInitBasic() {
    let initClient = BluefinDecryptxP2peSDK.testSDK(nil, nil)
  ...
  func testExtendOptionInstallsAndInitialisesTheFeature() {
    ...
    let client = BluefinDecryptxP2peSDK.testSDK(nil, opts)
  ...
  func testFetcherLive() throws {
    ...
    let liveClient = BluefinDecryptxP2peSDK(opts)
  ...
  func testFetcherBlockedTestMode() {
    ...
    let opts = VMap()
    opts.entries["system"] = .map(vm(("fetch", .nat(fetch))))
    let blockedClient = BluefinDecryptxP2peSDK(opts)
  ...
  func testMakeSpecBasic() throws {
    ...
    let specClient = BluefinDecryptxP2peSDK.testSDK(nil, setupOpts)
  ...
  func testPrepareAuthBasic() throws {
    ...
    let authClient = BluefinDecryptxP2peSDK.testSDK(nil, setupOpts)
