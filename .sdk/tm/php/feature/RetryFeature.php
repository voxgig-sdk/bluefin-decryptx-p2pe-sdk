...
// BluefinDecryptxP2pe SDK retry feature
...
class BluefinDecryptxP2peRetryFeature extends BluefinDecryptxP2peBaseFeature
{
    ...
    public function init(BluefinDecryptxP2peContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BluefinDecryptxP2peContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BluefinDecryptxP2peContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BluefinDecryptxP2peContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
