...
class BluefinDecryptxP2peTestFeature extends BluefinDecryptxP2peBaseFeature
{
    ...
    public function init(BluefinDecryptxP2peContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (BluefinDecryptxP2peContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = BluefinDecryptxP2peParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (BluefinDecryptxP2peContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(BluefinDecryptxP2peContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = BluefinDecryptxP2peParam::call($ctx, $k);
