...
// BluefinDecryptxP2pe SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = BluefinDecryptxP2peOmni::makeStructRunner(
    ...
    {
        return BluefinDecryptxP2peOmni::objmodel(BluefinDecryptxP2peOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return BluefinDecryptxP2peOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return BluefinDecryptxP2peOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(BluefinDecryptxP2peOmni::structfix($now, Struct::undef()))
                        !== json_encode(BluefinDecryptxP2peOmni::structfix($val, Struct::undef()))) {
