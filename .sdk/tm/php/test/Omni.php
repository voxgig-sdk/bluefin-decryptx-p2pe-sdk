...
//    only - a typed BluefinDecryptxP2peContext there would make every ctx
...
final class BluefinDecryptxP2peOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new BluefinDecryptxP2peContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new BluefinDecryptxP2peSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new BluefinDecryptxP2peResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new BluefinDecryptxP2peError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new BluefinDecryptxP2peResponse($clean['response']);
    ...
    public static function err_from_map($m): ?BluefinDecryptxP2peError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new BluefinDecryptxP2peError(is_string($code) ? $code : '', $msg);
    }
