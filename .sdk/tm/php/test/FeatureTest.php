...
// BluefinDecryptxP2pe SDK feature test
//
...
class FtCtrl extends BluefinDecryptxP2peControl
{
...
class FtClient
{
    ...
    private BluefinDecryptxP2peUtility $utility;

    public function __construct(BluefinDecryptxP2peUtility $utility, array $options)
    {
    ...
    public function get_utility(): BluefinDecryptxP2peUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BluefinDecryptxP2peContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BluefinDecryptxP2peContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BluefinDecryptxP2peContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BluefinDecryptxP2peConfig::make_feature($name);
    ...
    private static function buildUrl(BluefinDecryptxP2peSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BluefinDecryptxP2peError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BluefinDecryptxP2peResponse($fetched);
        }
    ...
    private function fail(BluefinDecryptxP2peContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BluefinDecryptxP2peContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
