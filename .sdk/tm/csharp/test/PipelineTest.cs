...
public class PipelineTest
{
    ...
    private static (BluefinDecryptxP2peSDK, Utility) PlClient(Dictionary<string, object?>? sdkopts)
    {
        var client = BluefinDecryptxP2peSDK.TestSDK(null, sdkopts);
        return (client, client.GetUtility());
    }

    private static Context PlCtx(BluefinDecryptxP2peSDK client, Utility utility,
    ...
    private static string ErrCode(Action act)
    {
        ...
        catch (BluefinDecryptxP2peError err)
        {
            return err.Code;
        }
        catch (Exception)
    ...
    private class PlEntity : IEntity
    {
        ...
        // BluefinDecryptxP2peEntityBase: `remove` marks the instance, which keeps the
    ...
    private static Utility UtilWith(BluefinDecryptxP2peSDK client, FetcherFunc fetcher)
    {
    ...
    public void MakeRequestTransportErrorCarriedOnResponse()
    {
        ...
        Assert.True(resp.Err is BluefinDecryptxP2peError pe && pe.Code == "boom",
