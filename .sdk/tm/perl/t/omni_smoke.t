...
sub smoke_pack {
  my $runner = BluefinDecryptxP2peOmni::make_runner($SPEC, BluefinDecryptxP2peSDK->test(undef, undef));
  return $runner->('smoke');
}
...
# A wrong result fails, with an OmniError naming the mismatch.
{
  ...
  my $err = $@;
  ok(BluefinDecryptxP2peOmni::is_omni_error($err), 'the failure is an OmniError');
...
# must fail.
{
  ...
  my $err = $@;
  ok(BluefinDecryptxP2peOmni::is_omni_error($err), 'the missing-error failure is an OmniError');
