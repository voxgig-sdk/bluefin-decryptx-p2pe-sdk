...
sub resp {
  ...
  return BluefinDecryptxP2peResponse->new({
    'status' => $status,
...
sub spec_of {
  my (%map) = @_;
  return BluefinDecryptxP2peSpec->new({
...
sub is_sdk_error {
  my ($err) = @_;
  return Scalar::Util::blessed($err) && $err->isa('BluefinDecryptxP2peError');
}
...

{
  ...
  $ctx->{spec} = spec_of();
  $ctx->{result} = BluefinDecryptxP2peResult->new({ 'ok' => 1, 'resdata' => [] });
  my ($result, $err) = $utility->{make_result}->($ctx);
...
{
  my $ctx = make_ctx();
  my $preset = BluefinDecryptxP2peResult->new({ 'ok' => 1 });
...
{
  my $boom = BluefinDecryptxP2peError->new('boom', 'boom');
...

{
  ...
  $u->{make_fetch_def} = sub {
    return (undef, BluefinDecryptxP2peError->new('fetchdef_boom', 'boom'));
  };
  my $ctx = make_ctx(utility => $u);
  $ctx->{spec} = request_spec();
  my ($response, $err) = $u->{make_request}->($ctx);
...
{
  my $ctx = make_ctx();
  $ctx->{result} = BluefinDecryptxP2peResult->new({ 'ok' => 1, 'resdata' => 42 });
  is($utility->{done}->($ctx), 42, 'done returns resdata on success');
}

{
  my $ctx = make_ctx();
  $ctx->{result} = BluefinDecryptxP2peResult->new({ 'ok' => 0 });
...

{
  ...
  $ctx->{result} = BluefinDecryptxP2peResult->new({ 'ok' => 1, 'resdata' => 7 });
...

{
  ...
  $ctx->{result} = BluefinDecryptxP2peResult->new({ 'ok' => 0, 'resdata' => 'fallback' });
...
{
  my $oclient = BluefinDecryptxP2peSDK->test(undef, undef);
  ...
  $oclient->{features} = [];
  my $a = BluefinDecryptxP2peBaseFeature->new;
  my $b = BluefinDecryptxP2peBaseFeature->new;
...
sub named_feature {
  my ($name) = @_;
  my $f = BluefinDecryptxP2peBaseFeature->new;
  $f->{name} = $name;
  return $f;
}
...

{
  ...
  ok(BluefinDecryptxP2peHelpers::is_true($o2->{feature}{metrics}{active}),
    'feature order: array normalized to a map (metrics opts preserved)');
  ok(BluefinDecryptxP2peHelpers::is_true($o2->{feature}{test}{active}),
...
sub auth_ctx {
  ...
  my $ctx = BluefinDecryptxP2peContext->new({
    'client' => PipelineOptClient->new($options),
    'utility' => $utility,
    'opname' => 'load',
  }, undef);
  $ctx->{spec} = defined $headers
    ? BluefinDecryptxP2peSpec->new({ 'headers' => $headers, 'step' => 's' })
    : undef;
  return $ctx;
}
...
{
  my $ctx = make_ctx();
  $ctx->{response} = BluefinDecryptxP2peResponse->new({ 'status' => 200 });
  $ctx->{result} = BluefinDecryptxP2peResult->new({});
...
{
  my $ctx = make_ctx();
  $ctx->{response} = BluefinDecryptxP2peResponse->new({
