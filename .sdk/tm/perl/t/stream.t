#!perl
# BluefinDecryptxP2pe SDK entity stream() test
#
...
sub has_feature {
  my ($name) = @_;
  my $f = BluefinDecryptxP2peConfig::make_config()->{feature};
...
sub find_list_entity {
  ...
  my $entities = BluefinDecryptxP2peHelpers::to_map(
    BluefinDecryptxP2peHelpers::gp($config, 'entity')) || {};
  for my $name (sort keys %$entities) {
    my $points = BluefinDecryptxP2peHelpers::gpath($config, "entity.$name.op.list.points");
    next unless Voxgig::Struct::islist($points);
    for my $p (@$points) {
      my $params = BluefinDecryptxP2peHelpers::gpath($p, 'args.params');
      my $reqd = Voxgig::Struct::islist($params)
        ? scalar(grep { BluefinDecryptxP2peHelpers::is_true($_->{reqd}) } @$params)
        : 0;
...
SKIP: {
  ...
  {
    my $sdk = BluefinDecryptxP2peSDK->test({ 'entity' => $seed }, undef);
  ...
  if (has_feature('streaming')) {
    {
      my $sdk = BluefinDecryptxP2peSDK->test(
        { 'entity' => $seed },
        { 'feature' => { 'streaming' => {
