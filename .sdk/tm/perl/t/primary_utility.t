#!perl
# BluefinDecryptxP2pe SDK primary utility test
#
...
#   live blessed context with BluefinDecryptxP2peOmni::livectx before calling a
...
my $_runner = BluefinDecryptxP2peOmni::make_runner($TEST_JSON, BluefinDecryptxP2peSDK->test(undef, undef));
...

{
  ...
  our @ISA = ('BluefinDecryptxP2peBaseFeature');

  sub new {
    my ($class, $fn) = @_;
    my $self = BluefinDecryptxP2peBaseFeature::new($class);
    $self->{hook_fn} = $fn;
    return $self;
  }
...
sub _err_from_map {
  ...
  return BluefinDecryptxP2peError->new($code, $msg);
}
...
sub _def_setup {
  ...
  return BluefinDecryptxP2peOmni::tostruct(Voxgig::Omni::Util::clone($setup));
}
...
runsection('done', sub {
  my ($view) = @_;
  return $utility->{done}->(BluefinDecryptxP2peOmni::livectx($view));
});
...
runsection('makeError', sub {
  my ($view, $errmap) = @_;
  my $ctx = BluefinDecryptxP2peOmni::livectx($view);
...

{
  ...
  $ctx->{result} = BluefinDecryptxP2peResult->new({
    'ok' => 0,
    'resdata' => { 'id' => 'safe01' },
  });
...

{
  ...
  my $feature = BluefinDecryptxP2peBaseFeature->new;
...
{
  my $hook_client = BluefinDecryptxP2peSDK->test(undef, undef);
...
{
  my $init_client = BluefinDecryptxP2peSDK->test(undef, undef);
...
{
  my $calls = [];
  my $live_client = BluefinDecryptxP2peSDK->new({
...
{
  my $blocked_client = BluefinDecryptxP2peSDK->new({
    'base' => 'http://localhost:8080',
    'system' => {
...
runsection('makeResponse', sub {
  my ($view) = @_;
  return _unwrap($utility->{make_response}->(BluefinDecryptxP2peOmni::livectx($view)));
});
...
{
  my $ctx = make_test_full_ctx($client, $utility);
  $ctx->{spec} = BluefinDecryptxP2peSpec->new({ 'step' => 'start' });
  $ctx->{result} = undef;
  my (undef, $err) = $utility->{make_result}->($ctx);
...
{
  my $spec_client = BluefinDecryptxP2peSDK->test(undef, _def_setup('makeSpec'));
...
runsection('makePoint', sub {
  my ($view) = @_;
  my ($point, $err) = $utility->{make_point}->(BluefinDecryptxP2peOmni::livectx($view));
  # The corpus asserts refusals by code (`match: {out: {code: ...}}`), so
  # an error is the RESULT here, flattened to the map the corpus reads.
  return BluefinDecryptxP2peOmni::errify($err) if defined $err;
  return $point;
});
...
runsection('makeUrl', sub {
  my ($view) = @_;
  my $ctx = BluefinDecryptxP2peOmni::livectx($view);
  $ctx->{result} = BluefinDecryptxP2peResult->new({}) unless $ctx->{result};
...
runsection('operator', sub {
  my ($vin) = @_;
  $vin = {} unless ref($vin) eq 'HASH';
  my $op = BluefinDecryptxP2peOperation->new($vin);
  return {
    'entity' => $op->{entity},
    'name' => $op->{name},
    'input' => $op->{input},
...
runsection('param', sub {
  my ($view, $paramdef) = @_;
  return $utility->{param}->(BluefinDecryptxP2peOmni::livectx($view), $paramdef);
});
...
{
  my $auth_client = BluefinDecryptxP2peSDK->test(undef, _def_setup('prepareAuth'));
...
runsection('prepareBody', sub {
  my ($view) = @_;
  return $utility->{prepare_body}->(BluefinDecryptxP2peOmni::livectx($view));
});
...
runsection('prepareHeaders', sub {
  my ($view) = @_;
  return $utility->{prepare_headers}->(BluefinDecryptxP2peOmni::livectx($view));
});
...
runsection('prepareMethod', sub {
  my ($view) = @_;
  return $utility->{prepare_method}->(BluefinDecryptxP2peOmni::livectx($view));
});
...
runsection('prepareParams', sub {
  my ($view) = @_;
  return $utility->{prepare_params}->(BluefinDecryptxP2peOmni::livectx($view));
});
...
runsection('preparePath', sub {
  my ($view) = @_;
  return $utility->{prepare_path}->(BluefinDecryptxP2peOmni::livectx($view));
});
...
runsection('prepareQuery', sub {
  my ($view) = @_;
  return $utility->{prepare_query}->(BluefinDecryptxP2peOmni::livectx($view));
});
...
runsection('resultBasic', sub {
  my ($view) = @_;
  my $result = $utility->{result_basic}->(BluefinDecryptxP2peOmni::livectx($view));
...
runsection('resultBody', sub {
  my ($view) = @_;
  return $utility->{result_body}->(BluefinDecryptxP2peOmni::livectx($view));
});
...
runsection('resultHeaders', sub {
  my ($view) = @_;
  return $utility->{result_headers}->(BluefinDecryptxP2peOmni::livectx($view));
});
...
runsection('transformRequest', sub {
  my ($view) = @_;
  return $utility->{transform_request}->(BluefinDecryptxP2peOmni::livectx($view));
});
...
runsection('transformResponse', sub {
  my ($view) = @_;
  return $utility->{transform_response}->(BluefinDecryptxP2peOmni::livectx($view));
});
