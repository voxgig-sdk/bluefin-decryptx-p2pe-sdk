# BluefinDecryptxP2pe SDK utility: prepare_auth
...
$REGISTRY{prepare_auth} = sub {
  ...
  if (!defined BluefinDecryptxP2peHelpers::gp($options, 'auth')) {
  ...
  if (!defined $apikey || Voxgig::Struct::is_none($apikey)
    ...
    my $auth_prefix = BluefinDecryptxP2peHelpers::gpath($options, 'auth.prefix');
