# BluefinDecryptxP2pe SDK utility: graphql
#
...
sub graphql_error_code {
  ...
  my $ext = BluefinDecryptxP2peHelpers::gp($gqlerr, 'extensions');

  my $raw = BluefinDecryptxP2peHelpers::gp($ext, 'code');
  $raw = BluefinDecryptxP2peHelpers::gp($ext, 'type') if !defined $raw || '' eq $raw;
...
$REGISTRY{graphql_body} = sub {
  ...
  my $gql = BluefinDecryptxP2peHelpers::gp($ctx->{point}, 'graphql');
  ...
  my $varlist = BluefinDecryptxP2peHelpers::gp($gql, 'vars');
  $varlist = [] unless ref($varlist) eq 'ARRAY';

  for my $spec (@$varlist) {
    next unless ref($spec) eq 'HASH';

    my $name = BluefinDecryptxP2peHelpers::gp($spec, 'name');
    my $from = BluefinDecryptxP2peHelpers::gp($spec, 'from');
    ...
    my $val = BluefinDecryptxP2peHelpers::gp($reqsrc, $from);
    $val = BluefinDecryptxP2peHelpers::gp($datasrc, $from) unless defined $val;
    $variables{$name} = $val if defined $val;
  }

  return {
    query     => BluefinDecryptxP2peHelpers::gp($gql, 'doc'),
    variables => \%variables,
  };
};
...
$REGISTRY{graphql_errors} = sub {
  ...
  my $kind = BluefinDecryptxP2peHelpers::gp($ctx->{point}, 'kind');
  return 0 unless defined $kind && 'graphql' eq $kind;

  my $errors = BluefinDecryptxP2peHelpers::gp($result->{body}, 'errors');
  return 0 unless ref($errors) eq 'ARRAY' && 0 < scalar(@$errors);

  my $first = $errors->[0];
  my $msg = BluefinDecryptxP2peHelpers::gp($first, 'message');
