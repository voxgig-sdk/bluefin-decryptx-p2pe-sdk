# BluefinDecryptxP2pe SDK utility: fetcher
...
our $DefaultHttpFetch = sub {
  ...
  $hdrs{'User-Agent'} = 'Mozilla/5.0 (compatible; BluefinDecryptxP2peSDK/1.0)' unless $has_ua;
...
$REGISTRY{fetcher} = sub {
  ...
  if (BluefinDecryptxP2peHelpers::is_true(BluefinDecryptxP2peHelpers::gpath($options, 'feature.test.active'))) {
    return (undef, $ctx->make_error('fetch_test_block',
      "Request blocked as test feature is active (URL was: \"$fullurl\")"));
  }

  my $sys_fetch = BluefinDecryptxP2peHelpers::gpath($options, 'system.fetch');
