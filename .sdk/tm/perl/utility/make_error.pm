...
$REGISTRY{make_error} = sub {
  my ($ctx, $err) = @_;
  if (!defined $ctx) {
    $ctx = BluefinDecryptxP2peContext->new({}, undef);
  }
  my $op = $ctx->{op} || BluefinDecryptxP2peOperation->new({});
  my $opname = $op->{name};
  $opname = 'unknown operation' if !defined $opname || '' eq $opname || '_' eq $opname;

  my $result = $ctx->{result} || BluefinDecryptxP2peResult->new({});
  $result->{ok} = 0;

  $err = $result->{err} if !defined $err;
  $err = $ctx->make_error('unknown', 'unknown error') if !defined $err;

  my $errmsg = (Scalar::Util::blessed($err) && $err->isa('BluefinDecryptxP2peError'))
    ? $err->{msg} : "$err";
  $errmsg =~ s/\s+\z//;
  my $msg = "BluefinDecryptxP2peSDK: $opname: $errmsg";
  ...
  my $sdk_err = BluefinDecryptxP2peError->new('', $msg, $ctx);
