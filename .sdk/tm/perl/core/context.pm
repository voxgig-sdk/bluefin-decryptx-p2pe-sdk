...
package BluefinDecryptxP2peContext;
...
sub new {
  ...
  my $gcp = \&BluefinDecryptxP2peHelpers::get_ctx_prop;
  ...
  $self->{ctrl} = BluefinDecryptxP2peControl->new();
  ...
  $self->{data} = BluefinDecryptxP2peHelpers::to_map($gcp->($ctxmap, 'data')) || {};
  $self->{reqdata} = BluefinDecryptxP2peHelpers::to_map($gcp->($ctxmap, 'reqdata')) || {};
  $self->{match} = BluefinDecryptxP2peHelpers::to_map($gcp->($ctxmap, 'match')) || {};
  $self->{reqmatch} = BluefinDecryptxP2peHelpers::to_map($gcp->($ctxmap, 'reqmatch')) || {};

  my $pt = $gcp->($ctxmap, 'point');
  $self->{point} = Voxgig::Struct::ismap($pt) ? $pt : ($basectx ? $basectx->{point} : undef);

  my $sp = $gcp->($ctxmap, 'spec');
  $self->{spec} = (Scalar::Util::blessed($sp) && $sp->isa('BluefinDecryptxP2peSpec'))
    ? $sp : ($basectx ? $basectx->{spec} : undef);

  my $r = $gcp->($ctxmap, 'result');
  $self->{result} = (Scalar::Util::blessed($r) && $r->isa('BluefinDecryptxP2peResult'))
    ? $r : ($basectx ? $basectx->{result} : undef);

  my $rp = $gcp->($ctxmap, 'response');
  $self->{response} = (Scalar::Util::blessed($rp) && $rp->isa('BluefinDecryptxP2peResponse'))
...
sub resolve_op {
  ...
  return BluefinDecryptxP2peOperation->new({}) if '' eq $opname;

  my $opcfg = BluefinDecryptxP2peHelpers::gpath($self->{config}, "entity.$entname.op.$opname");

  my $input = ($opname eq 'update' || $opname eq 'create') ? 'data' : 'match';

  my $points = [];
  if (Voxgig::Struct::ismap($opcfg)) {
    my $t = BluefinDecryptxP2peHelpers::gp($opcfg, 'points');
    $points = $t if Voxgig::Struct::islist($t);
  }

  my $op = BluefinDecryptxP2peOperation->new({
    entity => $entname,
    name   => $opname,
    input  => $input,
    points => $points,
...
sub make_error {
  my ($self, $code, $msg) = @_;
  return BluefinDecryptxP2peError->new($code, $msg, $self);
}
