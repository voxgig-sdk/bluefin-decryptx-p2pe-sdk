# BluefinDecryptxP2pe SDK response
...
package BluefinDecryptxP2peResponse;
...
sub new {
  ...
  my $jf = BluefinDecryptxP2peHelpers::gp($resmap, 'json');
  my $json_func = (ref $jf eq 'CODE') ? $jf : undef;

  return bless {
    status      => $status,
    status_text => $status_text,
    headers     => BluefinDecryptxP2peHelpers::gp($resmap, 'headers'),
    json_func   => $json_func,
