...
from projectname_sdk.core.control import BluefinDecryptxP2peControl
from projectname_sdk.core.error import BluefinDecryptxP2peError


def make_error_util(ctx, err):
    if ctx is None:
        from projectname_sdk.core.context import BluefinDecryptxP2peContext
        ctx = BluefinDecryptxP2peContext({}, None)
    ...
    result = ctx.result
    if result is None:
        result = BluefinDecryptxP2peResult({})
    result.ok = False
    ...
    errmsg = ""
    if isinstance(err, BluefinDecryptxP2peError):
        errmsg = err.msg
    ...
    sdk_err = BluefinDecryptxP2peError("", msg, ctx)
