...
module BluefinDecryptxP2peUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BluefinDecryptxP2peResponse.new({})
    ...
    if fetch_err
      ...
      response = BluefinDecryptxP2peResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BluefinDecryptxP2peResponse.new(fetched)
    else
