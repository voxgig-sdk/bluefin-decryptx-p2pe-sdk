...
module BluefinDecryptxP2peOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if BluefinDecryptxP2peOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = BluefinDecryptxP2peOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| BluefinDecryptxP2peOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || BluefinDecryptxP2peOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = BluefinDecryptxP2peSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = BluefinDecryptxP2peError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { BluefinDecryptxP2peOmni.errify(err) }
