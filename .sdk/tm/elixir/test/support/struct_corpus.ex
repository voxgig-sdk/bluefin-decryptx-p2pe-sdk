# Vendored struct corpus runner for the BluefinDecryptxP2pe SDK.
#
...
defmodule BluefinDecryptxP2pe.StructCorpus do
  ...
  defp pclient_for(primary, name) do
    ...
    if S.ismap(setup), do: BluefinDecryptxP2pe.test(nil, S.clone(setup)), else: BluefinDecryptxP2pe.test()
  end
  ...
  def run_primary(testfile \\ nil) do
    ...
    client = BluefinDecryptxP2pe.test()
    Process.put(:pclient, client)
    Process.put(:pbaseclient, client)
    rootctx = BluefinDecryptxP2pe.get_root_ctx(client)
    ...
    run_primary_set("makeContext", psec(primary, "makeContext"), fn e ->
      ...
      BluefinDecryptxP2pe.Context.new(ctxmap, nil)
    end)
    ...
    run_primary_set("operator", psec(primary, "operator"), fn e ->
      inv = eget(e, "in")
      op = BluefinDecryptxP2pe.Operation.new(if(S.ismap(inv), do: inv, else: S.jm([])))
      S.jm([
