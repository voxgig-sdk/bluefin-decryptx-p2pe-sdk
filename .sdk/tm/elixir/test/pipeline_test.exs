...
defmodule BluefinDecryptxP2pe.PipelineFakeEntity do
...
defmodule BluefinDecryptxP2pe.PipelineTest do
  use ExUnit.Case

  alias Voxgig.Struct, as: S
  alias BluefinDecryptxP2pe.Helpers, as: H
  alias BluefinDecryptxP2pe.{Utility, Context, Spec, Result, Response}

  defp client, do: BluefinDecryptxP2pe.test()

  defp make_ctx(client, opname \\ "load") do
    Context.new(S.jm(["opname", opname, "ctrl", S.jm([])]), BluefinDecryptxP2pe.get_root_ctx(client))
  end
  ...
  defp util_override(c, key, fun) do
    S.setprop(BluefinDecryptxP2pe.get_utility(c), key, fun)
    c
  end
  ...
  defp resolve_order(feature) do
    ctx = Context.new(S.jm(["opname", "load", "ctrl", S.jm([])]), BluefinDecryptxP2pe.get_root_ctx(client()))
  ...
  defp list_ctx(c, collector) do
    entity =
      S.jm(["_module", BluefinDecryptxP2pe.PipelineFakeEntity, "_name", "widget", "_collector", collector])

    Context.new(S.jm(["opname", "list", "entity", entity, "ctrl", S.jm([])]), BluefinDecryptxP2pe.get_root_ctx(c))
  end
  ...
  test "make_request a transport error tuple lands on the response" do
    boom = BluefinDecryptxP2pe.Error.new("boom", "boom")
  ...
  test "make_request a fetchdef error surfaces as a response error" do
    c = util_override(client(), "make_fetch_def", fn _ctx -> {nil, BluefinDecryptxP2pe.Error.new("fetchdef_boom", "boom")} end)
    ctx = make_ctx(c)
    S.setprop(ctx, "spec", request_spec())
    {response, err} = Utility.make_request(ctx)
    assert err == nil
  ...
  test "done raises the error when not ok" do
    ...
    assert_raise BluefinDecryptxP2pe.Error, fn -> Utility.done(ctx) end
  end
  ...
  test "make_error preserves the error code" do
    ...
    err =
      assert_raise BluefinDecryptxP2pe.Error, fn ->
  ...
  test "feature_add appends in call order" do
    ...
    a = BluefinDecryptxP2pe.Feature.new()
    b = BluefinDecryptxP2pe.Feature.new()
  ...
  defp named_feature(name) do
    f = BluefinDecryptxP2pe.Feature.new()
  ...
  defp auth_ctx(c, options, headers) do
    ...
    ctx =
      Context.new(
        S.jm(["opname", "load", "client", fakeclient, "utility", BluefinDecryptxP2pe.get_utility(c), "ctrl", S.jm([])]),
        nil
      )
