...
# BluefinDecryptxP2pe.FeatureHarness — the same hook order and short-circuit rules as
...
defmodule BluefinDecryptxP2pe.FeatureTest do
  use ExUnit.Case

  alias Voxgig.Struct, as: S
  alias BluefinDecryptxP2pe.Helpers, as: H
  alias BluefinDecryptxP2pe.FeatureHarness, as: FH
    ...
    if has?("retry") do
      ...
      {server, calls} =
        FH.recording_server(fn n, _fd ->
          if n < 3,
            do: {nil, BluefinDecryptxP2pe.Error.new("boom", "boom")},
  ...
  test "retry exhausted transport error surfaces" do
    if has?("retry") do
      clock = FH.make_clock()
      {server, calls} = FH.recording_server(fn _n, _fd -> {nil, BluefinDecryptxP2pe.Error.new("boom", "boom")} end)
