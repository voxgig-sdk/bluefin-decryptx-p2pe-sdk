...
defmodule BluefinDecryptxP2pe.StreamTest do
  use ExUnit.Case
  ...
  defp has_feature?(name) do
    f = S.getprop(BluefinDecryptxP2pe.Config.make_config(), "feature")
  ...
  defp entity(sdk, name), do: apply(BluefinDecryptxP2pe, String.to_atom(name), [sdk, nil])
  defp entmod(name), do: Module.concat([BluefinDecryptxP2pe.Entity, Macro.camelize(name)])
  ...
  test "entity stream() runs the pipeline and yields items" do
    ...
    if name == nil do
      ...
      sdk = BluefinDecryptxP2pe.test(S.jm(["entity", seed(name)]), nil)
      items = do_stream(sdk, name, nil)
      assert length(items) == 3
      assert S.ismap(hd(items))

      # signal cancels iteration between yields.
      sdk2 = BluefinDecryptxP2pe.test(S.jm(["entity", seed(name)]), nil)
