...
defmodule BluefinDecryptxP2pe.PrimaryUtilityTest do
  use ExUnit.Case

  alias Voxgig.Struct, as: S
  alias BluefinDecryptxP2pe.Omni, as: O
  alias BluefinDecryptxP2pe.Utility, as: U
  ...
  defp livectx(node, opts) do
    ...
    util = BluefinDecryptxP2pe.get_utility(sdk)
    rootctx = BluefinDecryptxP2pe.get_root_ctx(sdk)

    ctxmap = S.clone(node)
    S.delprop(ctxmap, "client")
    S.delprop(ctxmap, "utility")

    ctx = BluefinDecryptxP2pe.Context.new(ctxmap, nil)
    ...
    if S.ismap(specmap), do: S.setprop(ctx, "spec", BluefinDecryptxP2pe.Spec.new(specmap))

    resmap = S.getprop(ctxmap, "result")

    if S.ismap(resmap) do
      result = BluefinDecryptxP2pe.Result.new(resmap)
      ...
      if S.ismap(errmap) do
        ...
        if is_binary(msg) and msg != "",
          do: S.setprop(result, "err", BluefinDecryptxP2pe.Error.new("", msg, nil))
      end
    ...
    if S.ismap(respmap) do
      response = BluefinDecryptxP2pe.Response.new(respmap)
  ...
  test "the primary corpus runs through the vendored omni engine", %{run: run, sdk: sdk} do
    ...
    runsection.("makeContext", fn args ->
      ...
      out = BluefinDecryptxP2pe.Context.new(ctxmap, nil)
    ...
    specsdk = BluefinDecryptxP2pe.test(nil, setup_opts(spec, "makeSpec"))
    ...
    runsection.("operator", fn args ->
      op = BluefinDecryptxP2pe.Operation.new(argnode(args, 0))
    ...
    authsdk = BluefinDecryptxP2pe.test(nil, setup_opts(spec, "prepareAuth"))
