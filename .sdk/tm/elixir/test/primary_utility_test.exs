...
defmodule BluefinDecryptxP2pe.PrimaryUtilityTest do
  ...
  alias BluefinDecryptxP2pe.{Context, Utility, Spec, Result}

  defp client, do: BluefinDecryptxP2pe.test()

  defp ctx(client, opname \\ "load") do
    Context.new(S.jm(["opname", opname]), BluefinDecryptxP2pe.get_root_ctx(client))
  end
  ...
  test "prepare_auth sets an authorization header when an apikey is present" do
    c = BluefinDecryptxP2pe.new(H.deep(%{"apikey" => "secret"}))
    ctx = ctx(c)
  ...
  test "make_options derives a clean key regex" do
    c = BluefinDecryptxP2pe.test()
    opts = BluefinDecryptxP2pe.options_map(c)
