// BluefinDecryptxP2pe SDK — drives the primary utility functions against the shared
...
static res::Run& primaryRun() {
  static res::Run run =
      res::makeRunner(TEST_JSON_FILE, BluefinDecryptxP2peSDK::testSDK())
          .runner("primary", Value::undef());
  return run;
}

static std::shared_ptr<BluefinDecryptxP2peSDK> client() { return BluefinDecryptxP2peSDK::testSDK(); }
...
static CtxPtr makeTestCtx(std::shared_ptr<BluefinDecryptxP2peSDK> client_, UtilityPtr utility) {
  CtxSpec cs;
  cs.setOpname("load");
...
static CtxPtr makeTestFullCtx(std::shared_ptr<BluefinDecryptxP2peSDK> client_, UtilityPtr utility) {
...
static void fetcherLive() {
  ...
  auto liveClient = std::make_shared<BluefinDecryptxP2peSDK>(opts);
...
static void fetcherBlockedTestMode() {
  ...
  Value opts = fhMap({{"system", fhMap({{"fetch", Value(fetchFn)}})}});
  auto blockedClient = std::make_shared<BluefinDecryptxP2peSDK>(opts);
...
static void makeSpecBasic() {
  ...
  auto specClient = BluefinDecryptxP2peSDK::testSDK(Value::undef(), setupOpts);
...
static void prepareAuthBasic() {
  ...
  auto authClient = BluefinDecryptxP2peSDK::testSDK(Value::undef(), setupOpts);
