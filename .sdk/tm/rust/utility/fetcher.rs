...
fn default_http_fetch(fullurl: &str, fetchdef: &Value) -> Result<Value, BluefinDecryptxP2peError> {
    ...
    if !has_ua {
        req = req.set("User-Agent", "Mozilla/5.0 (compatible; BluefinDecryptxP2peSDK/1.0)");
    }
    ...
    let resp = match sent {
        ...
        Err(e) => {
            return Err(BluefinDecryptxP2peError::new("fetch_transport", &format!("{}", e)));
        }
    };
    ...
    let body_txt = resp
        .into_string()
        .map_err(|e| BluefinDecryptxP2peError::new("fetch_body", &format!("{}", e)))?;
