...
// `Util` alone would resolve to the SDK's BluefinDecryptxP2peSdk.Util namespace,
...
public static class OmniResolver
{
        ...
        var provider = new Provider
        {
            ...
            Client = options =>
            {
                var opts = FixNums(options) as Dictionary<string, object?>
                    ?? new Dictionary<string, object?>();
                var sub = SdkProvider(BluefinDecryptxP2peSDK.TestSDK(null, opts));
                MarkDefProvider(sub);
                return sub;
            },
            ...
            Errify = err =>
            {
                if (err is BluefinDecryptxP2peError sdkerr)
                {
                    var outmap = new Dictionary<string, object?>
                    {
    ...
    /// </summary>
    public static Context OmniCtx(object? arg, BluefinDecryptxP2peSDK client, Utility utility)
    {
        ...
        if (ctxmap.TryGetValue("client", out var p) && p is Provider prov &&
            DefProviderClient(prov) is BluefinDecryptxP2peSDK live)
        {
            client = live;
