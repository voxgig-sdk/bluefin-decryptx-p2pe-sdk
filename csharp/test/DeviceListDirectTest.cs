// device_list direct API tests (generated from the API model).
...
public class DeviceListDirectTest
{
    [Fact]
    public void DirectLoad()
    {
        var setup = DeviceListDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-device_list", _mode);
        if (_shouldSkip)
        {
        ...
        if (setup.Live)
        {
            foreach (var _liveKey in new[] { "share_partner_to01" })
            {
        ...
        {
            pathParams["share_partner_to"] = "direct01";
        }
    ...
    private class DeviceListDirectSetupResult
    {
    ...
    private static DeviceListDirectSetupResult DeviceListDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_LIST_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_LIST_ENTID"];
            ...
            return new DeviceListDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
