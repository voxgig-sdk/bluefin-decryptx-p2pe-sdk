// device_build direct API tests (generated from the API model).
...
public class DeviceBuildDirectTest
{
    [Fact]
    public void DirectList()
    {
        var setup = DeviceBuildDirectSetup(new List<object?>
        {
        ...
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-list-device_build", _mode);
        if (_shouldSkip)
        {
    ...
    {
        var setup = DeviceBuildDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-device_build", _mode);
        if (_shouldSkip)
        {
            return; // skipped via sdk-test-control.json
        }
    ...
    private class DeviceBuildDirectSetupResult
    {
    ...
    private static DeviceBuildDirectSetupResult DeviceBuildDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFINDECRYPTXP_PE_TEST_DEVICE_BUILD_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["BLUEFINDECRYPTXP_PE_TEST_DEVICE_BUILD_ENTID"];
            ...
            return new DeviceBuildDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
