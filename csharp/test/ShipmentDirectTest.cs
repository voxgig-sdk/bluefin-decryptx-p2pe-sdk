// shipment direct API tests (generated from the API model).
...
public class ShipmentDirectTest
{
    [Fact]
    public void DirectList()
    {
        var setup = ShipmentDirectSetup(new List<object?>
        {
        ...
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-list-shipment", _mode);
        if (_shouldSkip)
        {
    ...
    {
        var setup = ShipmentDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-shipment", _mode);
        if (_shouldSkip)
        {
            return; // skipped via sdk-test-control.json
        }
    ...
    private class ShipmentDirectSetupResult
    {
    ...
    private static ShipmentDirectSetupResult ShipmentDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFIN_DECRYPTX_P2PE_TEST_SHIPMENT_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["BLUEFIN_DECRYPTX_P2PE_TEST_SHIPMENT_ENTID"];
            ...
            return new ShipmentDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
