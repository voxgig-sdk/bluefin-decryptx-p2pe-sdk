// inject_key direct API tests (generated from the API model).
...
public class InjectKeyDirectTest
{
    [Fact]
    public void DirectList()
    {
        var setup = InjectKeyDirectSetup(new List<object?>
        {
        ...
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-list-inject_key", _mode);
        if (_shouldSkip)
        {
    ...
    {
        var setup = InjectKeyDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-inject_key", _mode);
        if (_shouldSkip)
        {
            return; // skipped via sdk-test-control.json
        }
    ...
    private class InjectKeyDirectSetupResult
    {
    ...
    private static InjectKeyDirectSetupResult InjectKeyDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFIN_DECRYPTX_P2PE_TEST_INJECT_KEY_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["BLUEFIN_DECRYPTX_P2PE_TEST_INJECT_KEY_ENTID"];
            ...
            return new InjectKeyDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
