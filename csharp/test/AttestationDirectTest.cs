// attestation direct API tests (generated from the API model).
...
public class AttestationDirectTest
{
    [Fact]
    public void DirectList()
    {
        var setup = AttestationDirectSetup(new List<object?>
        {
        ...
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-list-attestation", _mode);
        if (_shouldSkip)
        {
    ...
    {
        var setup = AttestationDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-attestation", _mode);
        if (_shouldSkip)
        {
            return; // skipped via sdk-test-control.json
        }
    ...
    private class AttestationDirectSetupResult
    {
    ...
    private static AttestationDirectSetupResult AttestationDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFIN_DECRYPTX_P2PE_TEST_ATTESTATION_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["BLUEFIN_DECRYPTX_P2PE_TEST_ATTESTATION_ENTID"];
            ...
            return new AttestationDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
