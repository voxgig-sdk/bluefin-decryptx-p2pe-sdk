// location direct API tests (generated from the API model).
...
public class LocationDirectTest
{
    [Fact]
    public void DirectList()
    {
        var setup = LocationDirectSetup(new List<object?>
        {
        ...
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-list-location", _mode);
        if (_shouldSkip)
        {
    ...
    {
        var setup = LocationDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-location", _mode);
        if (_shouldSkip)
        {
            return; // skipped via sdk-test-control.json
        }
    ...
    private class LocationDirectSetupResult
    {
    ...
    private static LocationDirectSetupResult LocationDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFIN_DECRYPTX_P2PE_TEST_LOCATION_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["BLUEFIN_DECRYPTX_P2PE_TEST_LOCATION_ENTID"];
            ...
            return new LocationDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
