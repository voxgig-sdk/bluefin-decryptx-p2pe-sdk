// device_type direct API tests (generated from the API model).
...
public class DeviceTypeDirectTest
{
    [Fact]
    public void DirectList()
    {
        var setup = DeviceTypeDirectSetup(new List<object?>
        {
        ...
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-list-device_type", _mode);
        if (_shouldSkip)
        {
    ...
    {
        var setup = DeviceTypeDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-device_type", _mode);
        if (_shouldSkip)
        {
            return; // skipped via sdk-test-control.json
        }
    ...
    private class DeviceTypeDirectSetupResult
    {
    ...
    private static DeviceTypeDirectSetupResult DeviceTypeDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_TYPE_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_TYPE_ENTID"];
            ...
            return new DeviceTypeDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
