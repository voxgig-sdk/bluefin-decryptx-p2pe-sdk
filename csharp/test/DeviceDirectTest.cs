// device direct API tests (generated from the API model).
...
public class DeviceDirectTest
{
    [Fact]
    public void DirectList()
    {
        var setup = DeviceDirectSetup(new List<object?>
        {
        ...
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-list-device", _mode);
        if (_shouldSkip)
        {
    ...
    {
        var setup = DeviceDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-device", _mode);
        if (_shouldSkip)
        {
            return; // skipped via sdk-test-control.json
        }
    ...
    private class DeviceDirectSetupResult
    {
    ...
    private static DeviceDirectSetupResult DeviceDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_ENTID"];
            ...
            return new DeviceDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
