...
// `utility` (BluefinDecryptxP2peTypes.dart) — exporting both from this library makes
// the name ambiguous and NO dart SDK for such an API compiles:
//
//   Error: 'Utility' is exported from both 'lib/BluefinDecryptxP2peTypes.dart'
...
class BluefinDecryptxP2peSDK {
  ...
  dynamic _opDenied(String op) {
    ...
    return {
      'ok': false,
      'err': Exception('BluefinDecryptxP2peSDK: $op: operation not allowed by'
  ...
  Future<dynamic> graphql(String query,
    ...
    if (errors is List && 0 < errors.length) {
      ...
      res['ok'] = false;
      res['err'] = Exception('BluefinDecryptxP2peSDK: graphql: '
