...
```dart
final results = await client.Attestation().list({ client: "example" });
...
```dart
final results = await client.Location().list({ client: "example" });
...
```dart
final results = await client.Shipment().list({ kif: "example" });
