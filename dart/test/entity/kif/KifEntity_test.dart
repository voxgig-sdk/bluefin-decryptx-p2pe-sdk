  ...
  describe('KifEntity', () {
    ...
    test('basic', (t) async {
      ...
      for (final op in ['list']) {
        if (maybeSkipControl(t, 'entityOp', 'kif.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P_PE_TEST_KIF_ENTID JSON to run live');
        return;
      }
      ...
      final kif_ref01_data =
          (setup['data']['existing']['kif'] as Map).values.first;

      // LIST
      final kif_ref01_ent = client.Kif();
      final kif_ref01_match = <String, dynamic>{};

      final kif_ref01_list = await kif_ref01_ent.list(kif_ref01_match);
