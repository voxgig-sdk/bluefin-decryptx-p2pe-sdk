...
void tests() {
  describe('DeviceRkiActivateResultEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinDecryptxP2peSDK.test();
      final ent = testsdk.DeviceRkiActivateResult();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.DeviceRkiActivateResult();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'device_rki_activate_result.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P_PE_TEST_DEVICE_RKI_ACTIVATE_RESULT_ENTID JSON to run live');
        return;
      }
      ...
      // CREATE
      final device_rki_activate_result_ref01_ent = client.DeviceRkiActivateResult();
      dynamic device_rki_activate_result_ref01_data = setup['data']['new']['device_rki_activate_result']['device_rki_activate_result_ref01'];

      device_rki_activate_result_ref01_data = await device_rki_activate_result_ref01_ent.create(device_rki_activate_result_ref01_data);
      ok(null != device_rki_activate_result_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['device_rki_activate_result01', 'device_rki_activate_result02', 'device_rki_activate_result03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_DECRYPTX_P_PE_TEST_DEVICE_RKI_ACTIVATE_RESULT_ENTID'];
  final idmapOverridden =
      null != idmapEnvVal && idmapEnvVal.trim().startsWith('{');

  final env = envOverride({
    'BLUEFIN_DECRYPTX_P_PE_TEST_DEVICE_RKI_ACTIVATE_RESULT_ENTID': idmap,
    'BLUEFIN_DECRYPTX_P_PE_TEST_LIVE': 'FALSE',
    'BLUEFIN_DECRYPTX_P_PE_TEST_EXPLAIN': 'FALSE',
    'BLUEFIN_DECRYPTX_P_PE_APIKEY': 'NONE',
  });

  idmap = env['BLUEFIN_DECRYPTX_P_PE_TEST_DEVICE_RKI_ACTIVATE_RESULT_ENTID'];
