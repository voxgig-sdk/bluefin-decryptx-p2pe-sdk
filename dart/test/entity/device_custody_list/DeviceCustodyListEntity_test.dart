...
void tests() {
  describe('DeviceCustodyListEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinDecryptxP2peSDK.test();
      final ent = testsdk.DeviceCustodyList();
      ok(null != ent);
    });
...
test('stream', (t) async {
      ...
      final plainent = plainsdk.DeviceCustodyList();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['list']) {
        if (maybeSkipControl(t, 'entityOp', 'device_custody_list.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P_PE_TEST_DEVICE_CUSTODY_LIST_ENTID JSON to run live');
        return;
      }
      ...
      final device_custody_list_ref01_data =
          (setup['data']['existing']['device_custody_list'] as Map).values.first;

      // LIST
      final device_custody_list_ref01_ent = client.DeviceCustodyList();
      final device_custody_list_ref01_match = <String, dynamic>{};
      device_custody_list_ref01_match['device_type'] = setup['idmap']['device_type01'];
      device_custody_list_ref01_match['serial_number'] = setup['idmap']['serial_number01'];

      final device_custody_list_ref01_list = await device_custody_list_ref01_ent.list(device_custody_list_ref01_match);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['device_custody_list01', 'device_custody_list02', 'device_custody_list03', 'device01', 'device02', 'device03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_DECRYPTX_P_PE_TEST_DEVICE_CUSTODY_LIST_ENTID'];
  final idmapOverridden =
      null != idmapEnvVal && idmapEnvVal.trim().startsWith('{');

  final env = envOverride({
    'BLUEFIN_DECRYPTX_P_PE_TEST_DEVICE_CUSTODY_LIST_ENTID': idmap,
    'BLUEFIN_DECRYPTX_P_PE_TEST_LIVE': 'FALSE',
    'BLUEFIN_DECRYPTX_P_PE_TEST_EXPLAIN': 'FALSE',
    'BLUEFIN_DECRYPTX_P_PE_APIKEY': 'NONE',
  });

  idmap = env['BLUEFIN_DECRYPTX_P_PE_TEST_DEVICE_CUSTODY_LIST_ENTID'];
