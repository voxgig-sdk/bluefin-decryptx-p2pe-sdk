...
void tests() {
  describe('CreateResultEntity', () {
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.CreateResult();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'create_result.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P_PE_TEST_CREATE_RESULT_ENTID JSON to run live');
        return;
      }
      ...
      // CREATE
      final create_result_ref01_ent = client.CreateResult();
      dynamic create_result_ref01_data = setup['data']['new']['create_result']['create_result_ref01'];
      ...
      create_result_ref01_data = await create_result_ref01_ent.create(create_result_ref01_data);
      ok(null != create_result_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['create_result01', 'create_result02', 'create_result03', 'device01', 'device02', 'device03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
