...
void tests() {
  describe('DeviceStateEntity', () {
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.DeviceState();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['list']) {
        if (maybeSkipControl(t, 'entityOp', 'device_state.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P_PE_TEST_DEVICE_STATE_ENTID JSON to run live');
        return;
      }
      ...
      final device_state_ref01_data =
          (setup['data']['existing']['device_state'] as Map).values.first;

      // LIST
      final device_state_ref01_ent = client.DeviceState();
      final device_state_ref01_match = <String, dynamic>{};

      final device_state_ref01_list = await device_state_ref01_ent.list(device_state_ref01_match);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['device_state01', 'device_state02', 'device_state03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
