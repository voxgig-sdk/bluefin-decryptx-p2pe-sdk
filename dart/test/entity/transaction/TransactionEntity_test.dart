  ...
  describe('TransactionEntity', () {
    ...
    test('basic', (t) async {
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P_PE_TEST_TRANSACTION_ENTID JSON to run live');
        return;
      }
      ...
      dynamic transaction_ref01_data = setup['data']['new']['transaction']['transaction_ref01'];

      transaction_ref01_data = await transaction_ref01_ent.create(transaction_ref01_data);
      ok(null != transaction_ref01_data['id']);
      ...
      ok(!isempty(select(
          (transaction_ref01_list as List).map((e) => e.data()).toList(),
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_DECRYPTX_P_PE_TEST_TRANSACTION_ENTID'];
