...
void tests() {
  describe('DeviceDirect', () {
    ...
    test('direct-load-device', (t) async {
      final setup = directSetup({'id': 'direct01'});
      if (maybeSkipControl(
          t, 'direct', 'direct-load-device', true == setup['live'])) {
        return;
      }
      if (skipIfMissingIds(t, setup, ["device_type01","serial_number01"])) {
        return;
      }
      ...
      if (true == setup['live']) {
        ...
        final candidateId =
            (listArr[0] is Map ? listArr[0]['device_type'] : null) ??
    ...
    test('direct-list-device', (t) async {
      final setup = directSetup([
        {'id': 'direct01'},
        {'id': 'direct02'}
      ]);
      if (maybeSkipControl(
          t, 'direct', 'direct-list-device', true == setup['live'])) {
        return;
      }
      final client = setup['client'];
      final calls = setup['calls'];
...
Map<String, dynamic> directSetup([dynamic mockres]) {
  ...
  final env = envOverride({
    'BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_ENTID': <String, dynamic>{},
  ...
  if (live) {
    ...
    dynamic idmap = env['BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_ENTID'];
