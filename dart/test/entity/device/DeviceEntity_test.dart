...
void tests() {
  describe('DeviceEntity', () {
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.Device();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create', 'list', 'load']) {
        if (maybeSkipControl(t, 'entityOp', 'device.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P_PE_TEST_DEVICE_ENTID JSON to run live');
        return;
      }
      ...
      // CREATE
      final device_ref01_ent = client.Device();
      dynamic device_ref01_data = setup['data']['new']['device']['device_ref01'];
      device_ref01_data['serial_number'] = setup['idmap']['serial_number01'];

      device_ref01_data = await device_ref01_ent.create(device_ref01_data);
      ok(null != device_ref01_data['id']);


      // LIST
      final device_ref01_match = <String, dynamic>{};

      final device_ref01_list = await device_ref01_ent.list(device_ref01_match);
      ...
      // LOAD
      final device_ref01_match_dt0 = <String, dynamic>{};
      device_ref01_match_dt0['id'] = device_ref01_data['id'];
      final device_ref01_data_dt0 = await device_ref01_ent.load(device_ref01_match_dt0);
      ok(device_ref01_data_dt0['id'] == device_ref01_data['id']);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['device01', 'device02', 'device03', 'device01', 'device02', 'device03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
