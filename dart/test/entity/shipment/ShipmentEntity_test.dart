...
test('stream', (t) async {
      ...
      final plainent = plainsdk.Shipment();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create', 'list', 'load']) {
        if (maybeSkipControl(t, 'entityOp', 'shipment.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P_PE_TEST_SHIPMENT_ENTID JSON to run live');
        return;
      }
      ...
      // CREATE
      final shipment_ref01_ent = client.Shipment();
      dynamic shipment_ref01_data = setup['data']['new']['shipment']['shipment_ref01'];

      shipment_ref01_data = await shipment_ref01_ent.create(shipment_ref01_data);
      ok(null != shipment_ref01_data['id']);


      // LIST
      final shipment_ref01_match = <String, dynamic>{};

      final shipment_ref01_list = await shipment_ref01_ent.list(shipment_ref01_match);
      ...
      // LOAD
      final shipment_ref01_match_dt0 = <String, dynamic>{};
      shipment_ref01_match_dt0['id'] = shipment_ref01_data['id'];
      final shipment_ref01_data_dt0 = await shipment_ref01_ent.load(shipment_ref01_match_dt0);
      ok(shipment_ref01_data_dt0['id'] == shipment_ref01_data['id']);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['shipment01', 'shipment02', 'shipment03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_DECRYPTX_P_PE_TEST_SHIPMENT_ENTID'];
