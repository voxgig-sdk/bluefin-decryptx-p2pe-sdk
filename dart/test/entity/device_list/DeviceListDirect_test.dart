...
void tests() {
  describe('DeviceListDirect', () {
    ...
    test('direct-load-device_list', (t) async {
      final setup = directSetup({'id': 'direct01'});
      if (maybeSkipControl(
          t, 'direct', 'direct-load-device_list', true == setup['live'])) {
        return;
      }
      if (skipIfMissingIds(t, setup, ["share_partner_to01"])) {
        return;
      }
...
Map<String, dynamic> directSetup([dynamic mockres]) {
  ...
  final env = envOverride({
    'BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_LIST_ENTID': <String, dynamic>{},
  ...
  if (live) {
    ...
    dynamic idmap = env['BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_LIST_ENTID'];
