...
void tests() {
  describe('AttestationDirect', () {
    ...
    test('direct-load-attestation', (t) async {
      final setup = directSetup({'id': 'direct01'});
      if (maybeSkipControl(
          t, 'direct', 'direct-load-attestation', true == setup['live'])) {
        return;
      }
    ...
    test('direct-list-attestation', (t) async {
      final setup = directSetup([
        {'id': 'direct01'},
        {'id': 'direct02'}
      ]);
      if (maybeSkipControl(
          t, 'direct', 'direct-list-attestation', true == setup['live'])) {
        return;
      }
      final client = setup['client'];
      final calls = setup['calls'];
...
Map<String, dynamic> directSetup([dynamic mockres]) {
  ...
  final env = envOverride({
    'BLUEFIN_DECRYPTX_P2PE_TEST_ATTESTATION_ENTID': <String, dynamic>{},
  ...
  if (live) {
    ...
    dynamic idmap = env['BLUEFIN_DECRYPTX_P2PE_TEST_ATTESTATION_ENTID'];
