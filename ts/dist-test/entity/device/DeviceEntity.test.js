...
(0, node_test_1.describe)('DeviceEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['create', 'list', 'load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'device.' + op, live))
                return;
        }
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_ENTID JSON to run live');
            return;
        }
        ...
        (0, node_assert_1.default)(null != device_ref01_data.id);
        // LIST
        const device_ref01_match = {};
        const device_ref01_list = (await device_ref01_ent.list(device_ref01_match)).map((e) => e.data());
        (0, node_assert_1.default)(!isempty(select(device_ref01_list, { id: device_ref01_data.id })));
        // LOAD
        ...
        (0, node_assert_1.default)(device_ref01_data_dt0.id === device_ref01_data.id);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/device/DeviceTestData.json');
    ...
    let idmap = transform(['device01', 'device02', 'device03', 'device01', 'device02', 'device03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_ENTID'];
...
//# sourceMappingURL=DeviceEntity.test.js.map