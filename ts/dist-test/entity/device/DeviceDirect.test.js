...
(0, node_test_1.describe)('DeviceDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.BluefinDecryptxP2peSDK({
    ...
    (0, node_test_1.test)('direct-load-device', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-device', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["device_type01", "serial_number01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.device_type ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.device_type = candidateId;
    ...
    (0, node_test_1.test)('direct-list-device', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-device', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.BluefinDecryptxP2peSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
...
//# sourceMappingURL=DeviceDirect.test.js.map