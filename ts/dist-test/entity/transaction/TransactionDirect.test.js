...
(0, node_test_1.describe)('TransactionDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-transaction', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-transaction', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-transaction', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-transaction', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['BLUEFIN_DECRYPTX_P2PE_TEST_TRANSACTION_ENTID'];
...
//# sourceMappingURL=TransactionDirect.test.js.map