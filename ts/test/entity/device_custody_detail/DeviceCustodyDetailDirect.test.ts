...
describe('DeviceCustodyDetailDirect', async () => {
  ...
  test('direct-load-device_custody_detail', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-device_custody_detail', setup.live)) return
    if (skipIfMissingIds(t, setup, ["device_type01","id01","serial_number01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_CUSTODY_DETAIL_ENTID']
