...
describe('KifEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'kif.' + op, live)) return
    }
    ...
    const kif_ref01_list = await kif_ref01_ent.list(kif_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['BLUEFIN_DECRYPTX_P_PE_TEST_KIF_ENTID']
