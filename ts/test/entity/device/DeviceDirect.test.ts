...
describe('DeviceDirect', async () => {
  ...
  test('direct-load-device', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-device', setup.live)) return
  ...
  test('direct-list-device', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-device', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['BLUEFINDECRYPTXP_PE_TEST_DEVICE_ENTID']
