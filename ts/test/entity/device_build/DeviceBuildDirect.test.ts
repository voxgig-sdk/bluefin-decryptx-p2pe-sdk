...
describe('DeviceBuildDirect', async () => {
  ...
  test('direct-load-device_build', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-device_build', setup.live)) return
  ...
  test('direct-list-device_build', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-device_build', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_BUILD_ENTID']
