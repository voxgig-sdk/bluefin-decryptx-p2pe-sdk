...
describe('InjectKeyDirect', async () => {
  ...
  test('direct-load-inject_key', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-inject_key', setup.live)) return
  ...
  test('direct-list-inject_key', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-inject_key', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['BLUEFIN_DECRYPTX_P2PE_TEST_INJECT_KEY_ENTID']
