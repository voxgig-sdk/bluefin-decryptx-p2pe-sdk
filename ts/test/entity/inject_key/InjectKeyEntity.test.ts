...
describe('InjectKeyEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'inject_key.' + op, live)) return
    }
    ...
    const inject_key_ref01_list = await inject_key_ref01_ent.list(inject_key_ref01_match)


    // LOAD
    const inject_key_ref01_match_dt0: any = {}
    inject_key_ref01_match_dt0.id = inject_key_ref01_data.id
    const inject_key_ref01_data_dt0 = await inject_key_ref01_ent.load(inject_key_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['BLUEFIN_DECRYPTX_P_PE_TEST_INJECT_KEY_ENTID']
