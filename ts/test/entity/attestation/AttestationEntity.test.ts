...
describe('AttestationEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'attestation.' + op, live)) return
    }
    ...
    // LIST
    const attestation_ref01_match: any = {}

    const attestation_ref01_list = await attestation_ref01_ent.list(attestation_ref01_match)

    assert(!isempty(select(attestation_ref01_list, { id: attestation_ref01_data.id })))


    // LOAD
    const attestation_ref01_match_dt0: any = {}
    attestation_ref01_match_dt0.id = attestation_ref01_data.id
    const attestation_ref01_data_dt0 = await attestation_ref01_ent.load(attestation_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['BLUEFIN_DECRYPTX_P_PE_TEST_ATTESTATION_ENTID']
