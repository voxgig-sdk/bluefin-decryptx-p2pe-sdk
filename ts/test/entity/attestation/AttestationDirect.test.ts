...
describe('AttestationDirect', async () => {
  ...
  test('direct-load-attestation', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-attestation', setup.live)) return
  ...
  test('direct-list-attestation', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-attestation', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['BLUEFIN_DECRYPTX_P2PE_TEST_ATTESTATION_ENTID']
