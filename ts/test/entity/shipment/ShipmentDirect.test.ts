...
describe('ShipmentDirect', async () => {
  ...
  test('direct-load-shipment', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-shipment', setup.live)) return
  ...
  test('direct-list-shipment', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-shipment', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['BLUEFINDECRYPTXP_PE_TEST_SHIPMENT_ENTID']
