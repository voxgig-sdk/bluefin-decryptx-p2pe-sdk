...
describe('DeviceStateEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'device_state.' + op, live)) return
    }
    ...
    const device_state_ref01_list = await device_state_ref01_ent.list(device_state_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['BLUEFIN_DECRYPTX_P_PE_TEST_DEVICE_STATE_ENTID']
