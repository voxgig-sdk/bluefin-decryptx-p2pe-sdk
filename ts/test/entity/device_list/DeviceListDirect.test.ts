...
describe('DeviceListDirect', async () => {
  ...
  test('direct-load-device_list', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-device_list', setup.live)) return
    if (skipIfMissingIds(t, setup, ["share_partner_to01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['BLUEFINDECRYPTXP_PE_TEST_DEVICE_LIST_ENTID']
