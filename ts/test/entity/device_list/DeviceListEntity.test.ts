...
describe('DeviceListEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'device_list.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P_PE_TEST_DEVICE_LIST_ENTID JSON to run live')
      return
    }
    ...
    let device_list_ref01_data = Object.values(setup.data.existing.device_list)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['BLUEFIN_DECRYPTX_P_PE_TEST_DEVICE_LIST_ENTID']
