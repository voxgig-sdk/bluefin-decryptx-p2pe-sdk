# bluefin-decryptx-p2pe-mcp

[MCP](https://modelcontextprotocol.io) server exposing the BluefinDecryptxP2pe SDK as
two agent tools — `bluefin-decryptx-p2pe_list` and `bluefin-decryptx-p2pe_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bluefin-decryptx-p2pe-mcp)
make build
...
claude mcp add --scope user bluefin-decryptx-p2pe \
  -- /absolute/path/to/bluefin-decryptx-p2pe-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./bluefin-decryptx-p2pe-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// bluefin-decryptx-p2pe_list: first page of records
{ "entity": "attestation" }
{ "entity": "attestation", "query": { } }

// bluefin-decryptx-p2pe_load: one record by id
{ "entity": "attestation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bluefin-decryptx-p2pe-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user bluefin-decryptx-p2pe \
     -- "$PWD"/dist/*/bluefin-decryptx-p2pe-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `bluefin-decryptx-p2pe_list` and `bluefin-decryptx-p2pe_load` tools now appear
   in new sessions. Ask the agent to *"list attestation using bluefin-decryptx-p2pe"*
   and it calls `bluefin-decryptx-p2pe_list` with `{"entity":"attestation"}`.
...
### Call the `bluefin-decryptx-p2pe_list` tool
...
### Call the `bluefin-decryptx-p2pe_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `bluefin-decryptx-p2pe_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `bluefin-decryptx-p2pe_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./bluefin-decryptx-p2pe-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"bluefin-decryptx-p2pe_load","arguments":{"entity":"attestation","query":{"id":1}}}}'
```
