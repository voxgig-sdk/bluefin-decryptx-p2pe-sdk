// BluefinDecryptxP2pe SDK test runner - shared infrastructure for the generated test
...
enum SdkRunner {
  ...
  // three dirs from this file (swift/Tests/BluefinDecryptxP2peSDKTests -> project).
  static func testJsonPath(_ file: String = #filePath) -> String {
    let dir = URL(fileURLWithPath: file)
      .deletingLastPathComponent()  // BluefinDecryptxP2peSDKTests
  ...
  static func errCode(_ e: Error?) -> String {
    (e as? BluefinDecryptxP2peError)?.code ?? ""
  }
  ...
  static func makeCtxFromMap(_ ctxmapIn: VMap?, _ client: BluefinDecryptxP2peSDK?, _ utility: Utility?)
    -> Context
  {
    ...
    if let resMap = ctxmap.entries["result"]?.asMap {
      ...
      if let errMap = resMap.entries["err"]?.asMap, let msg = errMap.entries["message"]?.asString {
        ctx.result!.err = BluefinDecryptxP2peError("", msg, nil)
      }
    }
  ...
  static func fixCtx(_ ctx: Context, _ client: BluefinDecryptxP2peSDK) {
  ...
  static func errFromMap(_ m: VMap?) -> Error? {
    ...
    return BluefinDecryptxP2peError(code, msg, nil)
  }
}
