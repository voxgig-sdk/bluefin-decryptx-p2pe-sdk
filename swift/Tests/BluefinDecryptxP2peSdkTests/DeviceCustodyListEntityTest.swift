// device_custody_list entity test (generated from the API model).

import XCTest

@testable import BluefinDecryptxP2peSdk

final class DeviceCustodyListEntityTest: XCTestCase {
  func testInstance() {
    let sdk = BluefinDecryptxP2peSDK.testSDK(nil, nil)
    let ent = sdk.DeviceCustodyList()
    XCTAssertEqual(ent.getName(), "device_custody_list")
  }

  func testStream() async throws {
    // Seed two records (under the test feature's entity key) and activate
    // the streaming feature.
    let fixtures = vm(("entity", .map(vm(("device_custody_list", .map(vm(
    ...
    let ent2 = sdk2.DeviceCustodyList()
    var streamed2: [Value] = []
