// shipment entity test (generated from the API model).

import XCTest

@testable import BluefinDecryptxP2peSdk

final class ShipmentEntityTest: XCTestCase {
  func testInstance() {
    let sdk = BluefinDecryptxP2peSDK.testSDK(nil, nil)
    let ent = sdk.Shipment()
    XCTAssertEqual(ent.getName(), "shipment")
  }

  func testStream() async throws {
    // Seed two records (under the test feature's entity key) and activate
    // the streaming feature.
    let fixtures = vm(("entity", .map(vm(("shipment", .map(vm(
