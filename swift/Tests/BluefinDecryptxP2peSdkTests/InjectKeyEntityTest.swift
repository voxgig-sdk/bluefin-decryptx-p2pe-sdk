// inject_key entity test (generated from the API model).

import XCTest

@testable import BluefinDecryptxP2peSdk

final class InjectKeyEntityTest: XCTestCase {
  func testInstance() {
    let sdk = BluefinDecryptxP2peSDK.testSDK(nil, nil)
    let ent = sdk.InjectKey()
    XCTAssertEqual(ent.getName(), "inject_key")
  }

  func testStream() async throws {
    // Seed two records (under the test feature's entity key) and activate
    // the streaming feature.
    let fixtures = vm(("entity", .map(vm(("inject_key", .map(vm(
