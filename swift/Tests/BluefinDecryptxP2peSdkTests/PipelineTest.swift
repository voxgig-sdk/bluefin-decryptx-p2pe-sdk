...
final class PipelineTest: XCTestCase {
  ...
  private func plClient(_ sdkopts: VMap?) -> (BluefinDecryptxP2peSDK, Utility) {
    let client = BluefinDecryptxP2peSDK.testSDK(nil, sdkopts)
    return (client, client.getUtility())
  }

  private func plCtx(_ client: BluefinDecryptxP2peSDK, _ utility: Utility, _ ctrl: VMap?) -> Context {
  ...
  private func errCode(_ act: () throws -> Void) -> String {
    do { try act(); return "" } catch let e as BluefinDecryptxP2peError { return e.code } catch { return "" }
  }
  ...
  private func utilWith(_ client: BluefinDecryptxP2peSDK, _ fetcher: @escaping FetcherFunc) -> Utility {
  ...
  func testMakeRequestTransportErrorCarriedOnResponse() {
    ...
    XCTAssertEqual((resp.err as? BluefinDecryptxP2peError)?.code, "boom")
  }
