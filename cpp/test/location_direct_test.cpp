// Generated direct-call tests for the location entity (unit mode;
...
static LocationDirectSetup location_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void location_direct_list() {
  auto setup = location_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-location", "unit");
...
static void location_direct_load() {
  auto setup = location_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-location", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;
...
int main() {
  T_RUN(location_direct_list);
  T_RUN(location_direct_load);
  return sdktest::summary("location_direct_test");
}
