// Generated direct-call tests for the device_custody_list entity (unit mode;
...
static DeviceCustodyListDirectSetup device_custody_list_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
  ...
  DeviceCustodyListDirectSetup s;
  s.client = client;
  s.calls = calls;
  s.live = false;
  return s;
...
static void device_custody_list_direct_list() {
  auto setup = device_custody_list_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-device_custody_list", "unit");
  ...
  Value result = client->direct(vmap({
    {"path", Value("devices/{serial_number}/{device_type}/custody")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(device_custody_list_direct_list);
  return sdktest::summary("device_custody_list_direct_test");
}
