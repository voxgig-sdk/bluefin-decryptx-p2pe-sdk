// Generated direct-call tests for the device_list entity (unit mode;
...
static DeviceListDirectSetup device_list_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void device_list_direct_load() {
  auto setup = device_list_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-device_list", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;

  Value params = vmap();
  setp(params, "share_partner_to", Value("direct01"));

  Value result = client->direct(vmap({
    {"path", Value("virtualDevices/{share_partner_to}")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(device_list_direct_load);
  return sdktest::summary("device_list_direct_test");
}
