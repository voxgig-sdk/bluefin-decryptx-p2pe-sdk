// Generated direct-call tests for the kif entity (unit mode;
...
static KifDirectSetup kif_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void kif_direct_list() {
  auto setup = kif_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-kif", "unit");
...
int main() {
  T_RUN(kif_direct_list);
  return sdktest::summary("kif_direct_test");
}
