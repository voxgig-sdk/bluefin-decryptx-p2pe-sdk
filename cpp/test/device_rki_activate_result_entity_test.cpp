...
struct DeviceRkiActivateResultSetup {
...
static DeviceRkiActivateResultSetup device_rki_activate_result_basic_setup(const Value& extra) {
  load_env_local();

  std::string entity_data_file = "../.sdk/test/entity/device_rki_activate_result/DeviceRkiActivateResultTestData.json";
  ...
  Value idmap = Struct::transform(
      vlist({Value("device_rki_activate_result01"), Value("device_rki_activate_result02"), Value("device_rki_activate_result03")}),
      vmap({{"`$PACK`", vlist({
        Value(""),
        vmap({
          {"`$KEY`", Value("`$COPY`")},
...
static void device_rki_activate_result_entity_instance() {
  auto testsdk = BluefinDecryptxP2peSDK::testSDK();
  auto ent = testsdk->device_rki_activate_result();
...
static void device_rki_activate_result_entity_basic() {
  ...
  for (const std::string& op : std::vector<std::string>{"create"}) {
    auto sk = is_control_skipped("entityOp", std::string("device_rki_activate_result.") + op, mode);
    if (sk.first) { std::cerr << "skip: " << (sk.second.empty()? "sdk-test-control.json" : sk.second) << "\n"; return; }
  }
  auto client = setup.client;
  // CREATE
  auto device_rki_activate_result_ref01_ent = client->device_rki_activate_result();
