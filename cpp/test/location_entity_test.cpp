...
static LocationSetup location_basic_setup(const Value& extra) {
  load_env_local();

  std::string entity_data_file = "../.sdk/test/entity/location/LocationTestData.json";
  ...
  Value idmap = Struct::transform(
      vlist({Value("location01"), Value("location02"), Value("location03")}),
      vmap({{"`$PACK`", vlist({
        Value(""),
        vmap({
          {"`$KEY`", Value("`$COPY`")},
...
static void location_entity_instance() {
...
static void location_entity_stream() {
  ...
  Value seed = vmap({{"entity", vmap({{"location", vmap({
  ...
  auto se = strsdk->location();
  std::vector<Value> items = se->stream("list", Value::undef(), Value::undef());
  ASSERT_EQ((int)items.size(), 2, "stream yields both seeded items");

  auto plainsdk = BluefinDecryptxP2peSDK::testSDK(seed, Value::undef());
  auto pe = plainsdk->location();
  std::vector<Value> pitems = pe->stream("list", Value::undef(), Value::undef());
  ASSERT_EQ((int)pitems.size(), 2, "fallback stream yields both items");
}

static void location_entity_basic() {
  auto setup = location_basic_setup(Value::undef());
  std::string mode = setup.live ? "live" : "unit";
  for (const std::string& op : std::vector<std::string>{"create", "list", "load", "remove"}) {
    auto sk = is_control_skipped("entityOp", std::string("location.") + op, mode);
    if (sk.first) { std::cerr << "skip: " << (sk.second.empty()? "sdk-test-control.json" : sk.second) << "\n"; return; }
  }
  auto client = setup.client;
  // CREATE
  auto location_ref01_ent = client->location();
  ...
  if (!location_ref01_data.is_map()) location_ref01_data = vmap();
  {
    ...
    ASSERT_TRUE(!getp(location_ref01_data, "id").is_undef(), "expected created entity to have an id");
  }
  ...
  // REMOVE
  {
    Value location_ref01_match_rm0 = vmap({{"id", getp(location_ref01_data, "id")}});
    location_ref01_ent->remove(Struct::clone(location_ref01_match_rm0), Value::undef());
  }
...
int main() {
  T_RUN(location_entity_instance);
  T_RUN(location_entity_stream);
  T_RUN(location_entity_basic);
  return sdktest::summary("location_entity_test");
}
