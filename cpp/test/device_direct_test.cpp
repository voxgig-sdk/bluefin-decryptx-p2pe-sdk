// Generated direct-call tests for the device entity (unit mode;
...
static DeviceDirectSetup device_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void device_direct_list() {
  auto setup = device_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-device", "unit");
...
static void device_direct_load() {
  auto setup = device_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-device", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;

  Value params = vmap();
  setp(params, "device_type", Value("direct01"));
  setp(params, "serial_number", Value("direct02"));

  Value result = client->direct(vmap({
    {"path", Value("devices/{serial_number}/{device_type}")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(device_direct_list);
  T_RUN(device_direct_load);
  return sdktest::summary("device_direct_test");
}
