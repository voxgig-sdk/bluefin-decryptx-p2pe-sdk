// BluefinDecryptxP2pe SDK — direct unit tests for the operation-pipeline utilities
...
static std::shared_ptr<BluefinDecryptxP2peSDK> plClient(const Value& sdkopts) {
  return BluefinDecryptxP2peSDK::testSDK(Value::undef(), sdkopts);
}

static CtxPtr plCtx(std::shared_ptr<BluefinDecryptxP2peSDK> client, UtilityPtr utility, const Value& ctrl) {
  CtxSpec cs;
  cs.setOpname("load");
...
static std::string names(std::shared_ptr<BluefinDecryptxP2peSDK> client) {
  std::string out;
