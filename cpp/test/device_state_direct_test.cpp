// Generated direct-call tests for the device_state entity (unit mode;
...
static DeviceStateDirectSetup device_state_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void device_state_direct_list() {
  auto setup = device_state_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-device_state", "unit");
...
int main() {
  T_RUN(device_state_direct_list);
  return sdktest::summary("device_state_direct_test");
}
