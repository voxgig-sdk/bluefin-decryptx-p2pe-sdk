// Generated direct-call tests for the attestation entity (unit mode;
...
static AttestationDirectSetup attestation_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void attestation_direct_list() {
  auto setup = attestation_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-attestation", "unit");
...
static void attestation_direct_load() {
  auto setup = attestation_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-attestation", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;
...
int main() {
  T_RUN(attestation_direct_list);
  T_RUN(attestation_direct_load);
  return sdktest::summary("attestation_direct_test");
}
