// Generated direct-call tests for the inject_key entity (unit mode;
...
static InjectKeyDirectSetup inject_key_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void inject_key_direct_list() {
  auto setup = inject_key_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-inject_key", "unit");
...
static void inject_key_direct_load() {
  auto setup = inject_key_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-inject_key", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;
...
int main() {
  T_RUN(inject_key_direct_list);
  T_RUN(inject_key_direct_load);
  return sdktest::summary("inject_key_direct_test");
}
