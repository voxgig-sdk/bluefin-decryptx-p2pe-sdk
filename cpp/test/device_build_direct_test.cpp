// Generated direct-call tests for the device_build entity (unit mode;
...
static DeviceBuildDirectSetup device_build_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void device_build_direct_list() {
  auto setup = device_build_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-device_build", "unit");
...
static void device_build_direct_load() {
  auto setup = device_build_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-device_build", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;
...
int main() {
  T_RUN(device_build_direct_list);
  T_RUN(device_build_direct_load);
  return sdktest::summary("device_build_direct_test");
}
