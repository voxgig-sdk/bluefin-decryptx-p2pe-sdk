// Generated direct-call tests for the shipment entity (unit mode;
...
static ShipmentDirectSetup shipment_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void shipment_direct_list() {
  auto setup = shipment_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-shipment", "unit");
...
static void shipment_direct_load() {
  auto setup = shipment_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-shipment", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;
...
int main() {
  T_RUN(shipment_direct_list);
  T_RUN(shipment_direct_load);
  return sdktest::summary("shipment_direct_test");
}
