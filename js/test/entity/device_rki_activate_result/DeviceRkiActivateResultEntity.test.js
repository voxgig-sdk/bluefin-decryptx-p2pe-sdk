...
describe('DeviceRkiActivateResultEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        device_rki_activate_result: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.DeviceRkiActivateResult().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.DeviceRkiActivateResult().stream('list')) {
