...
describe('DeviceTypeEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        device_type: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.DeviceType().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.DeviceType().stream('list')) {
  ...
  test('basic', async () => {
    ...
    const device_type_ref01_list = await device_type_ref01_ent.list(device_type_ref01_match)


    // LOAD
    const device_type_ref01_match_dt0 = {}
    device_type_ref01_match_dt0.id = device_type_ref01_data.id
    const device_type_ref01_data_dt0 = await device_type_ref01_ent.load(device_type_ref01_match_dt0)
