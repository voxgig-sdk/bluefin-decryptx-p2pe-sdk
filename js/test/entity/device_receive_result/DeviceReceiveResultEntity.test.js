...
describe('DeviceReceiveResultEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        device_receive_result: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.DeviceReceiveResult().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.DeviceReceiveResult().stream('list')) {
