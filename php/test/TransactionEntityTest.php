...
class TransactionEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P2PE_TEST_TRANSACTION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function transaction_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("BLUEFIN_DECRYPTX_P2PE_TEST_TRANSACTION_ENTID");
