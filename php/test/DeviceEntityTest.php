...
class DeviceEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $device_ref01_data_dt0_loaded = $device_ref01_ent->load($device_ref01_match_dt0, null);
        $device_ref01_data_dt0_load_result = Helpers::to_map(is_object($device_ref01_data_dt0_loaded) && method_exists($device_ref01_data_dt0_loaded, 'data_get') ? $device_ref01_data_dt0_loaded->data_get() : $device_ref01_data_dt0_loaded);
        $this->assertNotNull($device_ref01_data_dt0_load_result);
        $this->assertEquals($device_ref01_data_dt0_load_result["id"], $device_ref01_data["id"]);
...
function device_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["device01", "device02", "device03", "serial_number01"] as $k) {
    ...
    $entid_env_raw = getenv("BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_ENTID");
