...
class DecryptionEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P2PE_TEST_DECRYPTION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $decryption_ref01_data = Helpers::to_map(is_object($decryption_ref01_data_result) && method_exists($decryption_ref01_data_result, 'data_get') ? $decryption_ref01_data_result->data_get() : $decryption_ref01_data_result);
...
function decryption_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("BLUEFIN_DECRYPTX_P2PE_TEST_DECRYPTION_ENTID");
