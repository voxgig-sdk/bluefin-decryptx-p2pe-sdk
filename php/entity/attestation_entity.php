...
// BluefinDecryptxP2pe SDK Attestation entity
...
class AttestationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new AttestationEntity($this->_client, $opts);
    }

    /**
     * @param Attestation|array $args Attestation data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = BluefinDecryptxP2peHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Attestation|array The current Attestation data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Attestation fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = BluefinDecryptxP2peHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Attestation fields).
     */
    ...
    /**
     * Load a single Attestation.
     *
     * @param AttestationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed AttestationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Attestation|array The loaded Attestation as an assoc-array at the
     *   SDK boundary; throws BluefinDecryptxP2peError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = BluefinDecryptxP2peHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Attestation items matching the given filter.
     *
     * @param AttestationListMatch|array|null $reqmatch Match filter (any subset
     *   of Attestation fields) as an assoc-array; AttestationListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Attestation[]|array A list of Attestation items as assoc-arrays at
     *   the SDK boundary; throws BluefinDecryptxP2peError on failure (item-5 convention).
     */
     ...
     *
     * @param AttestationCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed AttestationCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Attestation|array The created Attestation as an assoc-array at the
     *   SDK boundary; throws BluefinDecryptxP2peError on failure (item-5 convention).
     */
