...
class BluefinDecryptxP2peMakePoint
{
    public static function call(BluefinDecryptxP2peContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof BluefinDecryptxP2peError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = BluefinDecryptxP2peHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = BluefinDecryptxP2peHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
