...
pub const PartnerEntity = struct {
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *PartnerEntity {
    ...
    fn run_op_ent(self: *PartnerEntity, ctx: *Context, post_done: *const fn (*PartnerEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *PartnerEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *PartnerEntity) bool {
        return self.deleted;
    }
    ...
    pub fn load(self: *PartnerEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    pub fn list(self: *PartnerEntity, reqmatch: Value, ctrl: Value) EntListResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
        ...
        var items: std.ArrayList(*PartnerEntity) = .empty;
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = PartnerEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice(h.A()) catch &[_]*PartnerEntity{} };
    }
    ...
    pub fn create(self: *PartnerEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    pub fn update(self: *PartnerEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *PartnerEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
