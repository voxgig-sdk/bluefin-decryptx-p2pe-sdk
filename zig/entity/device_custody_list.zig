// DeviceCustodyList entity client (generated — mirrors the go/rust Entity fragment).
...
pub const EntResult = union(enum) {
    ok: *DeviceCustodyListEntity,
    err: *errmod.BluefinDecryptxP2peError,
};
...
pub const DeviceCustodyListEntity = struct {
    name: []const u8 = "device_custody_list",
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *DeviceCustodyListEntity {
        ...
        const e = h.A().create(DeviceCustodyListEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *DeviceCustodyListEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *DeviceCustodyListEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *DeviceCustodyListEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *DeviceCustodyListEntity, ctx: *Context, post_done: *const fn (*DeviceCustodyListEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *DeviceCustodyListEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *DeviceCustodyListEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *DeviceCustodyListEntity, ctx: *Context, post_done: *const fn (*DeviceCustodyListEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *DeviceCustodyListEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *DeviceCustodyListEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return DeviceCustodyListEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *DeviceCustodyListEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *DeviceCustodyListEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *DeviceCustodyListEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *DeviceCustodyListEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn list(self: *DeviceCustodyListEntity, reqmatch: Value, ctrl: Value) EntListResult {
        ...
        var items: std.ArrayList(*DeviceCustodyListEntity) = .empty;
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = DeviceCustodyListEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice(h.A()) catch &[_]*DeviceCustodyListEntity{} };
    }
    
    fn list_post_done(self: *DeviceCustodyListEntity, ctx: *Context) void {
    ...
    pub fn create(self: *DeviceCustodyListEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *DeviceCustodyListEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *DeviceCustodyListEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
