...
pub const UpdateResultEntity = struct {
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *UpdateResultEntity {
    ...
    fn run_op_ent(self: *UpdateResultEntity, ctx: *Context, post_done: *const fn (*UpdateResultEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *UpdateResultEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *UpdateResultEntity) bool {
        return self.deleted;
    }
    ...
    pub fn load(self: *UpdateResultEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn list(self: *UpdateResultEntity, reqmatch: Value, ctrl: Value) EntListResult {
        ...
        var items: std.ArrayList(*UpdateResultEntity) = .empty;
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = UpdateResultEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice(h.A()) catch &[_]*UpdateResultEntity{} };
    }
    ...
    pub fn create(self: *UpdateResultEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    pub fn update(self: *UpdateResultEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "update",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    pub fn remove(self: *UpdateResultEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
