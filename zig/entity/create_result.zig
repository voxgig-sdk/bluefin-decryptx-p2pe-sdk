// CreateResult entity client (generated — mirrors the go/rust Entity fragment).
...
pub const CreateResultEntity = struct {
    name: []const u8 = "create_result",
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *CreateResultEntity {
        ...
        const e = h.A().create(CreateResultEntity) catch unreachable;
        e.* = .{
            .name = "create_result",
            .client = client,
    ...
    fn ent_ctx(self: *CreateResultEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *CreateResultEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *CreateResultEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *CreateResultEntity, ctx: *Context, post_done: *const fn (*CreateResultEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *CreateResultEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *CreateResultEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *CreateResultEntity, ctx: *Context, post_done: *const fn (*CreateResultEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *CreateResultEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *CreateResultEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return CreateResultEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *CreateResultEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *CreateResultEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *CreateResultEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *CreateResultEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *CreateResultEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *CreateResultEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *CreateResultEntity, ctx: *Context) void {
    ...
    pub fn update(self: *CreateResultEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *CreateResultEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
