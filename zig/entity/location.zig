// Location entity client (generated — mirrors the go/rust Entity fragment).
...
pub const LocationEntity = struct {
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *LocationEntity {
        ...
        const e = h.A().create(LocationEntity) catch unreachable;
        e.* = .{
            .name = "location",
            .client = client,
    ...
    fn ent_ctx(self: *LocationEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *LocationEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *LocationEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *LocationEntity, ctx: *Context, post_done: *const fn (*LocationEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *LocationEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *LocationEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *LocationEntity, ctx: *Context, post_done: *const fn (*LocationEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *LocationEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *LocationEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return LocationEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *LocationEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *LocationEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *LocationEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *LocationEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *LocationEntity, ctx: *Context) void {
    ...
    pub fn list(self: *LocationEntity, reqmatch: Value, ctrl: Value) EntListResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
        ...
        var items: std.ArrayList(*LocationEntity) = .empty;
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = LocationEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice(h.A()) catch &[_]*LocationEntity{} };
    }
    
    fn list_post_done(self: *LocationEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            if (resmatch == .object) self.mtch = resmatch;
        }
    ...
    pub fn create(self: *LocationEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *LocationEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resdata = result.resdata;
            if (!h.is_noval(resdata)) {
                const cm = h.to_map(h.clone(resdata));
    ...
    pub fn update(self: *LocationEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
    ...
    pub fn remove(self: *LocationEntity, reqmatch: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "remove",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn remove_post_done(self: *LocationEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            const resdata = result.resdata;
            if (resmatch == .object) self.mtch = resmatch;
