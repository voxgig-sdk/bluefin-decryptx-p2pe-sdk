// Kif entity client (generated — mirrors the go/rust Entity fragment).
...
pub const KifEntity = struct {
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *KifEntity {
        ...
        const e = h.A().create(KifEntity) catch unreachable;
        e.* = .{
            .name = "kif",
            .client = client,
    ...
    fn ent_ctx(self: *KifEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *KifEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *KifEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *KifEntity, ctx: *Context, post_done: *const fn (*KifEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *KifEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *KifEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *KifEntity, ctx: *Context, post_done: *const fn (*KifEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *KifEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return KifEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *KifEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *KifEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *KifEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *KifEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn list(self: *KifEntity, reqmatch: Value, ctrl: Value) EntListResult {
        ...
        var items: std.ArrayList(*KifEntity) = .empty;
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = KifEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice(h.A()) catch &[_]*KifEntity{} };
    }
    
    fn list_post_done(self: *KifEntity, ctx: *Context) void {
    ...
    pub fn create(self: *KifEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *KifEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *KifEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
