// InjectKey entity client (generated — mirrors the go/rust Entity fragment).
...
pub const InjectKeyEntity = struct {
    name: []const u8 = "inject_key",
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *InjectKeyEntity {
        ...
        const e = h.A().create(InjectKeyEntity) catch unreachable;
        e.* = .{
            .name = "inject_key",
            .client = client,
    ...
    fn ent_ctx(self: *InjectKeyEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *InjectKeyEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *InjectKeyEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *InjectKeyEntity, ctx: *Context, post_done: *const fn (*InjectKeyEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *InjectKeyEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *InjectKeyEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *InjectKeyEntity, ctx: *Context, post_done: *const fn (*InjectKeyEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *InjectKeyEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *InjectKeyEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return InjectKeyEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *InjectKeyEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *InjectKeyEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *InjectKeyEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *InjectKeyEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *InjectKeyEntity, ctx: *Context) void {
    ...
    pub fn list(self: *InjectKeyEntity, reqmatch: Value, ctrl: Value) EntListResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
        ...
        var items: std.ArrayList(*InjectKeyEntity) = .empty;
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = InjectKeyEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice(h.A()) catch &[_]*InjectKeyEntity{} };
    }
    
    fn list_post_done(self: *InjectKeyEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            if (resmatch == .object) self.mtch = resmatch;
        }
    ...
    pub fn create(self: *InjectKeyEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *InjectKeyEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *InjectKeyEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
