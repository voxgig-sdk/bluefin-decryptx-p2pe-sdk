// Attestation entity client (generated — mirrors the go/rust Entity fragment).
...
pub const AttestationEntity = struct {
    name: []const u8 = "attestation",
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *AttestationEntity {
        ...
        const e = h.A().create(AttestationEntity) catch unreachable;
        e.* = .{
            .name = "attestation",
            .client = client,
    ...
    fn ent_ctx(self: *AttestationEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *AttestationEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *AttestationEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *AttestationEntity, ctx: *Context, post_done: *const fn (*AttestationEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *AttestationEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *AttestationEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *AttestationEntity, ctx: *Context, post_done: *const fn (*AttestationEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *AttestationEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *AttestationEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return AttestationEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *AttestationEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *AttestationEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *AttestationEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *AttestationEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *AttestationEntity, ctx: *Context) void {
    ...
    pub fn list(self: *AttestationEntity, reqmatch: Value, ctrl: Value) EntListResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
        ...
        var items: std.ArrayList(*AttestationEntity) = .empty;
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = AttestationEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice(h.A()) catch &[_]*AttestationEntity{} };
    }
    
    fn list_post_done(self: *AttestationEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            if (resmatch == .object) self.mtch = resmatch;
        }
    ...
    pub fn create(self: *AttestationEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *AttestationEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resdata = result.resdata;
            if (!h.is_noval(resdata)) {
                const cm = h.to_map(h.clone(resdata));
    ...
    pub fn update(self: *AttestationEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *AttestationEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
