// DeviceReceiveResult entity client (generated — mirrors the go/rust Entity fragment).
...
pub const DeviceReceiveResultEntity = struct {
    name: []const u8 = "device_receive_result",
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *DeviceReceiveResultEntity {
        ...
        const e = h.A().create(DeviceReceiveResultEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *DeviceReceiveResultEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *DeviceReceiveResultEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *DeviceReceiveResultEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *DeviceReceiveResultEntity, ctx: *Context, post_done: *const fn (*DeviceReceiveResultEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *DeviceReceiveResultEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *DeviceReceiveResultEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *DeviceReceiveResultEntity, ctx: *Context, post_done: *const fn (*DeviceReceiveResultEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *DeviceReceiveResultEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *DeviceReceiveResultEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return DeviceReceiveResultEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *DeviceReceiveResultEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *DeviceReceiveResultEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *DeviceReceiveResultEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *DeviceReceiveResultEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *DeviceReceiveResultEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *DeviceReceiveResultEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *DeviceReceiveResultEntity, ctx: *Context) void {
    ...
    pub fn update(self: *DeviceReceiveResultEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *DeviceReceiveResultEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
