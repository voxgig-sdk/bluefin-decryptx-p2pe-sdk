// Device entity client (generated — mirrors the go/rust Entity fragment).
...
pub const DeviceEntity = struct {
    name: []const u8 = "device",
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *DeviceEntity {
        ...
        const e = h.A().create(DeviceEntity) catch unreachable;
        e.* = .{
            .name = "device",
            .client = client,
    ...
    fn ent_ctx(self: *DeviceEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *DeviceEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *DeviceEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *DeviceEntity, ctx: *Context, post_done: *const fn (*DeviceEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *DeviceEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *DeviceEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *DeviceEntity, ctx: *Context, post_done: *const fn (*DeviceEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *DeviceEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *DeviceEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return DeviceEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *DeviceEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *DeviceEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *DeviceEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *DeviceEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *DeviceEntity, ctx: *Context) void {
    ...
    pub fn list(self: *DeviceEntity, reqmatch: Value, ctrl: Value) EntListResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    pub fn create(self: *DeviceEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *DeviceEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resdata = result.resdata;
            if (!h.is_noval(resdata)) {
                const cm = h.to_map(h.clone(resdata));
    ...
    pub fn update(self: *DeviceEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *DeviceEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
