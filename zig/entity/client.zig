...
pub const ClientEntity = struct {
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *ClientEntity {
    ...
    fn run_op_ent(self: *ClientEntity, ctx: *Context, post_done: *const fn (*ClientEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *ClientEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *ClientEntity) bool {
        return self.deleted;
    }
    ...
    pub fn load(self: *ClientEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    pub fn list(self: *ClientEntity, reqmatch: Value, ctrl: Value) EntListResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
        ...
        var items: std.ArrayList(*ClientEntity) = .empty;
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = ClientEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice(h.A()) catch &[_]*ClientEntity{} };
    }
    ...
    pub fn create(self: *ClientEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    pub fn update(self: *ClientEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
    ...
    pub fn remove(self: *ClientEntity, reqmatch: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "remove",
            .ctrl = ctrl,
            .mtch = self.mtch,
