...
pub const TransactionEntity = struct {
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *TransactionEntity {
    ...
    fn run_op_ent(self: *TransactionEntity, ctx: *Context, post_done: *const fn (*TransactionEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *TransactionEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *TransactionEntity) bool {
        return self.deleted;
    }
    ...
    pub fn load(self: *TransactionEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    pub fn list(self: *TransactionEntity, reqmatch: Value, ctrl: Value) EntListResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
        ...
        var items: std.ArrayList(*TransactionEntity) = .empty;
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = TransactionEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice(h.A()) catch &[_]*TransactionEntity{} };
    }
    ...
    pub fn create(self: *TransactionEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *TransactionEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resdata = result.resdata;
            if (!h.is_noval(resdata)) {
                const cm = h.to_map(h.clone(resdata));
    ...
    pub fn update(self: *TransactionEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *TransactionEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
