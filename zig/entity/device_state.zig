// DeviceState entity client (generated — mirrors the go/rust Entity fragment).
...
pub const DeviceStateEntity = struct {
    name: []const u8 = "device_state",
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *DeviceStateEntity {
        ...
        const e = h.A().create(DeviceStateEntity) catch unreachable;
        e.* = .{
            .name = "device_state",
            .client = client,
    ...
    fn ent_ctx(self: *DeviceStateEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *DeviceStateEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *DeviceStateEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *DeviceStateEntity, ctx: *Context, post_done: *const fn (*DeviceStateEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *DeviceStateEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *DeviceStateEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *DeviceStateEntity, ctx: *Context, post_done: *const fn (*DeviceStateEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *DeviceStateEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *DeviceStateEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return DeviceStateEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *DeviceStateEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *DeviceStateEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *DeviceStateEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *DeviceStateEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn list(self: *DeviceStateEntity, reqmatch: Value, ctrl: Value) EntListResult {
        ...
        var items: std.ArrayList(*DeviceStateEntity) = .empty;
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = DeviceStateEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice(h.A()) catch &[_]*DeviceStateEntity{} };
    }
    
    fn list_post_done(self: *DeviceStateEntity, ctx: *Context) void {
    ...
    pub fn create(self: *DeviceStateEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *DeviceStateEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *DeviceStateEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
