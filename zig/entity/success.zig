// Success entity client (generated — mirrors the go/rust Entity fragment).
...
pub const SuccessEntity = struct {
    name: []const u8 = "success",
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *SuccessEntity {
        ...
        const e = h.A().create(SuccessEntity) catch unreachable;
        e.* = .{
            .name = "success",
            .client = client,
    ...
    fn ent_ctx(self: *SuccessEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *SuccessEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *SuccessEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *SuccessEntity, ctx: *Context, post_done: *const fn (*SuccessEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *SuccessEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *SuccessEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *SuccessEntity, ctx: *Context, post_done: *const fn (*SuccessEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *SuccessEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *SuccessEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return SuccessEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *SuccessEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *SuccessEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *SuccessEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *SuccessEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *SuccessEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *SuccessEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *SuccessEntity, ctx: *Context) void {
    ...
    pub fn update(self: *SuccessEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
    ...
    pub fn remove(self: *SuccessEntity, reqmatch: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "remove",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn remove_post_done(self: *SuccessEntity, ctx: *Context) void {
        if (ctx.result) |result| {
