// DeviceType entity client (generated — mirrors the go/rust Entity fragment).
...
pub const DeviceTypeEntity = struct {
    name: []const u8 = "device_type",
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *DeviceTypeEntity {
        ...
        const e = h.A().create(DeviceTypeEntity) catch unreachable;
        e.* = .{
            .name = "device_type",
            .client = client,
    ...
    fn ent_ctx(self: *DeviceTypeEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *DeviceTypeEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *DeviceTypeEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *DeviceTypeEntity, ctx: *Context, post_done: *const fn (*DeviceTypeEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *DeviceTypeEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *DeviceTypeEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *DeviceTypeEntity, ctx: *Context, post_done: *const fn (*DeviceTypeEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *DeviceTypeEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *DeviceTypeEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return DeviceTypeEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *DeviceTypeEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *DeviceTypeEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *DeviceTypeEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *DeviceTypeEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *DeviceTypeEntity, ctx: *Context) void {
    ...
    pub fn list(self: *DeviceTypeEntity, reqmatch: Value, ctrl: Value) EntListResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
        ...
        var items: std.ArrayList(*DeviceTypeEntity) = .empty;
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = DeviceTypeEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice(h.A()) catch &[_]*DeviceTypeEntity{} };
    }
    
    fn list_post_done(self: *DeviceTypeEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            if (resmatch == .object) self.mtch = resmatch;
        }
    ...
    pub fn create(self: *DeviceTypeEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *DeviceTypeEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *DeviceTypeEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
