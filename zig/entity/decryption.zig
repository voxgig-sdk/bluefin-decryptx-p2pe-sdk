// Decryption entity client (generated — mirrors the go/rust Entity fragment).
...
pub const DecryptionEntity = struct {
    name: []const u8 = "decryption",
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *DecryptionEntity {
        ...
        const e = h.A().create(DecryptionEntity) catch unreachable;
        e.* = .{
            .name = "decryption",
            .client = client,
    ...
    fn ent_ctx(self: *DecryptionEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *DecryptionEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *DecryptionEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *DecryptionEntity, ctx: *Context, post_done: *const fn (*DecryptionEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *DecryptionEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *DecryptionEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *DecryptionEntity, ctx: *Context, post_done: *const fn (*DecryptionEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *DecryptionEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *DecryptionEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return DecryptionEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *DecryptionEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *DecryptionEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *DecryptionEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *DecryptionEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *DecryptionEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *DecryptionEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *DecryptionEntity, ctx: *Context) void {
    ...
    pub fn update(self: *DecryptionEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *DecryptionEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
