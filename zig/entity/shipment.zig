// Shipment entity client (generated — mirrors the go/rust Entity fragment).
...
pub const ShipmentEntity = struct {
    name: []const u8 = "shipment",
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *ShipmentEntity {
        ...
        const e = h.A().create(ShipmentEntity) catch unreachable;
        e.* = .{
            .name = "shipment",
            .client = client,
    ...
    fn ent_ctx(self: *ShipmentEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *ShipmentEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *ShipmentEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *ShipmentEntity, ctx: *Context, post_done: *const fn (*ShipmentEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *ShipmentEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *ShipmentEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *ShipmentEntity, ctx: *Context, post_done: *const fn (*ShipmentEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *ShipmentEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *ShipmentEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return ShipmentEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *ShipmentEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *ShipmentEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *ShipmentEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *ShipmentEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *ShipmentEntity, ctx: *Context) void {
    ...
    pub fn list(self: *ShipmentEntity, reqmatch: Value, ctrl: Value) EntListResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
        ...
        var items: std.ArrayList(*ShipmentEntity) = .empty;
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = ShipmentEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice(h.A()) catch &[_]*ShipmentEntity{} };
    }
    
    fn list_post_done(self: *ShipmentEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            if (resmatch == .object) self.mtch = resmatch;
        }
    ...
    pub fn create(self: *ShipmentEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *ShipmentEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resdata = result.resdata;
            if (!h.is_noval(resdata)) {
                const cm = h.to_map(h.clone(resdata));
    ...
    pub fn update(self: *ShipmentEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *ShipmentEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
