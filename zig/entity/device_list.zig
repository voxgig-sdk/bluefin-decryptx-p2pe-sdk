// DeviceList entity client (generated — mirrors the go/rust Entity fragment).
...
pub const DeviceListEntity = struct {
    name: []const u8 = "device_list",
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *DeviceListEntity {
        ...
        const e = h.A().create(DeviceListEntity) catch unreachable;
        e.* = .{
            .name = "device_list",
            .client = client,
    ...
    fn ent_ctx(self: *DeviceListEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *DeviceListEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *DeviceListEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *DeviceListEntity, ctx: *Context, post_done: *const fn (*DeviceListEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *DeviceListEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *DeviceListEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *DeviceListEntity, ctx: *Context, post_done: *const fn (*DeviceListEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *DeviceListEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *DeviceListEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return DeviceListEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *DeviceListEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *DeviceListEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *DeviceListEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *DeviceListEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *DeviceListEntity, ctx: *Context) void {
    ...
    pub fn list(self: *DeviceListEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("list", self.name) };
    }

    pub fn create(self: *DeviceListEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *DeviceListEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *DeviceListEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
