// DeviceBuild entity client (generated — mirrors the go/rust Entity fragment).
...
pub const DeviceBuildEntity = struct {
    name: []const u8 = "device_build",
    ...
    pub fn new(client: *sdk.BluefinDecryptxP2peSDK, entopts_in: Value) *DeviceBuildEntity {
        ...
        const e = h.A().create(DeviceBuildEntity) catch unreachable;
        e.* = .{
            .name = "device_build",
            .client = client,
    ...
    fn ent_ctx(self: *DeviceBuildEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *DeviceBuildEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *DeviceBuildEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *DeviceBuildEntity, ctx: *Context, post_done: *const fn (*DeviceBuildEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *DeviceBuildEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *DeviceBuildEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *DeviceBuildEntity, ctx: *Context, post_done: *const fn (*DeviceBuildEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *DeviceBuildEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *DeviceBuildEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return DeviceBuildEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *DeviceBuildEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *DeviceBuildEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *DeviceBuildEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *DeviceBuildEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *DeviceBuildEntity, ctx: *Context) void {
    ...
    pub fn list(self: *DeviceBuildEntity, reqmatch: Value, ctrl: Value) EntListResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
        ...
        var items: std.ArrayList(*DeviceBuildEntity) = .empty;
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = DeviceBuildEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice(h.A()) catch &[_]*DeviceBuildEntity{} };
    }
    
    fn list_post_done(self: *DeviceBuildEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            if (resmatch == .object) self.mtch = resmatch;
        }
    ...
    pub fn create(self: *DeviceBuildEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *DeviceBuildEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *DeviceBuildEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
