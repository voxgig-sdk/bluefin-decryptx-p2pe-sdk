// Kif entity client (generated — mirrors the rust Entity fragment).
...
static const char* kif_get_name(Entity* e);
...
Entity* kif_entity_new(BluefinDecryptxP2peSDK* client, voxgig_value* entopts) {
  ...
  kif_entity* self = (kif_entity*)calloc(1, sizeof(kif_entity));
  self->base.vt = &kif_VT;
  self->name = strdup("kif");
  self->client = client;
...
static voxgig_value* kif_run_op(kif_entity* self, Context* ctx,
                                    kif_postdone_fn post_done, PNError** err) {
...
voxgig_value* kif_stream(Entity* e, const char* action, voxgig_value* args,
...
static Entity* kif_make(Entity* e) {
  ...
  return kif_entity_new(self->client, opts);
}
...
static voxgig_value* kif_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(kif_ent_ctx(self), "SetData");
  }
  feature_hook_util(kif_ent_ctx(self), "GetData");
...
static voxgig_value* kif_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(kif_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(kif_ent_ctx(self), "GetMatch");
...
static void kif_list_postdone(kif_entity* self, Context* ctx) {
