// Device entity client (generated — mirrors the rust Entity fragment).
...
static const char* device_get_name(Entity* e);
...
Entity* device_entity_new(BluefinDecryptxP2peSDK* client, voxgig_value* entopts) {
  ...
  device_entity* self = (device_entity*)calloc(1, sizeof(device_entity));
  self->base.vt = &device_VT;
  self->name = strdup("device");
  self->client = client;
...
static voxgig_value* device_run_op(device_entity* self, Context* ctx,
                                    device_postdone_fn post_done, PNError** err) {
...
voxgig_value* device_stream(Entity* e, const char* action, voxgig_value* args,
...
static Entity* device_make(Entity* e) {
  ...
  return device_entity_new(self->client, opts);
}
...
static voxgig_value* device_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(device_ent_ctx(self), "SetData");
  }
  feature_hook_util(device_ent_ctx(self), "GetData");
...
static voxgig_value* device_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(device_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(device_ent_ctx(self), "GetMatch");
  return voxgig_clone(self->mtch);
}


static void device_load_postdone(device_entity* self, Context* ctx) {
...
static void device_list_postdone(device_entity* self, Context* ctx) {
  SdkResult* result = ctx->result;
  if (result) {
    voxgig_value* resmatch = result->resmatch;
    if (voxgig_is_map(resmatch)) self->mtch = resmatch;
...
static void device_create_postdone(device_entity* self, Context* ctx) {
  SdkResult* result = ctx->result;
  if (result) {
    voxgig_value* resdata = result->resdata;
    if (!v_is_noval(resdata) && !v_is_null(resdata)) {
