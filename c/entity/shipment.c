// Shipment entity client (generated — mirrors the rust Entity fragment).
...
typedef struct shipment_entity {
...
static const char* shipment_get_name(Entity* e);
...
Entity* shipment_entity_new(BluefinDecryptxP2peSDK* client, voxgig_value* entopts) {
  ...
  shipment_entity* self = (shipment_entity*)calloc(1, sizeof(shipment_entity));
  self->base.vt = &shipment_VT;
  self->name = strdup("shipment");
  self->client = client;
...
static voxgig_value* shipment_run_op(shipment_entity* self, Context* ctx,
                                    shipment_postdone_fn post_done, PNError** err) {
...
voxgig_value* shipment_stream(Entity* e, const char* action, voxgig_value* args,
...
static Entity* shipment_make(Entity* e) {
  ...
  return shipment_entity_new(self->client, opts);
}
...
static voxgig_value* shipment_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(shipment_ent_ctx(self), "SetData");
  }
  feature_hook_util(shipment_ent_ctx(self), "GetData");
...
static voxgig_value* shipment_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(shipment_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(shipment_ent_ctx(self), "GetMatch");
  return voxgig_clone(self->mtch);
}


static void shipment_load_postdone(shipment_entity* self, Context* ctx) {
...
static void shipment_list_postdone(shipment_entity* self, Context* ctx) {
  SdkResult* result = ctx->result;
  if (result) {
    voxgig_value* resmatch = result->resmatch;
    if (voxgig_is_map(resmatch)) self->mtch = resmatch;
...
static void shipment_create_postdone(shipment_entity* self, Context* ctx) {
  SdkResult* result = ctx->result;
  if (result) {
    voxgig_value* resdata = result->resdata;
    if (!v_is_noval(resdata) && !v_is_null(resdata)) {
