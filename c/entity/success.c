// Success entity client (generated — mirrors the rust Entity fragment).
...
static const char* success_get_name(Entity* e);
...
Entity* success_entity_new(BluefinDecryptxP2peSDK* client, voxgig_value* entopts) {
  ...
  success_entity* self = (success_entity*)calloc(1, sizeof(success_entity));
  self->base.vt = &success_VT;
  self->name = strdup("success");
  self->client = client;
...
static voxgig_value* success_run_op(success_entity* self, Context* ctx,
                                    success_postdone_fn post_done, PNError** err) {
...
voxgig_value* success_stream(Entity* e, const char* action, voxgig_value* args,
...
static Entity* success_make(Entity* e) {
  ...
  return success_entity_new(self->client, opts);
}
...
static voxgig_value* success_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(success_ent_ctx(self), "SetData");
  }
  feature_hook_util(success_ent_ctx(self), "GetData");
...
static voxgig_value* success_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(success_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(success_ent_ctx(self), "GetMatch");
...
static void success_create_postdone(success_entity* self, Context* ctx) {
...
static void success_remove_postdone(success_entity* self, Context* ctx) {
  SdkResult* result = ctx->result;
  if (result) {
