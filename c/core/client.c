// BluefinDecryptxP2peSDK client (generated — mirrors the rust Main fragment).
...
BluefinDecryptxP2peSDK* bluefindecryptxp2pe_sdk_new(voxgig_value* options) {
  BluefinDecryptxP2peSDK* sdk = (BluefinDecryptxP2peSDK*)calloc(1, sizeof(BluefinDecryptxP2peSDK));
...
static bool sdk_op_allowed(BluefinDecryptxP2peSDK* sdk, const char* op) {
...
static voxgig_value* sdk_op_denied(BluefinDecryptxP2peSDK* sdk, const char* op) {
  voxgig_value* allow_op = getpath2(sdk->options, "allow", "op");
...
static voxgig_value* sdk_raw_request(
  BluefinDecryptxP2peSDK* sdk, voxgig_value* fetchargs, PNError** err) {
  *err = NULL;
  Utility* utility = sdk->utility;
...
voxgig_value* sdk_graphql(BluefinDecryptxP2peSDK* sdk, const char* query,
  ...
  *err = NULL;
  ...
  if (voxgig_is_list(errors) && 0 < voxgig_as_list(errors)->len) {
    ...
    char msg[512];
    snprintf(msg, sizeof(msg), "BluefinDecryptxP2peSDK: graphql: %s", m);
...
BluefinDecryptxP2peSDK* test_sdk(voxgig_value* testopts, voxgig_value* sdkopts) {
  ...
  BluefinDecryptxP2peSDK* sdk = bluefindecryptxp2pe_sdk_new(sdkopts);
  free(sdk->mode);
  sdk->mode = strdup("test");
  return sdk;
}
