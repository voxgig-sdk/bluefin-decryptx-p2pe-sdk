// Generated direct-call test for the device_custody_detail entity (mirrors the
...
static voxgig_value* device_custody_detail_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinDecryptxP2peSDK* device_custody_detail_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(device_custody_detail_mock, mockres)));
...
int main(void) {
  ...
  // LOAD
  {
    ...
    BluefinDecryptxP2peSDK* sdk = device_custody_detail_direct_setup(mockres);
    voxgig_value* params = v_map();
    setp(params, "device_type", v_str("direct01"));
    setp(params, "id", v_str("direct02"));
    setp(params, "serial_number", v_str("direct03"));
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("devices/{serial_number}/{device_type}/custody/{id}"),
      "method", v_str("GET"),
      "params", params), &err);
  ...
  TEST_SUMMARY("bluefin_decryptx_p2pe_device_custody_detail_direct");
}
