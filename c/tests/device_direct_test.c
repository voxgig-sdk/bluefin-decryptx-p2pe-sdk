// Generated direct-call test for the device entity (mirrors the
...
static voxgig_value* device_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinDecryptxP2peSDK* device_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(device_mock, mockres)));
...
int main(void) {
  ...
  // LIST
  {
    ...
    BluefinDecryptxP2peSDK* sdk = device_direct_setup(mockres);
  ...
  // LOAD
  {
    ...
    setp(params, "serial_number", v_str("direct02"));
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("devices/{serial_number}/{device_type}"),
      "method", v_str("GET"),
      "params", params), &err);
  ...
  TEST_SUMMARY("bluefin_decryptx_p2pe_device_direct");
}
