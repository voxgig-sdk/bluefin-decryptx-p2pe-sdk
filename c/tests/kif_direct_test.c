// Generated direct-call test for the kif entity (mirrors the
...
static voxgig_value* kif_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinDecryptxP2peSDK* kif_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(kif_mock, mockres)));
...
int main(void) {
  ...
  // LIST
  {
    ...
    BluefinDecryptxP2peSDK* sdk = kif_direct_setup(mockres);
  ...
  TEST_SUMMARY("bluefin_decryptx_p2pe_kif_direct");
}
