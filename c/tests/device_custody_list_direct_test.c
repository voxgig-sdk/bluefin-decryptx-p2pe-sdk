// Generated direct-call test for the device_custody_list entity (mirrors the
...
static voxgig_value* device_custody_list_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinDecryptxP2peSDK* device_custody_list_direct_setup(voxgig_value* mockres) {
...
int main(void) {
  ...
  // LIST
  {
    ...
    BluefinDecryptxP2peSDK* sdk = device_custody_list_direct_setup(mockres);
    ...
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("devices/{serial_number}/{device_type}/custody"),
      "method", v_str("GET"),
      "params", params), &err);
