// Generated direct-call test for the device_list entity (mirrors the
...
static voxgig_value* device_list_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinDecryptxP2peSDK* device_list_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(device_list_mock, mockres)));
...
int main(void) {
  ...
  // LOAD
  {
    ...
    BluefinDecryptxP2peSDK* sdk = device_list_direct_setup(mockres);
    voxgig_value* params = v_map();
    setp(params, "share_partner_to", v_str("direct01"));
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("virtualDevices/{share_partner_to}"),
      "method", v_str("GET"),
      "params", params), &err);
  ...
  TEST_SUMMARY("bluefin_decryptx_p2pe_device_list_direct");
}
