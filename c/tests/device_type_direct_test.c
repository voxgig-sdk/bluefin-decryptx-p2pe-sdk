// Generated direct-call test for the device_type entity (mirrors the
...
static voxgig_value* device_type_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinDecryptxP2peSDK* device_type_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(device_type_mock, mockres)));
...
int main(void) {
  ...
  // LIST
  {
    ...
    BluefinDecryptxP2peSDK* sdk = device_type_direct_setup(mockres);
    voxgig_value* params = v_map();
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("deviceTypes"),
      "method", v_str("GET"),
      "params", params), &err);
  ...
  TEST_SUMMARY("bluefin_decryptx_p2pe_device_type_direct");
}
