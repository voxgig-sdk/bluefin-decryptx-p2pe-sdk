// Generated direct-call test for the device_state entity (mirrors the
...
static voxgig_value* device_state_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinDecryptxP2peSDK* device_state_direct_setup(voxgig_value* mockres) {
...
int main(void) {
  ...
  // LIST
  {
    ...
    BluefinDecryptxP2peSDK* sdk = device_state_direct_setup(mockres);
    voxgig_value* params = v_map();
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("deviceStates"),
      "method", v_str("GET"),
      "params", params), &err);
