// Command BluefinDecryptxP2pe-cli is a CLI + REPL for the BluefinDecryptxP2pe SDK,
...
const prompt = "bluefin-decryptx-p2pe"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "attestation client create_result decryption device device_build device_custody_detail device_custody_list device_list device_receive_result device_rki_activate_result device_state device_type inject_key kif location partner shipment success transaction update_result user"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BLUEFIN_DECRYPTX_P2PE_APIKEY (api key), BLUEFIN_DECRYPTX_P2PE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BLUEFIN_DECRYPTX_P2PE_APIKEY carries the API key and
	// BLUEFIN_DECRYPTX_P2PE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BLUEFIN_DECRYPTX_P2PE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BLUEFIN_DECRYPTX_P2PE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBluefinDecryptxP2peSDK(opts)
