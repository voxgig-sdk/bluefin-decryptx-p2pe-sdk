    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFIN_DECRYPTX_P2PE_TEST_DECRYPTION_ENTID JSON to run live")
        client = setup["client"]
        ...
        decryption_ref01_data = helpers.to_map(runner.entity_data(decryption_ref01_ent.create(decryption_ref01_data, None)))
...
def _decryption_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFIN_DECRYPTX_P2PE_TEST_DECRYPTION_ENTID"))
