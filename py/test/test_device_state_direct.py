...
class TestDeviceStateDirect:

    def test_should_direct_list_device_state(self):
        setup = _device_state_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-device_state", "live" if setup["live"] else "unit")
        if _skip:
...
def _device_state_direct_setup(mockres):
