...
class TestDeviceBuildDirect:

    def test_should_direct_list_device_build(self):
        setup = _device_build_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-device_build", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_device_build(self):
        setup = _device_build_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-device_build", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _device_build_direct_setup(mockres):
