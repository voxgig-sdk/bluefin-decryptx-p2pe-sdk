...
class TestKifDirect:
    ...
    def test_should_direct_list_kif(self):
        setup = _kif_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-kif", "live" if setup["live"] else "unit")
        if _skip:
