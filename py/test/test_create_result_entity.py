...
class TestCreateResultEntity:

    def test_should_create_instance(self):
        testsdk = BluefinDecryptxP2peSDK.test(None, None)
        ent = testsdk.CreateResult(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _create_result_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "create_result." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFIN_DECRYPTX_P2PE_TEST_CREATE_RESULT_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        create_result_ref01_ent = client.CreateResult(None)
        ...
        create_result_ref01_data = helpers.to_map(runner.entity_data(create_result_ref01_ent.create(create_result_ref01_data, None)))
        assert create_result_ref01_data is not None



def _create_result_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/create_result/CreateResultTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFIN_DECRYPTX_P2PE_TEST_CREATE_RESULT_ENTID"))
