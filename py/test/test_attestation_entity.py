...
class TestAttestationEntity:

    def test_should_create_instance(self):
        testsdk = BluefinDecryptxP2peSDK.test(None, None)
        ent = testsdk.Attestation(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Attestation(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Attestation(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _attestation_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "attestation." + _op, "live" if _live else "unit")
            if _skip:
        ...
        # CREATE
        attestation_ref01_ent = client.Attestation(None)
        ...
        attestation_ref01_list_result = attestation_ref01_ent.list(attestation_ref01_match, None)
        assert isinstance(attestation_ref01_list_result, list)
...
def _attestation_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/attestation/AttestationTestData.json")
