# BluefinDecryptxP2pe SDK test runner
...
class BluefinDecryptxP2peTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BluefinDecryptxP2peTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BluefinDecryptxP2peTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BluefinDecryptxP2peTestRunner.getenv("BLUEFIN_DECRYPTX_P2PE_TEST_LIVE")
        override = BluefinDecryptxP2peTestRunner.getenv("BLUEFIN_DECRYPTX_P2PE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BluefinDecryptxP2peTestRunner.getenv(key)
        ...
        explain = BluefinDecryptxP2peTestRunner.getenv("BLUEFIN_DECRYPTX_P2PE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if BluefinDecryptxP2peTestRunner._test_control is not None:
            return BluefinDecryptxP2peTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BluefinDecryptxP2peTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BluefinDecryptxP2peTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BluefinDecryptxP2peTestRunner.load_test_control()
...
def env_override(m):
    return BluefinDecryptxP2peTestRunner.env_override(m)


def entity_data(v):
    return BluefinDecryptxP2peTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BluefinDecryptxP2peTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BluefinDecryptxP2peTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BluefinDecryptxP2peTestRunner.load_test_control()


def live_client_options():
    return BluefinDecryptxP2peTestRunner.live_client_options()


def live_delay_ms():
    return BluefinDecryptxP2peTestRunner.live_delay_ms()
