    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_RKI_ACTIVATE_RESULT_ENTID JSON to run live")
        client = setup["client"]
        ...
        device_rki_activate_result_ref01_data = helpers.to_map(runner.entity_data(device_rki_activate_result_ref01_ent.create(device_rki_activate_result_ref01_data, None)))
...
def _device_rki_activate_result_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_RKI_ACTIVATE_RESULT_ENTID"))
