    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFINDECRYPTXP_PE_TEST_LOCATION_ENTID JSON to run live")
        client = setup["client"]
        ...
        location_ref01_data = helpers.to_map(location_ref01_ent.create(location_ref01_data, None))
...
def _location_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFINDECRYPTXP_PE_TEST_LOCATION_ENTID"))
