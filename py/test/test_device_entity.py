...
class TestDeviceEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Device(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Device(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _device_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "device." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        device_ref01_ent = client.Device(None)
        ...
        device_ref01_data = helpers.to_map(runner.entity_data(device_ref01_ent.create(device_ref01_data, None)))
        assert device_ref01_data is not None
        assert device_ref01_data["id"] is not None

        # LIST
        device_ref01_match = {}

        device_ref01_list_result = device_ref01_ent.list(device_ref01_match, None)
        assert isinstance(device_ref01_list_result, list)
        ...
        device_ref01_data_dt0_loaded = device_ref01_ent.load(device_ref01_match_dt0, None)
        device_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(device_ref01_data_dt0_loaded))
        assert device_ref01_data_dt0_load_result is not None
        assert device_ref01_data_dt0_load_result["id"] == device_ref01_data["id"]



def _device_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/device/DeviceTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_ENTID"))
