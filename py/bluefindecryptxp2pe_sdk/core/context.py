...
from bluefindecryptxp2pe_sdk.core.helpers import get_ctx_prop, to_map
...
class BluefinDecryptxP2peContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = BluefinDecryptxP2peControl()
        ...
        if isinstance(sp, BluefinDecryptxP2peSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, BluefinDecryptxP2peResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, BluefinDecryptxP2peResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return BluefinDecryptxP2peOperation({})
        ...
        op = BluefinDecryptxP2peOperation({
            "entity": entname,
            "name": opname,
            "input": inpt,
            "points": points,
    ...
    def make_error(self, code, msg):
        return BluefinDecryptxP2peError(code, msg, self)
