...
class SuccessEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return SuccessEntity(self._client, opts)
    ...
    def create(self, reqdata: SuccessCreateData, ctrl=None) -> Success:
        utility = self._utility
    ...
    def remove(self, reqmatch=None, ctrl=None) -> Success:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.Success().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
