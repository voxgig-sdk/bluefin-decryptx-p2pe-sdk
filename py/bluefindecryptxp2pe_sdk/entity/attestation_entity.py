...
from bluefindecryptxp2pe_sdk.bluefindecryptxp2pe_types import (
    Attestation,
...
class AttestationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AttestationEntity(self._client, opts)
    ...
    def match_get(self) -> Attestation:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Attestation:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Attestation().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Attestation]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Attestation().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
    ...
    def create(self, reqdata: AttestationCreateData, ctrl=None) -> Attestation:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
