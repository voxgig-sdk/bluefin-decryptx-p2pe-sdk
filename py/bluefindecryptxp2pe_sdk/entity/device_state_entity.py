...
class DeviceStateEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DeviceStateEntity(self._client, opts)
    ...
    def match_get(self) -> DeviceState:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[DeviceState]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.DeviceState().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
