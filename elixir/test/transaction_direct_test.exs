...
defmodule BluefinDecryptxP2pe.TransactionDirectTest do
  use ExUnit.Case
  ...
  test "should direct list transaction" do
    ...
    sdk = mk(canned)
    res = BluefinDecryptxP2pe.direct(sdk, H.deep(%{"path" => "/transaction", "method" => "GET", "params" => %{}}))
  ...
  test "should direct load transaction" do
    ...
    sdk = mk(canned)
    res = BluefinDecryptxP2pe.direct(sdk, H.deep(%{"path" => "/transaction/direct01", "method" => "GET", "params" => %{}}))
    assert S.getprop(res, "ok") == true
    assert H.to_int(S.getprop(res, "status")) == 200
