# InjectKey direct test (offline, mock system.fetch)

defmodule BluefinDecryptxP2pe.InjectKeyDirectTest do
  use ExUnit.Case
  ...
  test "should direct list inject_key" do
    canned = S.jt([S.jm(["id", "direct01"]), S.jm(["id", "direct02"])])
    sdk = mk(canned)
    res = BluefinDecryptxP2pe.direct(sdk, H.deep(%{"path" => "/inject_key", "method" => "GET", "params" => %{}}))
  ...
  test "should direct load inject_key" do
    canned = S.jm(["id", "direct01"])
    sdk = mk(canned)
    res = BluefinDecryptxP2pe.direct(sdk, H.deep(%{"path" => "/inject_key/direct01", "method" => "GET", "params" => %{}}))
    assert S.getprop(res, "ok") == true
    assert H.to_int(S.getprop(res, "status")) == 200
