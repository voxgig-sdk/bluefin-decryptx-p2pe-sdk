# Location direct test (offline, mock system.fetch)

defmodule BluefinDecryptxP2pe.LocationDirectTest do
  use ExUnit.Case
  ...
  test "should direct list location" do
    canned = S.jt([S.jm(["id", "direct01"]), S.jm(["id", "direct02"])])
    sdk = mk(canned)
    res = BluefinDecryptxP2pe.direct(sdk, H.deep(%{"path" => "/location", "method" => "GET", "params" => %{}}))
  ...
  test "should direct load location" do
    canned = S.jm(["id", "direct01"])
    sdk = mk(canned)
    res = BluefinDecryptxP2pe.direct(sdk, H.deep(%{"path" => "/location/direct01", "method" => "GET", "params" => %{}}))
    assert S.getprop(res, "ok") == true
    assert H.to_int(S.getprop(res, "status")) == 200
