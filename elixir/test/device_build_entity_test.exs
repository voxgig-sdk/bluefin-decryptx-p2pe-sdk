# DeviceBuild entity test (offline, mock transport)

defmodule BluefinDecryptxP2pe.DeviceBuildEntityTest do
  use ExUnit.Case
  ...
  defp fixture do
    Json.parse(File.read!("../.sdk/test/entity/device_build/DeviceBuildTestData.json"))
  end
  ...
  defp first_id do
    existing = H.or_(S.getpath(fixture(), "existing.device_build"), S.jm([]))
  ...
  test "should create instance" do
    ...
    ent = BluefinDecryptxP2pe.device_build(sdk)
    assert ent != nil
  end
  ...
  test "should list records" do
    ...
    result = BluefinDecryptxP2pe.Entity.DeviceBuild.list(ent, S.jm([]))
  ...
  test "should load an existing record" do
    ...
    if id != nil do
      sdk = mk_sdk()
      ent = BluefinDecryptxP2pe.device_build(sdk)
      loaded = BluefinDecryptxP2pe.Entity.DeviceBuild.load(ent, S.jm(["id", id]))
