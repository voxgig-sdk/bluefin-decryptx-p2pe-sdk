# DeviceBuild direct test (offline, mock system.fetch)

defmodule BluefinDecryptxP2pe.DeviceBuildDirectTest do
  use ExUnit.Case
  ...
  test "should direct list device_build" do
    canned = S.jt([S.jm(["id", "direct01"]), S.jm(["id", "direct02"])])
    sdk = mk(canned)
    res = BluefinDecryptxP2pe.direct(sdk, H.deep(%{"path" => "/device_build", "method" => "GET", "params" => %{}}))
  ...
  test "should direct load device_build" do
    canned = S.jm(["id", "direct01"])
    sdk = mk(canned)
    res = BluefinDecryptxP2pe.direct(sdk, H.deep(%{"path" => "/device_build/direct01", "method" => "GET", "params" => %{}}))
    assert S.getprop(res, "ok") == true
    assert H.to_int(S.getprop(res, "status")) == 200
