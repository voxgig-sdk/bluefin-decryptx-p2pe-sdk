# DeviceType entity test (offline, mock transport)

defmodule BluefinDecryptxP2pe.DeviceTypeEntityTest do
  use ExUnit.Case
  ...
  defp fixture do
    Json.parse(File.read!("../.sdk/test/entity/device_type/DeviceTypeTestData.json"))
  end
  ...
  defp first_id do
    existing = H.or_(S.getpath(fixture(), "existing.device_type"), S.jm([]))
  ...
  test "should create instance" do
    ...
    ent = BluefinDecryptxP2pe.device_type(sdk)
    assert ent != nil
  end
  ...
  test "should list records" do
    ...
    result = BluefinDecryptxP2pe.Entity.DeviceType.list(ent, S.jm([]))
  ...
  test "should load an existing record" do
    ...
    if id != nil do
      sdk = mk_sdk()
      ent = BluefinDecryptxP2pe.device_type(sdk)
      loaded = BluefinDecryptxP2pe.Entity.DeviceType.load(ent, S.jm(["id", id]))
