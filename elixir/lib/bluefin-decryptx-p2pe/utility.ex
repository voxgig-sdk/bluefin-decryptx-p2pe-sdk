# BluefinDecryptxP2pe SDK utility
#
...
defmodule BluefinDecryptxP2pe.Utility do
  alias Voxgig.Struct, as: S
  alias BluefinDecryptxP2pe.Helpers, as: H
  alias BluefinDecryptxP2pe.{Context, Spec, Result, Response, Operation}

  @default_user_agent "Mozilla/5.0 (compatible; BluefinDecryptxP2peSDK/1.0)"
  ...
  def make_options_impl(ctx) do
    ...
    # (BluefinDecryptxP2pe.Config.shared_config) and merge uses its nested nodes as
      ...
      resolved =
        Regex.replace(~r/\{([A-Za-z0-9_]+)\}/, base, fn _match, name ->
          ...
          cond do
            ...
            true ->
              raise BluefinDecryptxP2pe.Error,
  ...
  def make_error_impl(ctx, err) do
    ...
    errmsg =
      cond do
        match?(%BluefinDecryptxP2pe.Error{}, err) -> err.msg
    ...
    msg = "BluefinDecryptxP2peSDK: " <> opname <> ": " <> (errmsg || "")
    msg = clean(ctx, msg)
    ...
    sdk_err = %BluefinDecryptxP2pe.Error{
      code: "",
      msg: msg,
      sdk: "BluefinDecryptxP2pe",
      ctx: ctx,
    ...
    sdk_err = if match?(%BluefinDecryptxP2pe.Error{}, err), do: %{sdk_err | code: err.code}, else: sdk_err
  ...
  defp err_msg(e) do
    cond do
      match?(%BluefinDecryptxP2pe.Error{}, e) -> e.msg
  ...
  defp safe_json(str) do
    try do
      {:ok, BluefinDecryptxP2pe.Json.parse(str)}
    rescue
      _ -> :error
    end
  end
