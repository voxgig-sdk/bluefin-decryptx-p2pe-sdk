// Device entity client (generated — mirrors the go Entity fragment).
...
impl DeviceEntity {
    pub fn new(client: &Rc<BluefinDecryptxP2peSDK>, entopts: Value) -> Rc<DeviceEntity> {
...
impl Entity for DeviceEntity {
    ...
    fn make(&self) -> Rc<dyn Entity> {
        ...
        DeviceEntity::new(&self.client, opts) as Rc<dyn Entity>
    }
...
impl BluefinDecryptxP2peEntity for DeviceEntity {
    ...
    fn list(self: &Rc<Self>, reqmatch: Value, ctrl: Value) -> Result<Vec<Rc<Self>>, BluefinDecryptxP2peError> {
        ...
        if let Value::List(list) = &out {
            for entry in list.borrow().iter() {
                let ent = DeviceEntity::new(&self.client, vs::clone(&self.entopts));
