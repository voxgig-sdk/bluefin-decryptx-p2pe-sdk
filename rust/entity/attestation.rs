// Attestation entity client (generated — mirrors the go Entity fragment).
...
impl AttestationEntity {
    pub fn new(client: &Rc<BluefinDecryptxP2peSDK>, entopts: Value) -> Rc<AttestationEntity> {
        ...
        let e = Rc::new(AttestationEntity {
...
impl Entity for AttestationEntity {
    ...
    fn make(&self) -> Rc<dyn Entity> {
        ...
        AttestationEntity::new(&self.client, opts) as Rc<dyn Entity>
    }
...
impl BluefinDecryptxP2peEntity for AttestationEntity {
    ...
    fn list(self: &Rc<Self>, reqmatch: Value, ctrl: Value) -> Result<Vec<Rc<Self>>, BluefinDecryptxP2peError> {
        ...
        if let Value::List(list) = &out {
            for entry in list.borrow().iter() {
                let ent = AttestationEntity::new(&self.client, vs::clone(&self.entopts));
