// DeviceState entity client (generated — mirrors the go Entity fragment).
...
impl DeviceStateEntity {
    pub fn new(client: &Rc<BluefinDecryptxP2peSDK>, entopts: Value) -> Rc<DeviceStateEntity> {
        ...
        let e = Rc::new(DeviceStateEntity {
            name: "device_state".to_string(),
            client: client.clone(),
...
impl Entity for DeviceStateEntity {
    ...
    fn make(&self) -> Rc<dyn Entity> {
        ...
        DeviceStateEntity::new(&self.client, opts) as Rc<dyn Entity>
    }
...
impl BluefinDecryptxP2peEntity for DeviceStateEntity {
    ...
    fn list(self: &Rc<Self>, reqmatch: Value, ctrl: Value) -> Result<Vec<Rc<Self>>, BluefinDecryptxP2peError> {
        ...
        if let Value::List(list) = &out {
            for entry in list.borrow().iter() {
                let ent = DeviceStateEntity::new(&self.client, vs::clone(&self.entopts));
