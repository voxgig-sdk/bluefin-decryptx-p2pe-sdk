...
impl PartnerEntity {
    pub fn new(client: &Rc<BluefinDecryptxP2peSDK>, entopts: Value) -> Rc<PartnerEntity> {
...
impl BluefinDecryptxP2peEntity for PartnerEntity {
    ...
    fn list(self: &Rc<Self>, reqmatch: Value, ctrl: Value) -> Result<Vec<Rc<Self>>, BluefinDecryptxP2peError> {
        ...
        if let Value::List(list) = &out {
            for entry in list.borrow().iter() {
                let ent = PartnerEntity::new(&self.client, vs::clone(&self.entopts));
