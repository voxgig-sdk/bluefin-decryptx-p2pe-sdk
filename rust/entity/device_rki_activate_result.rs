...
impl BluefinDecryptxP2peEntity for DeviceRkiActivateResultEntity {
    fn load(&self, _reqmatch: Value, _ctrl: Value) -> Result<Value, BluefinDecryptxP2peError> {
        Err(crate::core::helpers::unsupported_op("load", &self.name))
    }

    fn list(&self, _reqmatch: Value, _ctrl: Value) -> Result<Value, BluefinDecryptxP2peError> {
        Err(crate::core::helpers::unsupported_op("list", &self.name))
    }


    fn create(&self, reqdata: Value, ctrl: Value) -> Result<Value, BluefinDecryptxP2peError> {
        let ctx = self.utility.make_context(
            CtxSpec {
