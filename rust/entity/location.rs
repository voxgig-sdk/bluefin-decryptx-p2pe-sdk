// Location entity client (generated — mirrors the go Entity fragment).
...
impl LocationEntity {
    pub fn new(client: &Rc<BluefinDecryptxP2peSDK>, entopts: Value) -> Rc<LocationEntity> {
...
impl Entity for LocationEntity {
    ...
    fn make(&self) -> Rc<dyn Entity> {
        ...
        LocationEntity::new(&self.client, opts) as Rc<dyn Entity>
    }
...
impl BluefinDecryptxP2peEntity for LocationEntity {
    ...
    fn list(self: &Rc<Self>, reqmatch: Value, ctrl: Value) -> Result<Vec<Rc<Self>>, BluefinDecryptxP2peError> {
        ...
        if let Value::List(list) = &out {
            for entry in list.borrow().iter() {
                let ent = LocationEntity::new(&self.client, vs::clone(&self.entopts));
