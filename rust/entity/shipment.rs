// Shipment entity client (generated — mirrors the go Entity fragment).
...
impl ShipmentEntity {
    pub fn new(client: &Rc<BluefinDecryptxP2peSDK>, entopts: Value) -> Rc<ShipmentEntity> {
...
impl Entity for ShipmentEntity {
    ...
    fn make(&self) -> Rc<dyn Entity> {
        ...
        ShipmentEntity::new(&self.client, opts) as Rc<dyn Entity>
    }
...
impl BluefinDecryptxP2peEntity for ShipmentEntity {
    ...
    fn list(self: &Rc<Self>, reqmatch: Value, ctrl: Value) -> Result<Vec<Rc<Self>>, BluefinDecryptxP2peError> {
        ...
        if let Value::List(list) = &out {
            for entry in list.borrow().iter() {
                let ent = ShipmentEntity::new(&self.client, vs::clone(&self.entopts));
