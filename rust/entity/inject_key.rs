// InjectKey entity client (generated — mirrors the go Entity fragment).
...
impl InjectKeyEntity {
    pub fn new(client: &Rc<BluefinDecryptxP2peSDK>, entopts: Value) -> Rc<InjectKeyEntity> {
...
impl Entity for InjectKeyEntity {
    ...
    fn make(&self) -> Rc<dyn Entity> {
        ...
        InjectKeyEntity::new(&self.client, opts) as Rc<dyn Entity>
    }
...
impl BluefinDecryptxP2peEntity for InjectKeyEntity {
    ...
    fn list(self: &Rc<Self>, reqmatch: Value, ctrl: Value) -> Result<Vec<Rc<Self>>, BluefinDecryptxP2peError> {
        ...
        if let Value::List(list) = &out {
            for entry in list.borrow().iter() {
                let ent = InjectKeyEntity::new(&self.client, vs::clone(&self.entopts));
