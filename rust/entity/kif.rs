// Kif entity client (generated — mirrors the go Entity fragment).
...
impl KifEntity {
    pub fn new(client: &Rc<BluefinDecryptxP2peSDK>, entopts: Value) -> Rc<KifEntity> {
...
impl Entity for KifEntity {
    ...
    fn make(&self) -> Rc<dyn Entity> {
        ...
        KifEntity::new(&self.client, opts) as Rc<dyn Entity>
    }
...
impl BluefinDecryptxP2peEntity for KifEntity {
    ...
    fn list(self: &Rc<Self>, reqmatch: Value, ctrl: Value) -> Result<Vec<Rc<Self>>, BluefinDecryptxP2peError> {
        ...
        if let Value::List(list) = &out {
            for entry in list.borrow().iter() {
                let ent = KifEntity::new(&self.client, vs::clone(&self.entopts));
