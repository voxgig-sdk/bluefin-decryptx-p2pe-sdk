// DeviceBuild entity client (generated — mirrors the go Entity fragment).
...
impl DeviceBuildEntity {
    pub fn new(client: &Rc<BluefinDecryptxP2peSDK>, entopts: Value) -> Rc<DeviceBuildEntity> {
        ...
        let e = Rc::new(DeviceBuildEntity {
...
impl Entity for DeviceBuildEntity {
    ...
    fn make(&self) -> Rc<dyn Entity> {
        ...
        DeviceBuildEntity::new(&self.client, opts) as Rc<dyn Entity>
    }
...
impl BluefinDecryptxP2peEntity for DeviceBuildEntity {
    ...
    fn list(self: &Rc<Self>, reqmatch: Value, ctrl: Value) -> Result<Vec<Rc<Self>>, BluefinDecryptxP2peError> {
        ...
        if let Value::List(list) = &out {
            for entry in list.borrow().iter() {
                let ent = DeviceBuildEntity::new(&self.client, vs::clone(&self.entopts));
