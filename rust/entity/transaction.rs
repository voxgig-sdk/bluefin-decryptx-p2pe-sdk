...
impl TransactionEntity {
    pub fn new(client: &Rc<BluefinDecryptxP2peSDK>, entopts: Value) -> Rc<TransactionEntity> {
...
impl BluefinDecryptxP2peEntity for TransactionEntity {
    ...
    fn list(self: &Rc<Self>, reqmatch: Value, ctrl: Value) -> Result<Vec<Rc<Self>>, BluefinDecryptxP2peError> {
        ...
        if let Value::List(list) = &out {
            for entry in list.borrow().iter() {
                let ent = TransactionEntity::new(&self.client, vs::clone(&self.entopts));
