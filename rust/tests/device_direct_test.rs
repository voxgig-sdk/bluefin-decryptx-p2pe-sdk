// Generated direct-call tests for the device entity (mirrors the
...
fn device_direct_setup(mockres: Value) -> DeviceDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFINDECRYPTXP_PE_TEST_DEVICE_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFINDECRYPTXP_PE_TEST_DEVICE_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return DeviceDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn device_direct_list() {
    let setup = device_direct_setup(ja(vec![
        jo(vec![("id", Value::str("direct01"))]),
        jo(vec![("id", Value::str("direct02"))]),
    ]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-list-device", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
...
#[test]
fn device_direct_load() {
    let setup = device_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-device", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        for live_key in ["device_type01", "serial_number01"] {
    ...
    if setup.live {
        ...
        setp(&params, "serial_number", Value::str("direct02"));
    }

    let result = client
        .direct(jo(vec![
            ("path", Value::str("devices/{serial_number}/{device_type}")),
            ("method", Value::str("GET")),
            ("params", params.clone()),
        ]))
        .expect("direct failed");
