// Generated direct-call tests for the device_custody_detail entity (mirrors the
...
use bluefin_decryptx_p2pe_sdk::core::helpers::{getp, ja, jo, json_thunk, setp, to_int, to_map};
use bluefin_decryptx_p2pe_sdk::utility::voxgigstruct as vs;
use bluefin_decryptx_p2pe_sdk::{Value, BluefinDecryptxP2peSDK};

struct DeviceCustodyDetailDirectSetup {
...
fn device_custody_detail_direct_setup(mockres: Value) -> DeviceCustodyDetailDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_CUSTODY_DETAIL_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_CUSTODY_DETAIL_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return DeviceCustodyDetailDirectSetup {
            client,
            calls,
            live: true,
            idmap,
    ...
    DeviceCustodyDetailDirectSetup {
        client,
        calls,
        live: false,
...
#[test]
fn device_custody_detail_direct_load() {
    let setup = device_custody_detail_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-device_custody_detail", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        for live_key in ["device_type01", "device_custody_detail01", "serial_number01"] {
    ...
    if setup.live {
        setp(&params, "device_type", getp(&setup.idmap, "device_type01"));
    } else {
        setp(&params, "device_type", Value::str("direct01"));
    }
    if setup.live {
        setp(&params, "id", getp(&setup.idmap, "device_custody_detail01"));
    } else {
        setp(&params, "id", Value::str("direct02"));
    }
    if setup.live {
        setp(&params, "serial_number", getp(&setup.idmap, "serial_number01"));
    } else {
        setp(&params, "serial_number", Value::str("direct03"));
    }

    let result = client
        .direct(jo(vec![
            ("path", Value::str("devices/{serial_number}/{device_type}/custody/{id}")),
    ...
    if setup.live {
        ...
        assert!(
            url.contains("direct03"),
            "expected url to contain direct03, got {}",
            url
        );
    }
}
