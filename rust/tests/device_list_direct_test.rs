// Generated direct-call tests for the device_list entity (mirrors the
...
fn device_list_direct_setup(mockres: Value) -> DeviceListDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_LIST_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_LIST_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return DeviceListDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn device_list_direct_load() {
    let setup = device_list_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-device_list", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        for live_key in ["share_partner_to01"] {
    ...
    if setup.live {
        setp(&params, "share_partner_to", getp(&setup.idmap, "share_partner_to01"));
    } else {
        setp(&params, "share_partner_to", Value::str("direct01"));
    }

    let result = client
        .direct(jo(vec![
            ("path", Value::str("virtualDevices/{share_partner_to}")),
