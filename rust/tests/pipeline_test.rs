...
use bluefin_decryptx_p2pe_sdk::core::helpers::{getp, getpath, ja, jo};
...
use bluefin_decryptx_p2pe_sdk::{
    ...
    BluefinDecryptxP2peSDK, Response, SdkResult, Spec, Utility, Value,
};

// plClient builds a client + isolated utility for pipeline utility tests.
fn pl_client(sdkopts: Value) -> (Rc<BluefinDecryptxP2peSDK>, Rc<Utility>) {
...
fn util_with(client: &Rc<BluefinDecryptxP2peSDK>, fetcher: FetcherFn) -> Rc<Utility> {
...
fn pipeline_feature_add_ordering_before_after_replace() {
    ...
    let names = |client: &Rc<BluefinDecryptxP2peSDK>| -> String {
        client
            .features
            .borrow()
            .iter()
...
use bluefin_decryptx_p2pe_sdk::core::helpers::get_str;
