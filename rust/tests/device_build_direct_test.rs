// Generated direct-call tests for the device_build entity (mirrors the
...
fn device_build_direct_setup(mockres: Value) -> DeviceBuildDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_BUILD_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_BUILD_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return DeviceBuildDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn device_build_direct_list() {
    let setup = device_build_direct_setup(ja(vec![
        jo(vec![("id", Value::str("direct01"))]),
        jo(vec![("id", Value::str("direct02"))]),
    ]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-list-device_build", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
...
#[test]
fn device_build_direct_load() {
    let setup = device_build_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-device_build", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        for live_key in ["device_build01"] {
    ...
    let params = Value::empty_map();
    if setup.live {
        setp(&params, "id", getp(&setup.idmap, "device_build01"));
    } else {
    ...
    let result = client
        .direct(jo(vec![
            ("path", Value::str("deviceBuilds/{id}")),
            ("method", Value::str("GET")),
            ("params", params.clone()),
        ]))
        .expect("direct failed");
