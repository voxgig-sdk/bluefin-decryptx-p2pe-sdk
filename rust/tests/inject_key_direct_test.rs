// Generated direct-call tests for the inject_key entity (mirrors the
...
fn inject_key_direct_setup(mockres: Value) -> InjectKeyDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFIN_DECRYPTX_P2PE_TEST_INJECT_KEY_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFIN_DECRYPTX_P2PE_TEST_INJECT_KEY_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return InjectKeyDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn inject_key_direct_list() {
    let setup = inject_key_direct_setup(ja(vec![
        jo(vec![("id", Value::str("direct01"))]),
        jo(vec![("id", Value::str("direct02"))]),
    ]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-list-inject_key", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
...
#[test]
fn inject_key_direct_load() {
    let setup = inject_key_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-inject_key", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    let params = Value::empty_map();
    if setup.live {
        setp(&params, "id", getp(&setup.idmap, "inject_key01"));
    } else {
