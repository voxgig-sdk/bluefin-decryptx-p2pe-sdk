...
fn location_entity_basic() {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINDECRYPTXP_PE_TEST_LOCATION_ENTID JSON to run live");
        return;
    }
    ...
    let location_ref01_data = to_map(&location_ref01_data_result);
    assert!(
    ...
    let location_ref01_list = location_ref01_ent
        ...
        .expect("list failed");
    assert!(
        matches!(location_ref01_list, Value::List(_)),
    ...
    let location_ref01_data_dt0_loaded = location_ref01_ent
        ...
        .expect("load failed");
    let location_ref01_data_dt0_load_result = to_map(&location_ref01_data_dt0_loaded);
    assert!(
    ...
    let location_ref01_list_rt0 = location_ref01_ent
        ...
        .expect("list failed");
    assert!(
        matches!(location_ref01_list_rt0, Value::List(_)),
        "expected list result to be an array"
    );
...
fn location_basic_setup(extra: Value) -> EntityTestSetup {
    ...
    let entid_env_raw = std::env::var("BLUEFINDECRYPTXP_PE_TEST_LOCATION_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINDECRYPTXP_PE_TEST_LOCATION_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINDECRYPTXP_PE_TEST_LOCATION_ENTID")) {
