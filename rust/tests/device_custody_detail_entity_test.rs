...
#[test]
fn device_custody_detail_entity_stream() {
    ...
    let plainent = plainsdk.device_custody_detail(Value::Noval);
...
fn device_custody_detail_entity_basic() {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINDECRYPTXP_PE_TEST_DEVICE_CUSTODY_DETAIL_ENTID JSON to run live");
        return;
    }
    ...
    let device_custody_detail_ref01_data_dt0_loaded = device_custody_detail_ref01_ent
        ...
        .expect("load failed");
    let device_custody_detail_ref01_data_dt0_load_result = to_map(&device_custody_detail_ref01_data_dt0_loaded);
    assert!(
...
fn device_custody_detail_basic_setup(extra: Value) -> EntityTestSetup {
    ...
    let entid_env_raw = std::env::var("BLUEFINDECRYPTXP_PE_TEST_DEVICE_CUSTODY_DETAIL_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINDECRYPTXP_PE_TEST_DEVICE_CUSTODY_DETAIL_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINDECRYPTXP_PE_TEST_DEVICE_CUSTODY_DETAIL_ENTID")) {
