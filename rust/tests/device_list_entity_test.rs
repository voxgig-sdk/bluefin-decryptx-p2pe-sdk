...
#[test]
fn device_list_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.device_list(Value::Noval);
    assert_eq!(ent.get_name(), "device_list");
}

#[test]
fn device_list_entity_stream() {
    ...
    let plainent = plainsdk.device_list(Value::Noval);
...
#[test]
fn device_list_entity_basic() {
    let setup = device_list_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["load"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("device_list.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINDECRYPTXP_PE_TEST_DEVICE_LIST_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();

    // Bootstrap entity data from existing test data (no create step in flow).
    let device_list_ref01_data_raw = vs::items(&to_map(&getpath(&["existing", "device_list"], &setup.data)));
    let device_list_ref01_data = to_map(&vs::get_elem(
        &vs::get_elem(&device_list_ref01_data_raw, &Value::Num(0.0), Value::Noval),
        &Value::Num(1.0),
        Value::Noval,
    ));
    // LOAD
    let device_list_ref01_ent = client.device_list(Value::Noval);
    let device_list_ref01_match_dt0 = Value::empty_map();
    let device_list_ref01_data_dt0_loaded = device_list_ref01_ent
        .load(device_list_ref01_match_dt0.clone(), Value::Noval)
        .expect("load failed");
    assert!(
        !device_list_ref01_data_dt0_loaded.is_noval(),
        "expected load result to be non-nil"
    );

}

fn device_list_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("device_list");
    entity_data_file.push("DeviceListTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("device_list01"), Value::str("device_list02"), Value::str("device_list03"), Value::str("virtual_device01"), Value::str("virtual_device02"), Value::str("virtual_device03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINDECRYPTXP_PE_TEST_DEVICE_LIST_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINDECRYPTXP_PE_TEST_DEVICE_LIST_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINDECRYPTXP_PE_TEST_DEVICE_LIST_ENTID")) {
