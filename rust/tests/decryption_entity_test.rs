...
#[test]
fn decryption_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.decryption(Value::Noval);
    assert_eq!(ent.get_name(), "decryption");
}

#[test]
fn decryption_entity_stream() {
    ...
    let plainent = plainsdk.decryption(Value::Noval);
...
#[test]
fn decryption_entity_basic() {
    let setup = decryption_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("decryption.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINDECRYPTXP_PE_TEST_DECRYPTION_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let decryption_ref01_ent = client.decryption(Value::Noval);
    let decryption_ref01_data = to_map(&getp(
        &getpath(&["new", "decryption"], &setup.data),
        "decryption_ref01",
    ));

    let decryption_ref01_data_result = decryption_ref01_ent
        .create(decryption_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let decryption_ref01_data = to_map(&decryption_ref01_data_result);
    assert!(
        matches!(decryption_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn decryption_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("decryption");
    entity_data_file.push("DecryptionTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("decryption01"), Value::str("decryption02"), Value::str("decryption03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINDECRYPTXP_PE_TEST_DECRYPTION_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINDECRYPTXP_PE_TEST_DECRYPTION_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINDECRYPTXP_PE_TEST_DECRYPTION_ENTID")) {
