...
#[test]
fn kif_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.kif(Value::Noval);
    assert_eq!(ent.get_name(), "kif");
}

#[test]
fn kif_entity_stream() {
    ...
    let plainent = plainsdk.kif(Value::Noval);
...
#[test]
fn kif_entity_basic() {
    let setup = kif_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["list"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("kif.{}", op), mode);
        if skip {
    ...
    let kif_ref01_data_raw = vs::items(&to_map(&getpath(&["existing", "kif"], &setup.data)));
    let kif_ref01_data = to_map(&vs::get_elem(
        &vs::get_elem(&kif_ref01_data_raw, &Value::Num(0.0), Value::Noval),
        &Value::Num(1.0),
        Value::Noval,
    ));
    // LIST
    let kif_ref01_ent = client.kif(Value::Noval);
    let kif_ref01_match = Value::empty_map();
...
fn kif_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("kif");
    entity_data_file.push("KifTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("kif01"), Value::str("kif02"), Value::str("kif03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
