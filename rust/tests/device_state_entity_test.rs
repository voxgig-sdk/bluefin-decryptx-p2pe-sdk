...
fn device_state_entity_basic() {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINDECRYPTXP_PE_TEST_DEVICE_STATE_ENTID JSON to run live");
        return;
    }
    ...
    let device_state_ref01_list = device_state_ref01_ent
        ...
        .expect("list failed");
    assert!(
        matches!(device_state_ref01_list, Value::List(_)),
...
fn device_state_basic_setup(extra: Value) -> EntityTestSetup {
    ...
    let entid_env_raw = std::env::var("BLUEFINDECRYPTXP_PE_TEST_DEVICE_STATE_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINDECRYPTXP_PE_TEST_DEVICE_STATE_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINDECRYPTXP_PE_TEST_DEVICE_STATE_ENTID")) {
