...
#[test]
fn device_state_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.device_state(Value::Noval);
    assert_eq!(ent.get_name(), "device_state");
}

#[test]
fn device_state_entity_stream() {
    ...
    let plainent = plainsdk.device_state(Value::Noval);
...
#[test]
fn device_state_entity_basic() {
    let setup = device_state_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["list"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("device_state.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_STATE_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();

    // Bootstrap entity data from existing test data (no create step in flow).
    let device_state_ref01_data_raw = vs::items(&to_map(&getpath(&["existing", "device_state"], &setup.data)));
    let device_state_ref01_data = to_map(&vs::get_elem(
        &vs::get_elem(&device_state_ref01_data_raw, &Value::Num(0.0), Value::Noval),
        &Value::Num(1.0),
        Value::Noval,
    ));
    // LIST
    let device_state_ref01_ent = client.device_state(Value::Noval);
    let device_state_ref01_match = Value::empty_map();

    let device_state_ref01_list = device_state_ref01_ent
        .list(device_state_ref01_match.clone(), Value::Noval)
        .expect("list failed");
    // list resolves to one ENTITY per record; the flow asserts on the
    // records, so map each through data().
    let device_state_ref01_list = ja(device_state_ref01_list.iter().map(|e| e.data(None)).collect::<Vec<Value>>());

}

fn device_state_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("device_state");
    entity_data_file.push("DeviceStateTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("device_state01"), Value::str("device_state02"), Value::str("device_state03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_STATE_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_STATE_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_STATE_ENTID")) {
