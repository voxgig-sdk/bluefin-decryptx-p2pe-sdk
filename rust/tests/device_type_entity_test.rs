...
fn device_type_entity_basic() {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINDECRYPTXP_PE_TEST_DEVICE_TYPE_ENTID JSON to run live");
        return;
    }
    ...
    let device_type_ref01_list = device_type_ref01_ent
        ...
        .expect("list failed");
    assert!(
        matches!(device_type_ref01_list, Value::List(_)),
    ...
    let device_type_ref01_data_dt0_loaded = device_type_ref01_ent
        ...
        .expect("load failed");
    let device_type_ref01_data_dt0_load_result = to_map(&device_type_ref01_data_dt0_loaded);
    assert!(
...
fn device_type_basic_setup(extra: Value) -> EntityTestSetup {
    ...
    let entid_env_raw = std::env::var("BLUEFINDECRYPTXP_PE_TEST_DEVICE_TYPE_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINDECRYPTXP_PE_TEST_DEVICE_TYPE_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINDECRYPTXP_PE_TEST_DEVICE_TYPE_ENTID")) {
