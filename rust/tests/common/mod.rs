...
/// env_override (mirrors go): when BLUEFIN_DECRYPTX_P2PE_TEST_LIVE or
/// BLUEFIN_DECRYPTX_P2PE_TEST_OVERRIDE is TRUE, environment variables replace the
/// defaults in `m` (JSON-shaped values are parsed).
pub fn env_override(m: Value) -> Value {
    let live = std::env::var("BLUEFIN_DECRYPTX_P2PE_TEST_LIVE").unwrap_or_default() == "TRUE";
    let over = std::env::var("BLUEFIN_DECRYPTX_P2PE_TEST_OVERRIDE").unwrap_or_default() == "TRUE";
    ...
    if let Ok(explain) = std::env::var("BLUEFIN_DECRYPTX_P2PE_TEST_EXPLAIN") {
        if !explain.is_empty() {
            setp(&m, "BLUEFIN_DECRYPTX_P2PE_TEST_EXPLAIN", Value::str(explain));
        }
    }
...
pub struct EntityTestSetup {
    pub client: Rc<BluefinDecryptxP2peSDK>,
    pub data: Value,
    pub idmap: Value,
    pub env: Value,
    pub explain: bool,
...
pub fn match_string(pattern: &str, val: &str) -> bool {
    if pattern.len() >= 2 && pattern.starts_with('/') && pattern.ends_with('/') {
        return bluefin_decryptx_p2pe_sdk::utility::voxgigstruct::re::Regex::new(
...
pub fn runset(
    testspec: &Value,
    subject: &mut dyn FnMut(&Value) -> Result<Value, BluefinDecryptxP2peError>,
) {
...
pub fn make_ctx_from_map(
    ctxmap: &Value,
    client: &Rc<BluefinDecryptxP2peSDK>,
    utility: &Rc<Utility>,
) -> Rc<Context> {
    ...
    if let Value::Map(_) = getp(&ctxmap, "result") {
        ...
        if let Value::Map(_) = getp(&res_map, "err") {
            if let Some(msg) = get_str(&getp(&res_map, "err"), "message") {
                result.borrow_mut().err = Some(BluefinDecryptxP2peError::new("", &msg));
            }
        }
...
pub fn fixctx(ctx: &Rc<Context>, client: &Rc<BluefinDecryptxP2peSDK>) {
    if ctx.options.borrow().is_noval() {
        *ctx.options.borrow_mut() = client.options_map();
    }
}
...
pub fn err_from_map(m: &Value) -> Option<BluefinDecryptxP2peError> {
    let msg = get_str(m, "message").unwrap_or_default();
    if msg.is_empty() {
        return None;
    }
    let code = get_str(m, "code").unwrap_or_default();
    Some(BluefinDecryptxP2peError::new(&code, &msg))
}
...
pub fn fh_has_feature(name: &str) -> bool {
    let config = bluefin_decryptx_p2pe_sdk::make_config();
...
pub type FhReply = Rc<dyn Fn(i64, &Value) -> Result<Value, BluefinDecryptxP2peError>>;
...
pub struct FhOpResult {
    pub ok: bool,
    pub data: Value,
    pub err: Option<BluefinDecryptxP2peError>,
...
fn fh_populate_result(ctx: &Rc<Context>, fetched: &Result<Value, BluefinDecryptxP2peError>) {
    ...
    *ctx.result.borrow_mut() = Some(result.clone());
    ...
    let resp = Response::new(response_map);
    {
        ...
        if let Value::Func(_) = &resp.json {
            r.body = bluefin_decryptx_p2pe_sdk::call_json(&resp.json);
        }
...
impl FhHarness {
    ...
    pub fn op(&self, o: FhOpSpec) -> FhOpResult {
        ...
        *ctx.spec.borrow_mut() = Some(spec.clone());
        ...
        let fetched: Result<Value, BluefinDecryptxP2peError> = match ctx.out_get("request") {
    ...
    fn fail(&self, ctx: Rc<Context>, err: BluefinDecryptxP2peError) -> FhOpResult {
        {
...
pub fn fh_err_code(err: &Option<BluefinDecryptxP2peError>) -> String {
