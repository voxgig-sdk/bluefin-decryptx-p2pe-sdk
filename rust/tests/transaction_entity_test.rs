...
fn transaction_entity_basic() {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_P2PE_TEST_TRANSACTION_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let transaction_ref01_ent = client.transaction(Value::Noval);
    let transaction_ref01_data = to_map(&getp(
        &getpath(&["new", "transaction"], &setup.data),
        "transaction_ref01",
    ));

    let transaction_ref01_data_result = transaction_ref01_ent
        .create(transaction_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let transaction_ref01_data = to_map(&transaction_ref01_data_result.data(None));
    assert!(
        matches!(transaction_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );
    assert!(
        !getp(&transaction_ref01_data, "id").is_noval(),
    ...
    let found_item = vs::select(
        &entity_list_to_data(&transaction_ref01_list),
        &jo(vec![("id", getp(&transaction_ref01_data, "id"))]),
    );
    assert!(
...
fn transaction_basic_setup(extra: Value) -> EntityTestSetup {
    ...
    let entid_env_raw = std::env::var("BLUEFIN_DECRYPTX_P2PE_TEST_TRANSACTION_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFIN_DECRYPTX_P2PE_TEST_TRANSACTION_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFIN_DECRYPTX_P2PE_TEST_TRANSACTION_ENTID")) {
