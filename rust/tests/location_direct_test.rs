// Generated direct-call tests for the location entity (mirrors the
...
fn location_direct_setup(mockres: Value) -> LocationDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFIN_DECRYPTX_P2PE_TEST_LOCATION_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFIN_DECRYPTX_P2PE_TEST_LOCATION_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return LocationDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn location_direct_list() {
    let setup = location_direct_setup(ja(vec![
        jo(vec![("id", Value::str("direct01"))]),
        jo(vec![("id", Value::str("direct02"))]),
    ]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-list-location", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
...
#[test]
fn location_direct_load() {
    let setup = location_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-location", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    let params = Value::empty_map();
    if setup.live {
        setp(&params, "id", getp(&setup.idmap, "location01"));
    } else {
