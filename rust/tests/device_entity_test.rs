...
#[test]
fn device_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.device(Value::Noval);
    assert_eq!(ent.get_name(), "device");
}

#[test]
fn device_entity_stream() {
    ...
    let plainent = plainsdk.device(Value::Noval);
...
#[test]
fn device_entity_basic() {
    let setup = device_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create", "list", "load"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("device.{}", op), mode);
        if skip {
    ...
    // CREATE
    let device_ref01_ent = client.device(Value::Noval);
    let device_ref01_data = to_map(&getp(
        &getpath(&["new", "device"], &setup.data),
        "device_ref01",
    ));
    ...
    assert!(
        !getp(&device_ref01_data, "id").is_noval(),
        "expected created entity to have an id"
    );

    // LIST
    let device_ref01_match = Value::empty_map();
    ...
    let found_item = vs::select(
        ...
        &jo(vec![("id", getp(&device_ref01_data, "id"))]),
    );
    assert!(
    ...
    // LOAD
    let device_ref01_match_dt0 = jo(vec![("id", getp(&device_ref01_data, "id"))]);
    ...
    assert_eq!(
        getp(&device_ref01_data_dt0_load_result, "id"),
        getp(&device_ref01_data, "id"),
        "expected load result id to match"
    );

}

fn device_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("device");
    entity_data_file.push("DeviceTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("device01"), Value::str("device02"), Value::str("device03"), Value::str("serial_number01")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
