// Generated direct-call tests for the device_state entity (mirrors the
...
fn device_state_direct_setup(mockres: Value) -> DeviceStateDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_STATE_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFIN_DECRYPTX_P2PE_TEST_DEVICE_STATE_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return DeviceStateDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn device_state_direct_list() {
    let setup = device_state_direct_setup(ja(vec![
        jo(vec![("id", Value::str("direct01"))]),
        jo(vec![("id", Value::str("direct02"))]),
    ]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-list-device_state", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
